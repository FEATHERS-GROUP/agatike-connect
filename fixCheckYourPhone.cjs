...
for (const file of files) {
  ...
  const earlyReturnRegex = /if\s*\(\s*isPollingPawaPay\s*\|\|\s*\(\(\s*isCheckingOut\s*\|\|\s*isGenerating\s*\)\s*&&\s*paymentMethod\s*===\s*"momo"\)\)\s*\{\s*return\s*\(\s*(?:<div[^>]*>\s*(?:{!isSubdomain[^}]*}|)\s*)?<CheckYourPhone[\s\S]*?\/>\s*(?:<\/div>\s*)?\);\s*\}/;
...
`;
    ...
    const earlyReturnRegex2 = /if\s*\(\s*isPollingPawaPay\s*\|\|\s*\(\s*isCheckingOut\s*&&\s*paymentMethod\s*===\s*"momo"\)\)\s*\{\s*return\s*\(\s*(?:<div[^>]*>\s*(?:{!isSubdomain[^}]*}|)\s*)?<CheckYourPhone[\s\S]*?\/>\s*(?:<\/div>\s*)?\);\s*\}/;
