...
export function SponsoredVouchersPanel({
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      return await batchGenerateSponsoredVouchers({
        data: {
          ...
          generation_type:
            formData.generation_type === "on_purchase" ? "on_purchase" : formData.generation_type,
          linked_ticket_ids:
    ...
    onSuccess: () => {
      toast.success(
        formData.generation_type === "link_existing" ? "Campaign linked!" : "Campaign created!",
      );
      setOpen(false);
      setFormData({
        batch_name: "",
  ...
  const entityLabel =
    entityType === "experience" ? "experience" : entityType === "venue" ? "venue" : "cinema";
    ...
    <div className="bg-card border border-border/60 rounded-3xl p-6 shadow-sm">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Create a standalone batch or attach to ticket purchases.
              </DialogDescription>
            </DialogHeader>
            <form
              onSubmit={(e) => {
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                >
                  <div className="flex items-center gap-3">
                    <Wand2
                      className={`h-5 w-5 shrink-0 ${formData.generation_type === "manual" ? "text-primary" : "text-muted-foreground"}`}
                    />
                    <div>
                      <p className="font-semibold text-sm">Pre-generate Batch</p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Create a set number of vouchers upfront — e.g. 200 vouchers worth 5,000 RWF
                        each.
                      </p>
                    </div>
                  </div>
                ...
                {entityTickets.length > 0 && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Ticket
                        className={`h-5 w-5 shrink-0 ${formData.generation_type === "ticket_linked" ? "text-primary" : "text-muted-foreground"}`}
                      />
                      <div>
                        <p className="font-semibold text-sm">Per Ticket / Booking Tier</p>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Automatically generate one voucher for each customer who buys the selected
                          ticket type. No quantity needed.
                        </p>
                      </div>
                    </div>
                  </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    <ShoppingBag
                      className={`h-5 w-5 shrink-0 ${formData.generation_type === "on_purchase" ? "text-primary" : "text-muted-foreground"}`}
                    />
                    <div>
                      <p className="font-semibold text-sm">Create One On Every Purchase</p>
                      <p className="text-xs text-muted-foreground mt-0.5">
                        One voucher is created automatically for every single booking at this{" "}
                        {entityLabel} — no tier selection needed.
                      </p>
                    </div>
                  </div>
                </button>
                ...
                {unlinkedBatches.length > 0 && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <LinkIcon
                        className={`h-5 w-5 shrink-0 ${formData.generation_type === "link_existing" ? "text-primary" : "text-muted-foreground"}`}
                      />
                      <div>
                        <p className="font-semibold text-sm">Link Existing Batch</p>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Attach a batch you already created from the Products & Add-ons page.
                        </p>
                      </div>
                    </div>
                  </button>
              ...
              {formData.generation_type !== "link_existing" && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="e.g. Welcome Drinks, Free Popcorn..."
                  />
                </div>
              )}
              ...
              {(formData.generation_type === "ticket_linked" ||
                ...
                entityTickets.length > 0 && (
                  <div className="space-y-2">
                    <Label>
                      Trigger on these ticket / booking types{" "}
                      <span className="text-red-500">*</span>
                    </Label>
                    <p className="text-xs text-muted-foreground -mt-1">
                      A voucher will be sent automatically when any of these are purchased.
                    </p>
                    <div className="space-y-2 max-h-[200px] overflow-y-auto p-3 border rounded-xl bg-secondary/5">
                      {entityTickets.map((t) => (
                        <div key={t.id} className="flex items-center space-x-2">
                          ...
                          />
                          <label
                            htmlFor={`svp-ticket-${t.id}`}
                            className="text-sm font-medium cursor-pointer"
                          >
                            {t.name || t.type} — {t.cost?.toLocaleString()} RWF
                          </label>
                        </div>
                      ))}
                    </div>
              ...
              {formData.generation_type === "ticket_linked" && (
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="match_ticket_price">
                        Match Ticket / Booking Price
                      </SelectItem>
              ...
              {(formData.generation_type === "manual" ||
                formData.generation_type === "on_purchase" ||
                (formData.generation_type === "ticket_linked" &&
                  formData.value_type === "fixed")) && (
                <div className="space-y-2">
                  <Label>
                    Value per Voucher (RWF) <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    required
                    type="number"
              ...
              {formData.generation_type === "manual" && (
                <div className="space-y-2">
                  <Label>
                    Number of Vouchers to Generate <span className="text-red-500">*</span>
                  </Label>
                  <Input
                    required
                    type="number"
                  ...
                  <p className="text-xs text-muted-foreground">
                    This many individual vouchers will be created immediately.
                  </p>
                </div>
              )}
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div className="bg-secondary/30 rounded-2xl p-4">
          ...
          <p className="text-2xl font-bold mt-1 text-primary">
            {totalProvisioned.toLocaleString()} RWF
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-10 border border-dashed rounded-2xl text-muted-foreground text-sm">
          No voucher campaigns linked yet.
        </div>
      ) : (
        <div className="space-y-3">
          {batches.map((batch: any) => {
            const spent = (batch.vouchers || []).reduce(
              (s: number, v: any) =>
                s + Number(v.voucher_transactions_aggregate?.aggregate?.sum?.amount || 0),
              0,
            );
            const remaining = (batch.vouchers || []).reduce(
              (s: number, v: any) => s + Number(v.current_balance || 0),
              0,
            );
            ...
            return (
              <div
                key={batch.id}
                className="p-4 rounded-2xl border border-border/50 bg-secondary/10 space-y-2"
              >
                ...
                <p className="text-xs text-muted-foreground">
                  {batch.vouchers?.length || 0} vouchers · {pct}% spent
                </p>
              </div>
            );
          })}
