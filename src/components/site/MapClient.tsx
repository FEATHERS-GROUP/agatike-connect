...
import { ArrowLeft, CheckCircle2, ChevronRight, MapPin, Search, List, Calendar, ShoppingBag, User, Settings2 } from "lucide-react";
...
function MapController({ selectedCity, selectedMarker }: { selectedCity: any | null, selectedMarker: any | null }) {
  const map = useMap();
  useEffect(() => {
    if (selectedMarker) {
...
export default function MapClient() {
  ...
  // Queries
  const { data: dbEvents = [] } = useQuery({ queryKey: ["public-events"], queryFn: () => getPublicEvents() });
  const { data: dbVenues = [] } = useQuery({ queryKey: ["public-venues"], queryFn: () => getPublicVenues() });
  const { data: dbSpaces = [] } = useQuery({ queryKey: ["public-spaces"], queryFn: () => getPublicSpaces() });
  const { data: dbCinemas = [] } = useQuery({ queryKey: ["public-cinemas"], queryFn: () => getPublicCinemas() });
  ...
  const mapMarkers = useMemo(() => {
    ...
    dbEvents.forEach((e: any) => {
      ...
      if (e.allowed_public && !e.deleted && firstStop) {
        markers.push({
          id: `event-${e.id}`, title: e.title, date: new Date(e.created_at).toLocaleDateString(),
          lat: parseFloat(firstStop.latitude), lng: parseFloat(firstStop.longitude),
          image: e.cover || "https://images.unsplash.com/photo-1511192336575-5a79af67a629?auto=format&fit=crop&w=100",
          type: "event", city: normalizeCityName(firstStop.city), raw: e,
        });
      }
    });
    dbVenues.forEach((v: any) => {
      if (v.latitude && v.longitude) {
        markers.push({
          id: `venue-${v.id}`, title: v.name, date: v.city,
          ...
          type: "venue", city: normalizeCityName(v.city), raw: v,
        });
      }
    });
    dbSpaces.forEach((s: any) => {
      ...
      if (loc && loc.lat && loc.lng) {
        markers.push({
          id: `space-${s.id}`, title: s.name, date: loc.city,
          ...
          type: "space", city: normalizeCityName(loc.city), raw: s,
        });
      }
    });
    dbCinemas.forEach((c: any) => {
      if (c.latitude && c.longitude) {
        markers.push({
          id: `cinema-${c.id}`, title: c.name, date: c.city,
          ...
          type: "cinema", city: normalizeCityName(c.city), raw: c,
        });
      }
    });
  ...
  const groupedCities = useMemo(() => {
    const cityMap = new Map<string, { name: string, count: number, image: string, bounds: [number, number][] }>();
    mapMarkers.forEach(m => {
      const c = m.city;
      if (!cityMap.has(c)) {
        const cityImage = m.image || "https://images.unsplash.com/photo-1542204165-65bf26472b9b?auto=format&fit=crop&w=200";
  ...
  return (
    <div className="relative h-[100dvh] w-full bg-background overflow-hidden flex flex-col">
      ...
      <div className="absolute inset-0 z-0">
        <MapContainer center={[-1.9441, 30.0619]} zoom={13} className="h-full w-full" zoomControl={false}>
          <TileLayer
          ...
          {mapMarkers.filter(m => !selectedCityName || m.city === selectedCityName).map((marker) => (
            <Marker
              key={marker.id}
      ...
      <div className="absolute top-safe-top left-0 right-0 z-[20] pt-4 px-4 pointer-events-none flex flex-col gap-3">
        <div className="flex gap-2 pointer-events-auto">
          <Button variant="outline" className="rounded-full w-12 h-12 p-0 shadow-md shrink-0 bg-background/95 backdrop-blur-md" onClick={() => setShowCityModal(true)}>
      ...
      <div className={`absolute bottom-20 left-0 right-0 z-[20] transition-transform duration-300 px-4 ${selectedMarker ? "translate-y-0 opacity-100" : "translate-y-full opacity-0 pointer-events-none"}`}>
        {selectedMarker && (
          <div className="bg-background rounded-3xl shadow-xl border border-border/40 p-5 mb-4 pointer-events-auto relative max-h-[50vh] overflow-y-auto hide-scrollbar flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedMarker.type === "event" && selectedMarker.raw?.event_tickets?.some((t: any) => t.cost === 0) && (
                <span className="text-[9px] font-bold text-green-500 bg-green-500/10 px-2 py-1 rounded-md">FREE TIER AVAILABLE</span>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 flex items-start gap-1">
                ...
                <span className="line-clamp-1">{selectedMarker.raw?.address || selectedMarker.city}</span>
              </p>
            </div>
            ...
            {selectedMarker.type === "event" && selectedMarker.raw?.workspaces?.organizer && (
              <div className="flex items-center gap-3 mt-1">
                <img src={selectedMarker.raw.workspaces.organizer.image} className="h-8 w-8 rounded-full object-cover" />
                <div>
                  <p className="text-[9px] uppercase font-bold text-muted-foreground mb-0.5">Organized by</p>
                  <p className="text-xs font-semibold">{selectedMarker.raw.workspaces.organizer.name}</p>
                </div>
              </div>
            )}
            ...
            <div className="mt-2">
              <Button 
                ...
                onClick={() => {
                  ...
                  if (selectedMarker.type === "event") router.navigate({ to: "/events/$eventId", params: { eventId: rawId } });
                  else if (selectedMarker.type === "venue") router.navigate({ to: "/venues/$venueId", params: { venueId: rawId } });
                  else if (selectedMarker.type === "space") router.navigate({ to: "/spaces/$spaceId", params: { spaceId: rawId } });
                  else if (selectedMarker.type === "cinema") router.navigate({ to: "/cinemas/$cinemaId", params: { cinemaId: rawId } });
                }}
              >
                View Details
              </Button>
      ...
      <div className="absolute bottom-28 right-4 z-[10] pointer-events-auto">
        <Button variant="outline" className="w-12 h-12 rounded-full shadow-lg bg-background border-border/40 p-0" onClick={() => setShowCityModal(true)}>
      ...
      <div className="mt-auto h-20 bg-background border-t border-border/40 z-[50] flex items-center justify-around px-2 pb-safe-bottom pt-2 shrink-0">
         <div className="flex flex-col items-center gap-1 opacity-100 text-primary cursor-pointer" onClick={() => setShowCityModal(true)}>
      ...
      {showCityModal && (
        <div className="fixed inset-0 z-[500] bg-background flex flex-col pointer-events-auto">
          ...
          <div className="h-[30vh] w-full relative shrink-0 bg-muted">
            <img src="https://images.unsplash.com/photo-1534438327276-14e5300c3a48?auto=format&fit=crop&w=1200" className="w-full h-full object-cover" />
            ...
            {/* Back button */}
            <button className="absolute top-safe-top left-4 mt-2 p-2 bg-black/20 backdrop-blur-md rounded-full text-white" onClick={() => router.history.back()}>
