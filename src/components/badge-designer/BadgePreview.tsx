...
interface BadgePreviewProps {
  ...
  handlePointerDown: (e: React.PointerEvent, id: string, initialX: number, initialY: number) => void;
}
...
export function BadgePreview({
  ...
  const renderQRCode = (theme: string, accentColor: string) => {
    ...
    return (
      <div 
        className={`absolute flex flex-col items-center animate-in fade-in zoom-in duration-500 z-40 cursor-move ${draggingId === 'qrcode' ? 'opacity-70 scale-105' : 'hover:scale-105'} transition-transform bg-transparent p-2`}
        style={{ 
          left: `${x}%`, 
          top: `${y}%`, 
      ...
      >
        ...
        <p className={`font-mono text-[9px] mt-3 uppercase tracking-[0.3em] font-bold ${theme === 'minimal' ? 'text-slate-400' : 'text-white/50 drop-shadow-md'}`}>STAFF-DK8492X</p>
      </div>
    );
  };
      ...
      <div className="relative transform transition-all duration-700 hover:scale-[1.02]" style={{ perspective: '1000px' }}>
        ...
        >
          ...
          >
             ...
             {/* Back Custom Text */}
             <div className="flex-1 flex flex-col items-center justify-center px-4 pointer-events-none" style={{ transform: 'translateZ(5px)' }}>
