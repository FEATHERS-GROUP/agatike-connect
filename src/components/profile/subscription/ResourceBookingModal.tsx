...
export function ResourceBookingModal({
  ...
  const resourceBookings = useMemo(() => {
    return allBookings.filter(
      (b: any) => b.resource_id === resource?.id && b.status !== "cancelled",
    );
  ...
  const availableTimeSlots = useMemo(() => {
    ...
    const startHour = resource.rules?.operatingHours?.start
      ? parseInt(resource.rules.operatingHours.start.split(":")[0])
      : 8;
    const endHour = resource.rules?.operatingHours?.end
      ? parseInt(resource.rules.operatingHours.end.split(":")[0])
      : 18;
  ...
  React.useEffect(() => {
    if (
      ...
      (!bookingTime || !availableTimeSlots.includes(bookingTime))
    ) {
  ...
  const handleBook = () => {
    ...
    if (resource.rules?.requireExclusiveBooking !== false) {
      ...
      if (isOverlap) {
        toast.error(
          "The selected duration overlaps with an existing booking. Please reduce duration or pick another time.",
        );
        return;
      }
    }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md rounded-3xl p-0 overflow-hidden border-border/40 shadow-2xl">
        ...
        <div className="flex justify-end gap-3 p-4 bg-secondary/5 border-t border-border/40">
          <Button variant="outline" className="h-11 rounded-xl px-6 font-bold" onClick={onClose}>
            Cancel
          </Button>
          <Button
