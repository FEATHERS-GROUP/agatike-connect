    ...
    <div className="bg-card border border-border/40 rounded-[32px] md:rounded-3xl p-6 md:p-8 shadow-sm animate-in slide-in-from-bottom-4 fade-in duration-500">
      <div className="mb-6 md:mb-8 max-w-xl">
        ...
        <p className="text-muted-foreground text-sm md:text-base leading-relaxed">
          Book a desk, meeting room, or equipment for your next visit.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        {resources?.length > 0 ? (
          resources.map((res: any) => (
            <div
              key={res.id}
              className="group relative bg-secondary/5 rounded-2xl md:rounded-3xl p-5 md:p-6 border border-border/40 hover:border-primary/50 hover:shadow-md transition-all cursor-pointer flex flex-col justify-between"
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs md:text-sm text-muted-foreground line-clamp-2 mb-3">
                  {res.description || `Book this ${res.type.replace(/_/g, " ")} for your work.`}
                </p>
