          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-primary-foreground/80 font-medium text-xs mt-1 mb-2">
                {space?.locations?.[0]?.city ? `${space.locations[0].city}, ` : ""}
                {subscription?.plan_name}
              </p>

              {space?.locations &&
                space.locations.length > 0 &&
                space.locations[0]?.opening_hours && (
                  <Popover>
                    <PopoverTrigger asChild>
                      <button className="flex items-center gap-1 bg-primary-foreground/10 hover:bg-primary-foreground/20 px-2 py-1 rounded-lg text-[10px] font-bold transition-colors w-fit text-left">
                        ...
                        <span>
                          {space.locations.length} {space.locations.length === 1 ? "Loc" : "Locs"}
                        </span>
                        <span className="opacity-50">·</span>
                        <Clock className="h-3 w-3 shrink-0 ml-0.5" />
                        <span className="truncate max-w-[100px]">
                          Today: {todayHours(space.locations[0].opening_hours) || "See hours"}
                        </span>
                        <ChevronDown className="h-3 w-3 shrink-0 opacity-70 ml-0.5" />
                      </button>
                    </PopoverTrigger>
                    <PopoverContent
                      className="w-[calc(100vw-32px)] p-4 rounded-2xl shadow-xl border-border/40 mx-4 max-h-[60vh] overflow-y-auto"
                      align="start"
                    >
                          ...
                          <div key={i} className="space-y-2">
                            ...
                            {loc.opening_hours && (
                              <div className="pl-6 space-y-1">
                                {summarizeHours(loc.opening_hours).map(
                            ...
                            {i < space.locations.length - 1 && (
                              <div className="border-b border-border/40 my-3 pt-2" />
                            )}
                          </div>
                        ))}
                      </div>
        ...
        <div className="grid grid-cols-3 gap-4 border-t border-primary-foreground/20 pt-6">
          <div className="text-center">
            ...
            <p className="text-[9px] uppercase tracking-wider font-bold text-primary-foreground/70 mt-1">
              ...
              {subscription?.billing_cycle?.substring(0, 3)}
            </p>
          </div>
          <div className="text-center border-l border-r border-primary-foreground/20 px-2">
            <p className="text-lg font-bold mt-1">
              {subscription?.start_date ? format(new Date(subscription.start_date), "MMM d") : "-"}
            </p>
            <p className="text-[9px] uppercase tracking-wider font-bold text-primary-foreground/70 mt-1">
              Start
            </p>
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center justify-center w-14 h-14 rounded-[20px] shadow-sm transition-all snap-center ${
                  isActive
                    ? "bg-background text-primary scale-110 shadow-md"
                    : "bg-primary-foreground/20 text-primary-foreground hover:bg-primary-foreground/30"
                }`}
              >
                <Icon
                  className={`h-6 w-6 ${isActive ? "text-primary" : "text-primary-foreground"}`}
          ...
          <div className="space-y-4">
            {resources?.length > 0 ? (
              resources.map((res: any) => (
                <div
                  key={res.id}
                  className="bg-background rounded-[32px] p-5 shadow-sm border border-border/40 flex items-center justify-between"
                >
                  <div className="flex items-center gap-4">
          ...
          <div className="space-y-4">
            {myResourceBookings.length > 0 ? (
              myResourceBookings.map((b: any) => (
                <div
                  key={b.id}
                  className="bg-background rounded-[32px] p-5 shadow-sm border border-border/40 relative overflow-hidden"
                >
              ...
              <div className="text-center py-16 bg-background rounded-[32px] border border-border/40">
                ...
                <Button className="mt-4 rounded-xl px-6" onClick={() => setActiveTab("resources")}>
                  Book a Resource
                </Button>
              </div>
            )}
