        ...
        <div className="relative mx-auto max-w-7xl px-4 md:px-6 py-12 md:py-20 lg:py-24">
          <div className="max-w-2xl text-left">
            ...
            <p className="mt-4 text-muted-foreground md:text-lg font-medium">
              Get access tickets to parks, museums, gaming centers, and more. Skip the line and enjoy your time.
            </p>
          </div>
        ...
        {filteredVenues.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredVenues.map((venue) => (
              <div key={venue.id} className="group relative overflow-hidden rounded-3xl border border-border/50 bg-card shadow-[var(--shadow-card)] transition-all hover:shadow-[var(--shadow-glow)] hover:-translate-y-1">
                <div className="aspect-video relative overflow-hidden">
                  <img src={venue.cover} alt={venue.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                ...
                <div className="p-6">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mt-3 text-sm text-muted-foreground font-medium">
                    ...
                    <span className="flex items-center gap-1"><Clock className="w-4 h-4" /> {venue.openTime} - {venue.closeTime}</span>
                  </div>
                  <p className="mt-4 text-sm text-muted-foreground line-clamp-2">{venue.description}</p>
                  
                  <div className="mt-6 flex items-center justify-between border-t border-border/40 pt-4">
                    <div>
                      <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider mb-1">Entry Fee</p>
                      <span className="text-xl font-bold text-primary">{venue.price > 0 ? `${venue.currency} ${venue.price}` : "Free"}</span>
                    </div>
                    <Link to="/venues/$venueId" params={{ venueId: venue.id }}>
                      <Button className="rounded-xl px-6 shadow-[var(--shadow-glow)] transition-all group-hover:scale-105" style={{ background: "var(--gradient-primary)" }}>
                        Get Ticket
                      </Button>
                    </Link>
                  </div>
