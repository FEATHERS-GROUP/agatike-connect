...
import { Calendar, Clock, MapPin, Star, Users, Heart, Share2, Plus, Minus, Shield } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative h-[60vh] min-h-[420px] w-full overflow-hidden">
        <img src={event.cover} alt={event.title} className="absolute inset-0 h-full w-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
        <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-10">
          <span className="w-fit rounded-full bg-background/70 px-3 py-1 text-xs backdrop-blur">{event.category}</span>
          <h1 className="mt-3 max-w-3xl text-4xl font-semibold md:text-5xl">{event.title}</h1>
          <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            <span className="inline-flex items-center gap-1"><Calendar className="h-4 w-4" /> {event.date || 'Today'}</span>
            <span className="inline-flex items-center gap-1"><Clock className="h-4 w-4" /> {event.time || event.duration || 'All day'}</span>
            <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" /> {event.venue || event.cinema || event.city}, {event.city}</span>
            <span className="inline-flex items-center gap-1"><Star className="h-4 w-4 fill-primary text-primary" /> {event.rating || "5.0"}</span>
          </div>
        </div>
      </section>
        ...
        <div className="space-y-10">
          <div className="flex items-center justify-between rounded-2xl border border-border/60 bg-card p-4">
            <div className="flex items-center gap-3">
              <img src={event.cover} className="h-12 w-12 rounded-full object-cover" alt={event.organizer || event.host || event.cinema || 'Organizer'} />
              <div>
                <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Organized by</p>
                <p className="font-semibold">{event.organizer || event.host || event.cinema || 'Host'} <span className="text-xs text-muted-foreground">@{event.organizerHandle || 'host'}</span></p>
              </div>
            </div>
          ...
          <div>
            ...
            <p className="mt-3 text-muted-foreground leading-relaxed">{event.description || event.synopsis || "Join us for an exciting experience."} Expect curated sound, immersive lighting and a crowd that brings the energy. Doors open one hour before showtime — bring an ID and your good vibes.</p>
          </div>
          ...
          <div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
              {["DJ Nala", "Burna Sound", "Amapiano Live", "Surprise Guest"].map((n) => (
                <div key={n} className="rounded-2xl border border-border/60 bg-card p-4 text-center">
                  <div className="mx-auto h-14 w-14 rounded-full" style={{ background: "var(--gradient-primary)" }} />
          ...
          <div>
            <h2 className="text-xl font-semibold">People going · {(event.attendees || event.spots || 0).toLocaleString()}</h2>
            <div className="mt-4 flex -space-x-3">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="h-10 w-10 rounded-full border-2 border-background" style={{ background: `oklch(${0.6 + (i % 3) * 0.1} 0.18 ${30 + i * 20})` }} />
              ))}
              <div className="ml-3 grid h-10 place-items-center rounded-full bg-secondary px-3 text-xs font-medium">+ {((event.attendees || event.spots || 0) - 8 > 0 ? (event.attendees || event.spots || 0) - 8 : 0).toLocaleString()}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-4 aspect-[16/9] overflow-hidden rounded-2xl border border-border/60 bg-secondary">
              <div className="h-full w-full bg-[linear-gradient(135deg,oklch(0.95_0.02_60),oklch(0.85_0.05_50))] flex items-center justify-center text-muted-foreground">
                <MapPin className="mr-2 h-5 w-5" /> {event.venue || event.cinema || event.city}, {event.city}
              </div>
            </div>
          </div>
            ...
            <div className="mt-4 space-y-3">
              {[
                ...
                <div key={r.n} className="rounded-2xl border border-border/60 bg-card p-4">
                  <div className="flex items-center gap-2 text-sm font-medium">{r.n}<span className="ml-auto inline-flex items-center gap-1 text-xs text-muted-foreground"><Star className="h-3 w-3 fill-primary text-primary" /> 5.0</span></div>
        ...
        <aside className="lg:sticky lg:top-24 h-fit">
          <div className="rounded-3xl border border-border/60 bg-card p-6 shadow-[var(--shadow-card)]">
            ...
            <div className="mt-5 rounded-2xl border border-dashed border-border p-4 text-center">
              <div className="mx-auto grid h-28 w-28 place-items-center rounded-xl bg-foreground text-background text-xs font-mono">QR PREVIEW</div>
              <p className="mt-3 text-xs text-muted-foreground">Tickets are scanned at the door from your phone.</p>
            </div>
          </div>

          <div className="mt-4 flex items-center justify-between rounded-2xl border border-border/60 bg-card p-4 text-sm">
            <span className="inline-flex items-center gap-2 text-muted-foreground"><Users className="h-4 w-4" /> {(event.attendees || event.spots || 0).toLocaleString()} going</span>
            <Link to="/feed" className="text-primary hover:underline">See moments</Link>
          </div>
        </aside>
      </div>
