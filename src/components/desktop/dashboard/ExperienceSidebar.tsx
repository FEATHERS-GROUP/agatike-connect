...
const steps = ["Details", "Tickets", "Itinerary", "Media", "Products", "VIP", "Publish"] as const;
...
export function ExperienceSidebar() {
  ...
  // Extract experienceId from the pathname: /dashboard/kigali-arenas/experiences/123/...
  const pathParts = location.pathname.split("/");
  const experienceId = pathParts[4] || "";
  const isCreateExperience = experienceId === "create-experience";

  // For create experience, get current step from URL search params
  const currentStep = (location.search as any)?.step || 0;

  const nav = [
    {
      label: "Overview",
      href: `/dashboard/${workspaceSlug}/experiences/${experienceId}`,
      icon: LayoutDashboard,
    },
    {
      label: "Attendees",
      href: `/dashboard/${workspaceSlug}/experiences/${experienceId}/attendees`,
      icon: Users,
    },
    {
      label: "Budget & Planning",
      href: `/dashboard/${workspaceSlug}/experiences/${experienceId}/planning`,
      icon: Wallet,
    },
    {
      label: "Staff",
      href: `/dashboard/${workspaceSlug}/experiences/${experienceId}/staff`,
      icon: UserCheck,
    },
    {
      label: "Products & Add-ons",
      href: `/dashboard/${workspaceSlug}/experiences/${experienceId}/products&add-ons`,
      icon: ShoppingBag,
    },
    {
      label: "Experience",
      href: `/dashboard/${workspaceSlug}/experiences/${experienceId}/experience`,
      icon: Sparkles,
    },
  ];
