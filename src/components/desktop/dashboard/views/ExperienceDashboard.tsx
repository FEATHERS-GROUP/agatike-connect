...
export function ExperienceDashboard() {
  ...
  // -- Queries --
  // Experiences currently share the events table structure
  ...
  const stats = useMemo(() => {
    ...
    const topExperiencesList = rawExperiences.map((e: any) => {
      let sold = 0;
      let rev = 0;
      ...
      if (stops.length > 0) {
        ...
        if (lastStop && lastStop.date) {
          ...
          if (stopDate < now) {
            isCompleted = true;
            if (stopDate.getFullYear() === currentYear) completedExperiences++;
          } else {
            pendingExperiences++;
          }
        } else {
    ...
    // Sort Top Experiences by Sold (primary) and Rating (secondary)
    topExperiencesList.sort((a, b) => b.sold - a.sold || b.avgRating - a.avgRating);
    const topExperiences = topExperiencesList.slice(0, 5);
  ...
  const kpis = [
    ...
    {
      label: "Completed (YTD)",
      value: stats.completedExperiences,
      icon: CheckCircle2,
      color: "text-blue-500",
      bg: "bg-blue-500/10",
    },
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <section className="rounded-3xl border border-border/60 bg-card p-6 shadow-sm flex flex-col">
          <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
            <Star className="h-5 w-5 text-primary fill-primary" /> Top Experiences
          </h2>
          ...
          {stats.topExperiences.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center text-muted-foreground p-8 border border-dashed border-border/60 rounded-2xl">
              ...
              <p className="text-xs mt-1">Create an experience to see it here.</p>
            </div>
          ) : (
              ...
              {stats.topExperiences.map((exp: any, idx: number) => (
                ...
                >
                  ...
                  <div className="w-12 h-12 rounded-xl bg-secondary overflow-hidden shrink-0">
                    {exp.cover ? (
                      <img src={exp.cover} alt={exp.title} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <Users className="h-3 w-3" /> {exp.sold.toLocaleString()}
                      </span>
                      <span className="flex items-center gap-1 text-orange-500 font-medium">
                        <Star className="h-3 w-3 fill-orange-500" />{" "}
                        {exp.avgRating ? exp.avgRating.toFixed(1) : "0.0"}
                      </span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold">
                      {formatCurrency(exp.rev, activeWorkspace?.currency)}
                    </p>
                  </div>
                </div>
              ))}
