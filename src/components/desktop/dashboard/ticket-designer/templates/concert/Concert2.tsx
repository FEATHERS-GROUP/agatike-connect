        ...
        {isBack ? (
          <div className="relative flex-1 p-6 flex flex-row border border-white/10 rounded-[24px]">
             <div className="flex-1 pr-6 border-r border-white/10 flex flex-col">
               <div className="text-left border-b border-white/10 pb-3 mb-3">
                 <span className="text-sm font-black tracking-[0.3em] uppercase opacity-80">{logoText}</span>
               </div>
             ...
             <div className="w-[180px] pl-6 flex flex-col items-center justify-center">
                ...
                <p className="text-[10px] uppercase tracking-widest text-white/60 mb-1">Scan for Entry</p>
          ...
          <>
            ...
            <div className="relative flex-1 flex flex-col p-6 overflow-hidden border border-white/10 rounded-l-[24px] border-r-0">
              {/* Dynamic Cover Image */}
              {cover ? (
                <img crossOrigin="anonymous" src={cover} className="absolute inset-0 h-full w-full object-cover opacity-40 mix-blend-luminosity" alt="Cover" />
              ) : (
                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/stardust.png')] opacity-20"></div>
              )}
              {/* Vibrant Gradient Overlay */}
              <div className="absolute inset-0 opacity-60 mix-blend-color" style={{ background: `linear-gradient(45deg, ${palette.from}, ${palette.to})` }}></div>
              ...
              <div className="relative z-10 flex justify-between items-start mb-2">
                 {logoImage ? (
                    ...
                    <span className="text-sm font-black tracking-[0.25em] uppercase drop-shadow-md">{logoText}</span>
                 )}
                 <div className="bg-black/50 backdrop-blur-md px-3 py-1 rounded-full border border-white/20">
                    <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: palette.from }}>{tier}</span>
                 </div>
              </div>

              {/* Title Block */}
              <div className="relative z-10 flex-1 flex flex-col justify-end pb-2" style={{ textAlign: layout.titleAlign as any, marginBottom: `${layout.titleOffsetY}%` }}>
                 <p className="text-xs uppercase tracking-widest font-bold text-white/70 mb-1" style={{ fontSize: `${layout.subtitleSize}px`, color: palette.to }}>
                    {subtitle}
                 </p>
                 <h2 className="font-black leading-none tracking-tight drop-shadow-lg" style={{ fontSize: `${layout.titleSize + 8}px` }}>
                    {title}
                 </h2>
              </div>
            </div>
            ...
            <div className="relative w-1 h-full z-20 flex items-center justify-center">
               ...
               <div className="w-[2px] h-full shadow-[0_0_10px_rgba(255,255,255,0.5)]" style={{ background: `linear-gradient(to bottom, ${palette.from}, ${palette.to})` }}></div>
            </div>
            ...
            <div className="relative w-[220px] bg-[#111827] flex flex-col justify-between p-6 overflow-hidden border border-white/10 rounded-r-[24px] border-l-0">
               ...
               <div className="absolute -bottom-10 -right-10 text-[100px] opacity-5 rotate-[-15deg] font-black">#</div>
               
               <div className="relative z-10 flex flex-col gap-4" style={{ fontSize: `${layout.metaSize}px` }}>
                  <div>
                     <p className="text-[8px] uppercase tracking-widest text-white/50 mb-0.5">Date & Time</p>
                     <p className="font-bold text-sm">{date} • {time}</p>
                  </div>
                  <div>
                     <p className="text-[8px] uppercase tracking-widest text-white/50 mb-0.5">Section / Row / Seat</p>
               ...
               <div className="relative z-10 flex justify-between items-end mt-auto">
                  <div>
                     <p className="text-[8px] uppercase tracking-widest text-white/50 mb-1">Admit One</p>
                     <p className="font-black text-xl" style={{ color: palette.from }}>{currency}{price}</p>
                  </div>
