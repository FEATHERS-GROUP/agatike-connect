      ...
      >
        {isBack ? (
          <div className="relative flex-1 p-6 flex flex-col justify-between">
            <div
              className="flex justify-between items-start border-b-2 border-dashed pb-4"
            ...
            <div
              className="ticket-back-content text-[10px] opacity-80 leading-relaxed py-4 text-[#1a1a1a]"
              dangerouslySetInnerHTML={{ __html: back.backText || "" }}
            />
          ...
          <>
            ...
            <div className="relative z-10 flex-1 p-6 flex flex-col">
                ...
                <div className="flex items-center gap-3">
                  {logoImage && (
                    <img
                      ...
                      className="max-w-[100px] object-contain cursor-pointer"
                      alt="Logo"
                      onClick={onLogoClick}
                    />
                  )}
                  <span className="text-xs font-black tracking-[0.3em] uppercase opacity-80">
                    {logoText}
                  </span>
                </div>
                <div
                  className="border-2 px-3 py-1 rounded-full"
                  style={{ borderColor: palette.from, color: palette.from }}
                >
              ...
              >
                <h2
                  ...
                  style={{ fontSize: `${layout.titleSize + 4}px`, color: palette.from }}
                >
                  {title}
                </h2>
                <p
                  className="mt-1 font-medium whitespace-pre-line opacity-70"
              ...
              <div
                className="grid grid-cols-4 gap-4 border-t-2 pt-3 mt-auto"
                style={{ borderColor: palette.from, fontSize: `${layout.metaSize}px` }}
              >
            ...
            <div className="relative w-px h-full flex flex-col justify-between items-center z-20">
              <div
                className="w-6 h-6 rounded-full bg-white absolute -top-3 shadow-inner border-b-2"
                style={{ borderColor: palette.from }}
              />
              <div
                className="h-full w-px border-l-[3px] border-dotted opacity-30"
                style={{ borderColor: palette.from }}
              />
              <div
                className="w-6 h-6 rounded-full bg-white absolute -bottom-3 shadow-inner border-t-2"
                style={{ borderColor: palette.from }}
              />
            </div>
            ...
            {/* Right Tear-off Stub */}
            <div
              className="w-[180px] p-5 flex flex-col justify-between items-center text-center relative"
              style={{ background: `linear-gradient(135deg, ${palette.from}15, ${palette.to}15)` }}
            >
              {cover && (
                <img
                  crossOrigin="anonymous"
              ...
              <div
                className="relative z-10 bg-white p-2 rounded-xl shadow-sm border"
                style={{ borderColor: `${palette.from}33` }}
              >
