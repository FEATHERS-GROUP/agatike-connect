...
export function AddVisitorModal({
  ...
  const canNextVisitor = () => {
    ...
    if (visitorStep === 1)
      return !!visitorSpaceId && !!visitorForm.visitDate && !!visitorForm.hostedBy;
    return true;
  };
    ...
    <Dialog open={open} onOpenChange={(v) => !visitorSubmitting && onOpenChange(v)}>
      <DialogContent className="max-w-md w-[95vw] rounded-3xl p-0 overflow-hidden border-border/60">
          ...
          {visitorStep === 1 && (
            <>
              <div>
                ...
                <select
                  value={visitorSpaceId}
                  onChange={(e) => setVisitorSpaceId(e.target.value)}
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setVisitorForm((f) => ({ ...f, visitDate: e.target.value }))}
                  className="rounded-xl h-10"
                />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-2 block">
                  Hosted by (member / host) *
                </Label>
                <Input
                  placeholder="Name of the member they came with *"
                  value={visitorForm.hostedBy}
                  onChange={(e) => setVisitorForm((f) => ({ ...f, hostedBy: e.target.value }))}
                  className="rounded-xl h-10"
                  list="members-list"
                />
                <datalist id="members-list">
                  {members
                    .filter((m) => m.category === "member")
                    .map((m) => (
                      <option key={m.id} value={m.name} />
                    ))}
                </datalist>
                <p className="text-[11px] text-muted-foreground mt-1">
                  Start typing to autocomplete from existing members.
                </p>
              </div>
              <div>
                <Label className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-2 block">
            ...
            <div className="space-y-3">
              <div className="bg-rose-500/5 border border-rose-500/20 rounded-2xl p-4 space-y-2 text-sm">
                ...
                <div className="border-t border-rose-500/10 pt-2 mt-2">
                  <Row
                    label="Space"
                    value={spaces.find((s) => s.id === visitorSpaceId)?.name || "—"}
                  />
              ...
              {visitorSuccess && (
                <p className="text-sm text-green-500 font-semibold">{visitorSuccess}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="px-6 pb-6 flex gap-3">
          ...
          {visitorStep < 2 ? (
            <Button
              className="gap-1 rounded-xl flex-1 bg-rose-500 hover:bg-rose-500/90 text-white"
              disabled={!canNextVisitor()}
              onClick={() => setVisitorStep((s) => s + 1)}
            >
            ...
            <Button
              className="rounded-xl flex-1 bg-rose-500 hover:bg-rose-500/90 text-white"
              disabled={visitorSubmitting}
