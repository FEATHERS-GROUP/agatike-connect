        ...
        return (
          <div key={i} className="flex items-center gap-1 flex-1">
            <div className={cn(
              ...
              active ? "bg-orange-500 text-white shadow-[0_0_12px_rgba(249,115,22,0.5)]" :
            ...
            {i < steps.length - 1 && (
              <div className={cn("h-px flex-1 mx-1 transition-all", done ? "bg-green-500/50" : "bg-border/40")} />
            )}
          </div>
        );
      })}
...
export function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-start gap-2 py-0.5">
      ...
      <span className={cn("text-xs font-semibold text-right", highlight ? "text-orange-500" : "text-foreground")}>{value}</span>
    </div>
  );
}

export function AddMemberModal({ open, onOpenChange, spaces, workspaceId }: { open: boolean, onOpenChange: (open: boolean) => void, spaces: any[], workspaceId: string }) {
  ...
  const [memberForm, setMemberForm] = useState({
    name: "", email: "", phone: "",
    planName: "", price: "", billingCycle: "Monthly",
  ...
  const canNextMember = () => {
    ...
    if (memberStep === 1) return !!memberForm.planName && !!memberForm.price && !!memberForm.startDate;
  ...
  const handleMemberSubmit = async () => {
    ...
    try {
      ...
      const groupPlanName = bookingType === "group" ? `${memberForm.planName} (Group of ${teamMembers.length})` : memberForm.planName;
      ...
      const formattedStart = new Date(memberForm.startDate).toLocaleDateString("en-GB", { day: "2-digit", month: "long", year: "numeric" });

      if (bookingType === "group") {
        await sendCompanyRosterEmail({ data: { to: memberForm.email, companyName: memberForm.name, spaceName: selectedSpace?.name || "Our Space", planName: groupPlanName, price: finalPriceString, billingCycle: memberForm.billingCycle, startDate: formattedStart, invoiceNumber, invoiceDate, memberCount: savedMembers.length, members: savedMembers, pdfBase64 } });
        for (const m of savedMembers) {
          if (m.email) await sendMemberWelcomeEmail({ data: { to: m.email, memberName: m.name, companyName: memberForm.name, spaceName: selectedSpace?.name || "Our Space", planName: memberForm.planName, startDate: formattedStart, membershipId: m.membership_id || "—" } });
        }
      } else {
        await sendSubscriptionConfirmationEmail({ data: { to: memberForm.email, customerName: memberForm.name, spaceName: selectedSpace?.name || "Our Space", planName: memberForm.planName, price: finalPriceString, billingCycle: memberForm.billingCycle, startDate: memberForm.startDate } });
        await sendSubscriptionInvoiceEmail({ data: { to: memberForm.email, customerName: memberForm.name, spaceName: selectedSpace?.name || "Our Space", planName: memberForm.planName, price: finalPriceString, billingCycle: memberForm.billingCycle, invoiceDate, invoiceNumber, startDate: memberForm.startDate, pdfBase64 } });
      }
      ...
      setTimeout(() => {
        onOpenChange(false);
        setMemberStep(0);
        setMemberForm({ name: "", email: "", phone: "", planName: "", price: "", billingCycle: "Monthly", startDate: new Date().toISOString().split("T")[0] });
      ...
      <DialogContent className="max-w-lg w-[95vw] rounded-3xl p-0 overflow-hidden border-border/60">
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4">
          {memberStep === 0 && (
            <>
              <div>
                <Label className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-2 block">Booking Type</Label>
                <div className="flex gap-3">
                  {(["individual", "group"] as const).map(t => (
                    <button key={t} type="button" onClick={() => setBookingType(t)}
                      className={cn("flex-1 h-11 rounded-xl text-sm font-semibold border transition-all",
                        bookingType === t ? "border-orange-500 bg-orange-500/10 text-orange-500" : "border-border/60 bg-card text-muted-foreground hover:border-orange-500/40"
                      )}>
              ...
              <div>
                ...
                <select value={selectedSpaceId} onChange={e => { setSelectedSpaceId(e.target.value); setMemberForm(f => ({ ...f, planName: "", price: "" })); }}
            ...
            <>
              <div>
                ...
                {availablePlans.length > 0 ? (
                  <select value={memberForm.planName}
                    onChange={e => {
                      const p = availablePlans.find((x: any) => x.name === e.target.value);
                      setMemberForm(f => ({ ...f, planName: e.target.value, price: p?.price ? String(p.price) : f.price, billingCycle: p?.billing_cycle || f.billingCycle }));
                    }}
                    className="w-full h-10 rounded-xl border border-border/60 bg-card px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40">
                    <option value="">Select a plan…</option>
                    {availablePlans.map((p: any) => <option key={p.name} value={p.name}>{p.name} — {selectedSpace?.currency} {p.price}</option>)}
                  </select>
                ) : (
                  <Input placeholder="Plan name" value={memberForm.planName} onChange={e => setMemberForm(f => ({ ...f, planName: e.target.value }))} className="rounded-xl h-10" />
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input type="number" min="0" placeholder="0" value={memberForm.price} onChange={e => setMemberForm(f => ({ ...f, price: e.target.value }))} className="rounded-xl h-10" />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-2 block">Billing Cycle</Label>
                  <select value={memberForm.billingCycle} onChange={e => setMemberForm(f => ({ ...f, billingCycle: e.target.value }))}
                    className="w-full h-10 rounded-xl border border-border/60 bg-card px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/40">
              ...
              <div>
                ...
                <Input type="date" value={memberForm.startDate} onChange={e => setMemberForm(f => ({ ...f, startDate: e.target.value }))} className="rounded-xl h-10" />
              </div>
            </>
          )}
          ...
          {memberStep === 2 && (
            <>
              ...
              <Input placeholder={bookingType === "group" ? "Company name *" : "Full name *"} value={memberForm.name} onChange={e => setMemberForm(f => ({ ...f, name: e.target.value }))} className="rounded-xl h-10" />
              <Input type="email" placeholder="Email address *" value={memberForm.email} onChange={e => setMemberForm(f => ({ ...f, email: e.target.value }))} className="rounded-xl h-10" />
              <Input placeholder="Phone number *" value={memberForm.phone} onChange={e => setMemberForm(f => ({ ...f, phone: e.target.value }))} className="rounded-xl h-10" />

              {bookingType === "group" && (
                <div className="border-t border-border/40 pt-3 space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Team Members</p>
                    <Button type="button" variant="ghost" size="sm" className="h-7 gap-1 text-xs rounded-xl"
                      onClick={() => setTeamMembers(t => [...t, { name: "", email: "", phone: "" }])}>
                    ...
                    <div key={i} className="p-3 rounded-2xl border border-border/40 bg-secondary/5 space-y-2">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {teamMembers.length > 1 && (
                          <button type="button" onClick={() => setTeamMembers(t => t.filter((_, idx) => idx !== i))}
          ...
          {memberStep === 3 && (
            <div className="space-y-3">
              <div className="bg-secondary/10 rounded-2xl p-4 space-y-2 text-sm">
                <Row label="Type" value={bookingType === "group" ? "Company / Group" : "Individual"} />
                <Row label="Space" value={selectedSpace?.name || "—"} />
                <Row label="Plan" value={memberForm.planName} />
                <Row label="Price" value={`${selectedSpace?.currency || "RWF"} ${memberForm.price}`} />
                ...
                <div className="border-t border-border/30 pt-2 mt-2">
                  ...
                  {bookingType === "group" && <Row label="Team size" value={`${teamMembers.length} member${teamMembers.length !== 1 ? "s" : ""}`} />}
                </div>
              </div>
              <p className="text-xs text-muted-foreground">
                A confirmation email and invoice PDF will be sent to <strong>{memberForm.email}</strong>.
                {bookingType === "group" && " Each team member will also receive a welcome email with their personal Membership ID."}
              </p>
              {memberError && <p className="text-sm text-red-500 font-medium">{memberError}</p>}
              {memberSuccess && <p className="text-sm text-green-500 font-semibold">{memberSuccess}</p>}
            </div>
          )}
        </div>
        ...
        <div className="px-6 pb-6 flex gap-3">
          {memberStep > 0 && (
            <Button variant="outline" className="gap-1 rounded-xl flex-1" onClick={() => setMemberStep(s => s - 1)} disabled={memberSubmitting}>
          ...
          {memberStep < 3 ? (
            <Button className="gap-1 rounded-xl flex-1" style={{ background: "var(--gradient-primary)" }}
              disabled={!canNextMember()} onClick={() => setMemberStep(s => s + 1)}>
              Next <ChevronRight className="h-4 w-4" />
            </Button>
          ) : (
            <Button className="gap-1 rounded-xl flex-1" style={{ background: "var(--gradient-primary)" }}
              disabled={memberSubmitting} onClick={handleMemberSubmit}>
              {memberSubmitting ? <><Loader2 className="h-4 w-4 animate-spin mr-1" />Processing…</> : "Register & Send Invoice"}
            </Button>
          )}
        </div>
      </DialogContent>
