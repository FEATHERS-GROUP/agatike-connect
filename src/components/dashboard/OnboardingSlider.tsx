  ...
  useEffect(() => {
    if (pricingPlans.length > 0 || providerFees.length > 0) {
      ...
      pricingPlans.forEach((plan: any) => {
        ...
        console.log(`- ${plan.name}: ${orgCollection}% (Sale) + ${withdrawalFee}% (Withdraw) + ~${highestProviderFee}% (Network) = ${(orgCollection + withdrawalFee + highestProviderFee).toFixed(1)}% ${totalFixed > 0 ? `+ ${totalFixed} Fixed ` : ''}Total`);
      });
      console.groupEnd();
    }
  ...
  const slides = [
    {
      ...
      description: "Powering the next generation of live experiences. Manage your events, analyze data, and engage your audience all in one unified platform.",
      image: "/illustrations/slide_0.png"
    },
    {
      title: "Ticketing, Badges & Access",
      description: "Design custom digital tickets and staff badges. Use our Venue Designer to map seating, and securely scan attendees at the gate with the Agatike Scanner app.",
      image: "/illustrations/slide_1.png"
    },
    {
      title: "Forms, Books & Page Builder",
      description: "Gather data using custom RSVP forms. Track expenses and rosters in your Agatike Books, and launch beautiful branded event landing pages without writing code.",
      image: "/illustrations/slide_2.png"
    },
    {
      title: "Unified Wallets & Vendors",
      description: "Track all ticket sales, physical merch, and sponsored vouchers in one place. We integrate directly with Mobile Money for seamless customer checkouts.",
      ...
      <DialogContent className="sm:max-w-4xl p-0 overflow-hidden rounded-[24px] border-0 shadow-2xl bg-white dark:bg-[#0a0a0a]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 h-[600px]">
          ...
          <div className="flex flex-col h-full min-h-0 overflow-hidden relative">
            ...
            <div className={`flex-1 min-h-0 overflow-y-auto p-8 flex flex-col ${currentSlide < 4 ? 'justify-center pt-16' : 'justify-start pt-10'}`}>
              {currentSlide < 4 ? (
                ...
                <div className="animate-in fade-in slide-in-from-right-8 duration-500 space-y-6">
                  ...
                  <div className="space-y-3">
                    {pricingPlans.map((plan: any) => {
                      ...
                      return (
                        <div key={plan.id} className={`p-4 rounded-xl border ${isBasic ? 'border-primary shadow-sm bg-primary/[0.02]' : 'border-gray-200 dark:border-white/10'} flex justify-between items-center`}>
                          <div>
                            <div className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                              {plan.name}
                              {isBasic && <span className="text-[10px] bg-primary/20 text-primary px-2 py-0.5 rounded-full">Current Plan</span>}
                            </div>
                            <div className="text-xs text-gray-500 dark:text-white/50 mt-1">
                              {orgCollection}% (Sale) per ticket sold + {parseFloat((withdrawalFee + highestProviderFee).toFixed(1))}% (Withdraw){totalFixed > 0 && ` + ${totalFixed} RWF (Network)`}
                            </div>
                          </div>
                          <div className="text-xl font-extrabold text-primary flex items-end gap-1">
                            {totalRate}% {totalFixed > 0 && <span className="text-sm font-medium text-gray-500 pb-0.5">+ {totalFixed}</span>}
                          </div>
                        </div>
                      );
                    })}
                  ...
                  <div className="text-[10px] text-gray-400 text-center">
                    * Customer collection fees are charged directly to the buyer on checkout and are excluded from the above organizer rates.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="p-6 border-t border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/[0.01]">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={nextSlide} className="px-8 shadow-[0_0_15px_rgba(242,87,29,0.3)] hover:shadow-[0_0_20px_rgba(242,87,29,0.5)] transition-all">
