...
export function UsersTable({ users, workspaces = [] }: { users: any[]; workspaces?: any[] }) {
  ...
  const selectedUser =
    selectedUsers.length === 1 ? users.find((u) => u.id === selectedUsers[0]) : null;
    ...
    <div className="space-y-4">
            ...
            {selectedUsers.length === 1 && (
              <>
                ...
                <Button
                  ...
                  className="h-8 gap-2"
                  onClick={() =>
                    navigate({ to: `/dashboard/${workspaceSlug}/users/${selectedUsers[0]}/edit` })
                  }
                >
      ...
      <Sheet open={detailsOpen} onOpenChange={setDetailsOpen}>
        <SheetContent className="w-full sm:max-w-md overflow-y-auto">
          ...
          {(() => {
            const u =
              selectedUsers.length === 1 ? users.find((x) => x.id === selectedUsers[0]) : null;
            if (!u) return null;
                ...
                <div className="flex items-center gap-4">
                  <div className="h-16 w-16 rounded-2xl bg-secondary overflow-hidden shrink-0 border border-border/50">
                    <img
                      src={
                        u.image ||
                        `https://api.dicebear.com/7.x/initials/svg?seed=${encodeURIComponent(u.name)}&backgroundColor=f97316`
                      }
                      alt={u.name}
                      className="w-full h-full object-cover"
                    />
                ...
                <div className="space-y-4 pt-4 border-t border-border/40">
                  <div>
                    <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-1.5">
                      <User className="h-3 w-3" /> Role & Status
                    </h4>
                    <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      {u.is_temporary && u.expires_at
                        ? format(new Date(u.expires_at), "PPP")
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {u.pages && u.pages.length > 0 ? (
                        u.pages.map((p: string) => (
                          <span
                            key={p}
                            className="px-2.5 py-1 rounded-lg bg-secondary text-foreground text-xs font-medium font-mono"
                          >
                            {p}
                          </span>
                        ))
