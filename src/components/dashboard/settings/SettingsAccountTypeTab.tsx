...
import { UploadCloud, CheckCircle2, Building2, User, Loader2, KeyRound, ShieldAlert } from "lucide-react";
...
export function SettingsAccountTypeTab({ profile }: SettingsAccountTypeTabProps) {
  ...
  const [targetType, setTargetType] = useState<"business" | "personal">(profile?.business ? "business" : "personal");
  ...
  const sendOtpMutation = useMutation({
    mutationFn: async () => await sendAccountConversionOtp({ data: { email: profile?.email, phone: profile?.phone } } as any),
    onSuccess: (data) => {
    ...
    <div className="animate-in fade-in duration-500 max-w-3xl">
      {step === 0 && (
        <div className="space-y-6">
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">Manage your account classification and capabilities.</p>
          </div>

          <div className="grid md:grid-cols-2 gap-6">
            {/* Personal Card */}
            <Card className={`border-2 transition-all ${!profile?.business ? 'border-primary ring-4 ring-primary/10' : 'border-border hover:border-primary/50'}`}>
              <CardContent className="p-6">
                <div className="flex items-center gap-4 mb-4">
                  <div className={`p-3 rounded-xl ${!profile?.business ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                    <User className="w-6 h-6" />
                  </div>
                  <div>
                    <h3 className="font-semibold text-lg">Personal Account</h3>
                    {!profile?.business && <span className="text-xs font-medium text-primary bg-primary/10 px-2 py-0.5 rounded-full">Current</span>}
                  </div>
                </div>
                <ul className="text-sm space-y-2 mb-6 text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Basic event creation</li>
                  <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Standard payout terms</li>
                  <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Limited features</li>
                </ul>
                {profile?.business && (
                  <Button variant="outline" className="w-full" onClick={() => handleStartConversion("personal")}>
            ...
            <Card className={`border-2 transition-all ${profile?.business ? 'border-primary ring-4 ring-primary/10' : 'border-border hover:border-primary/50'}`}>
              <CardContent className="p-6">
                ...
                <ul className="text-sm space-y-2 mb-6 text-muted-foreground">
                  <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Verified badge</li>
                  <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Priority payouts & low fees</li>
                  <li className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-green-500" /> Advanced analytics</li>
                </ul>
                {!profile?.business && (
                  <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/90" onClick={() => handleStartConversion("business")}>
                    Upgrade to Business
                  </Button>
                )}
              </CardContent>
             ...
             <div className="mt-4 p-4 bg-orange-500/10 border border-orange-500/20 rounded-lg flex gap-3 items-start">
               ...
               <div>
                 ...
                 <p className="text-sm text-orange-600/80 dark:text-orange-300/80 mt-1">
                   Your account conversion is currently pending admin approval. You will be notified once reviewed.
                 </p>
               </div>
             </div>
          )}
      ...
      {step === 1 && (
        <div className="animate-in slide-in-from-right-4 duration-300">
          <Button variant="ghost" onClick={() => setStep(0)} className="mb-4 -ml-4 text-muted-foreground hover:text-foreground">
            ← Back
          </Button>
          <h2 className="text-[17px] font-bold mb-2">Upload Business Certificate</h2>
          <p className="text-sm text-muted-foreground mb-6">Please provide your official business registration certificate.</p>
          ...
          <div className="border-2 border-dashed border-border rounded-xl p-8 text-center hover:border-primary/50 transition-colors bg-muted/30">
            ...
            {certFile && (
              <div className="mt-4 p-3 bg-background border border-border rounded-md text-sm font-medium flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={(e) => { e.preventDefault(); setCertFile(null); }}>Remove</Button>
              </div>
            )}
          </div>
          
          <div className="mt-8 flex justify-end">
            <Button onClick={handleNextFromCert} className="bg-primary text-primary-foreground hover:bg-primary/90">
              Continue to Plans
            </Button>
          </div>
        </div>
      ...
      {step === 2 && (
        <div className="animate-in slide-in-from-right-4 duration-300">
          <Button variant="ghost" onClick={() => setStep(targetType === "business" ? 1 : 0)} className="mb-4 -ml-4 text-muted-foreground hover:text-foreground">
            ← Back
          </Button>
          <h2 className="text-[17px] font-bold mb-2">Select a {targetType === "business" ? "Business" : "Personal"} Plan</h2>
          <p className="text-sm text-muted-foreground mb-6">By continuing, you agree that your next billing cycle will be charged based on the selected plan.</p>
                ...
                <CardContent className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xs text-muted-foreground block">/{plan.billing_cycle === 'yearly' ? 'yr' : 'mo'}</span>
                    </div>
                  </div>
          ...
          <div className="mt-8 flex justify-end">
            <Button onClick={handleNextFromPlan} disabled={!selectedPlanId} className="bg-primary text-primary-foreground hover:bg-primary/90">
              Confirm & Continue
            </Button>
          </div>
        </div>
      ...
      {step === 3 && (
        <div className="animate-in slide-in-from-right-4 duration-300 max-w-sm">
          <Button variant="ghost" onClick={() => setStep(2)} className="mb-4 -ml-4 text-muted-foreground hover:text-foreground">
            ← Back
          </Button>
          ...
          <p className="text-sm text-muted-foreground mb-6">Please enter your current password to authorize this change.</p>
      ...
      {step === 4 && (
        <div className="animate-in slide-in-from-right-4 duration-300 max-w-sm">
          <Button variant="ghost" onClick={() => setStep(3)} className="mb-4 -ml-4 text-muted-foreground hover:text-foreground">
            ← Back
          </Button>
          <h2 className="text-[17px] font-bold mb-2">Check your device</h2>
          <p className="text-sm text-muted-foreground mb-6">We sent a 6-digit verification code to your email and phone. Enter it below to finalize the conversion.</p>
        ...
        <DialogContent className="sm:max-w-md [&>button]:hidden bg-background/95 backdrop-blur-sm border-0 shadow-2xl">
           ...
           <div className="py-12 flex flex-col items-center justify-center text-center">
             ...
             <p className="text-sm text-muted-foreground max-w-[280px]">
               Please do not close this window. We are updating your workspaces, generating new billing profiles, and notifying the admin team.
             </p>
           </div>
        </DialogContent>
      </Dialog>
