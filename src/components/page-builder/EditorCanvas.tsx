          ...
          <div className="bg-card border border-border/60 rounded-xl overflow-hidden shadow-sm">
            ...
            <div
              className={`relative h-[400px] bg-secondary flex flex-col p-8 transition-all ${
                editorState.heroAlign === "top-left" ? "justify-start items-start text-left" :
                editorState.heroAlign === "top-center" ? "justify-start items-center text-center" :
                editorState.heroAlign === "top-right" ? "justify-start items-end text-right" :
                editorState.heroAlign === "center-left" ? "justify-center items-start text-left" :
                editorState.heroAlign === "center" ? "justify-center items-center text-center" :
                editorState.heroAlign === "center-right" ? "justify-center items-end text-right" :
                editorState.heroAlign === "bottom-left" ? "justify-end items-start text-left" :
                editorState.heroAlign === "bottom-center" ? "justify-end items-center text-center" :
            ...
            >
              ...
              <div className="absolute top-3 right-3 z-20 flex gap-2">
                <div className="bg-card/90 backdrop-blur border border-border/50 rounded-md flex items-center p-1 shadow-sm gap-1">
                   <div className="flex flex-col gap-1 pr-2 border-r border-border/50">
                     <span className="text-[9px] font-bold text-muted-foreground px-1 uppercase leading-none">Align</span>
                     <div className="grid grid-cols-3 gap-0.5 px-1">
                       {["top-left", "top-center", "top-right", "center-left", "center", "center-right", "bottom-left", "bottom-center", "bottom-right"].map((align) => (
                         <button
                           key={align}
                   ...
                   <div className="flex flex-col gap-1 px-2 border-r border-border/50">
                     <span className="text-[9px] font-bold text-muted-foreground uppercase leading-none">Overlay</span>
                      ...
                      {editorState.heroForegroundImageUrl && (
                        <>
                          <Button
                            ...
                            onClick={() => set("heroForegroundPosition")(editorState.heroForegroundPosition === "left" ? "right" : "left")}
                          >
              ...
              <div className={`relative z-10 w-full max-w-5xl mx-auto flex items-center gap-8 ${
                 editorState.heroForegroundImageUrl && editorState.heroForegroundPosition === "left" ? "flex-row-reverse" : ""
              }`}>
                <div className={`flex flex-col space-y-3 flex-1 ${
                  editorState.heroAlign?.includes("left") ? "items-start text-left" :
                  editorState.heroAlign?.includes("right") ? "items-end text-right" :
                  ...
                  <Input
                    ...
                    className={`text-3xl md:text-5xl font-bold bg-transparent border-transparent text-white placeholder:text-white/40 focus:bg-black/30 hover:bg-black/20 px-2 h-auto py-1 w-full ${
                      editorState.heroAlign?.includes("left") ? "text-left" :
                      editorState.heroAlign?.includes("right") ? "text-right" :
                      "text-center"
                    }`}
                  />
                  <textarea
                  ...
                  <div className={`flex flex-wrap items-center gap-2 mt-4 bg-black/40 p-2 rounded-lg border border-white/20 hover:border-white/40 transition-colors`}>
                    <Input
                    ...
                    {editorState.heroButtonActionType === "page" ? (
                      ...
                      >
                        <option value="" className="text-black">Select Page...</option>
                        {allPages?.map((p: any) => (
                          <option key={p.id} value={`/p/${p.slug}`} className="text-black">{p.title || p.slug}</option>
                        ))}
                      </select>
                      ...
                      <select
                        ...
                        className="h-8 bg-transparent border border-white/20 text-white placeholder:text-white/50 w-[160px] text-xs rounded-md px-2 outline-none"
                      >
                        <option value="" className="text-black">Select Form...</option>
                        {forms?.filter((f:any)=>f.is_active).map((f: any) => (
                          <option key={f.id} value={`#form-${f.id}`} className="text-black">{f.title}</option>
                        ))}
                      </select>
                    ) : (
                      <Input
                        value={editorState.heroButtonLink || ""}
                        onChange={(e) => set("heroButtonLink")(e.target.value)}
                        placeholder={editorState.heroButtonActionType === "phone" ? "+1234567890" : "https://..."}
                ...
                {editorState.heroForegroundImageUrl && (
                  <div className="flex-1 hidden md:block relative h-64 border-2 border-dashed border-white/30 rounded-xl bg-black/20 overflow-hidden">
                     <img src={editorState.heroForegroundImageUrl} alt="Foreground" className="w-full h-full object-contain" />
                  </div>
                )}
              </div>
            </div>
