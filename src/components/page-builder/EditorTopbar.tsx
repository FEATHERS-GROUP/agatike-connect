  ...
  return (
    <div className="sticky top-0 z-20 bg-card border-b border-border/60 px-4 py-2 flex items-center justify-between gap-4 h-14 shrink-0">
      ...
      <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <DropdownMenuContent align="start" className="w-64">
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {(() => {
                const currentMainPageId = editorState.parent_id || editorState.id;
                const currentMainPage = allPages.find((p: any) => p.id === currentMainPageId);
                const currentSubPages = allPages.filter((p: any) => p.parent_id === currentMainPageId);
                    ...
                    <DropdownMenuItem asChild>
                      ...
                      >
                        ...
                        <div className="flex flex-col min-w-0 flex-1">
                          <span className="truncate font-medium">{currentMainPage.title || "Untitled"}</span>
                      ...
                      <DropdownMenuItem key={subPage.id} asChild>
                        ...
                        >
                          ...
                          <div className="flex flex-col min-w-0 flex-1 relative z-10">
                            <span className="truncate font-medium text-xs">{subPage.title || "Untitled"}</span>
        ...
        <Button variant="outline" size="sm" className="h-8 gap-1.5 ml-2 rounded-full hidden sm:flex" asChild>
      ...
      <div className="flex items-center gap-1.5 flex-1 justify-end">
        <Button variant="ghost" size="icon" className="w-8 h-8 rounded-full text-muted-foreground hover:text-foreground hover:bg-secondary/80" onClick={() => handleCopyLink(editorState.slug)} title="Share link">
