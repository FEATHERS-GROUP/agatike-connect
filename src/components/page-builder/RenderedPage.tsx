          ...
          >
            ...
            >
              ...
              >
                ...
                {title && <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white leading-tight">{title}</h1>}
                {description && <p className="text-lg md:text-2xl text-white/90 font-medium max-w-2xl">{description}</p>}
                
                {/* Hero Button Action */}
                {heroButtonText && (
                  <div className="mt-8">
                    {heroButtonActionType === "url" && (
                      <Button asChild size="lg" className="rounded-full px-8 py-6 text-lg font-bold shadow-xl hover:shadow-2xl hover:scale-105 transition-all" style={{ background: theme_color, color: "#fff" }}>
                    ...
                    {heroButtonActionType === "form" && (
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button size="lg" className="rounded-full px-8 py-6 text-lg font-bold shadow-xl hover:shadow-2xl hover:scale-105 transition-all cursor-pointer" style={{ background: theme_color, color: "#fff" }}>
                            {heroButtonText}
                          </Button>
                        </DialogTrigger>
                    ...
                    {heroButtonActionType === "payment" && (
                      <Button
                        ...
                        onClick={() => {
                          const pBlock = actualComponents.find((c: any) => c.type === "payment_button" && c.id === heroButtonLink);
                          if (pBlock) {
                    ...
                    {heroButtonActionType === "section" && (
                      <Button size="lg" className="rounded-full px-8 py-6 text-lg font-bold shadow-xl hover:shadow-2xl hover:scale-105 transition-all cursor-pointer" style={{ background: theme_color, color: "#fff" }} onClick={() => scrollToSection(`section-${heroButtonLink}`)}>
                        {heroButtonText}
                      </Button>
                    )}
                  </div>
              ...
              {heroForegroundImageUrl && (
                <div className={`hidden md:block flex-1 flex ${heroForegroundPosition === "left" ? "justify-start" : "justify-end"}`}>
                  <img
