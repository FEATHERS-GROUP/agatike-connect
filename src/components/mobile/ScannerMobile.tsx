...
function ScannerHeader({
  ...
  return (
    <header className="flex items-center justify-between px-4 pt-safe-top pb-4 bg-black/60 backdrop-blur-2xl z-30 border-b border-white/5 relative">
      <button
        onClick={onClose}
        className="p-2 -ml-2 text-white/80 hover:text-white active:scale-95 transition-all"
      >
        <ArrowLeft className="h-7 w-7" />
      </button>
      <div className="text-center flex-1 mx-2">
        <h1 className="font-black text-base tracking-tight text-white flex flex-col items-center">
          <span className="text-[10px] text-white/50 uppercase tracking-[0.2em] font-bold mb-0.5">
            Scanning
          </span>
        ...
        <div className="mt-1 flex items-center justify-center gap-1.5">
          <span className="relative flex h-2 w-2">
            <span
              className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${online ? "bg-emerald-400" : "bg-red-400"}`}
            ></span>
            <span
              className={`relative inline-flex rounded-full h-2 w-2 ${online ? "bg-emerald-500" : "bg-red-500"}`}
            ></span>
          </span>
      ...
      <div className="flex gap-2 items-center">
        <div
          className={`p-1.5 rounded-full transition-colors ${online ? "bg-emerald-500/20 text-emerald-400" : "bg-red-500/20 text-red-400"}`}
        >
...
function ScannerViewport({ result, onScan }: { result: Result; onScan: (text: string) => void }) {
  ...
  return (
    <div className="flex-1 relative flex flex-col items-center justify-center overflow-hidden">
      ...
      {/* The Viewfinder Box */}
      <div
        className={`relative w-full max-w-[280px] sm:max-w-xs aspect-square z-10 transition-transform duration-500 ${result !== "idle" ? "scale-105" : "scale-100"}`}
      >
        ...
        {/* Corner Brackets */}
        <div
          className={`absolute -top-1 -left-1 w-12 h-12 border-t-[5px] border-l-[5px] rounded-tl-[2.5rem] transition-colors duration-300 ${getBorderColor()}`}
        />
        <div
          className={`absolute -top-1 -right-1 w-12 h-12 border-t-[5px] border-r-[5px] rounded-tr-[2.5rem] transition-colors duration-300 ${getBorderColor()}`}
        />
        <div
          className={`absolute -bottom-1 -left-1 w-12 h-12 border-b-[5px] border-l-[5px] rounded-bl-[2.5rem] transition-colors duration-300 ${getBorderColor()}`}
        />
        <div
          className={`absolute -bottom-1 -right-1 w-12 h-12 border-b-[5px] border-r-[5px] rounded-br-[2.5rem] transition-colors duration-300 ${getBorderColor()}`}
        />
        ...
        {(result === "success" || result === "fail" || result === "vip") && (
          <div
            className={`absolute inset-0 flex flex-col items-center justify-center rounded-[2.5rem] backdrop-blur-xl z-20 ${result === "success" || result === "vip" ? "bg-emerald-500/20" : "bg-red-500/20"}`}
          >
...
function ScannerManualEntryModal({
  onScan,
  isPending,
}: {
    ...
    <>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent
          ...
          className="z-[150] bg-[#111] text-white border-t border-white/10 rounded-t-[2.5rem] p-6 shadow-[0_-20px_50px_rgba(0,0,0,0.8)]"
        >
          <SheetHeader className="mb-6">
            <SheetTitle className="text-white text-left text-2xl font-black">
              Manual Entry
            </SheetTitle>
          </SheetHeader>
...
export function ScannerMobile({
  ...
  return (
    <div className="h-[100dvh] w-full bg-black text-white flex flex-col overflow-hidden font-sans">
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 bg-[#111] rounded-t-[2.5rem] border-t border-white/10 p-6 pb-safe transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] ${result !== "idle" ? "translate-y-0 shadow-[0_-30px_60px_rgba(0,0,0,0.9)]" : "translate-y-[120%]"}`}
      >
        {/* TICKET RESULT */}
        {(result === "success" || result === "fail" || result === "vip") && (
          <div className="animate-in slide-in-from-bottom-8 duration-300">
              ...
              <div className="flex items-center gap-4 mb-5">
                ...
                <div className="flex-1 overflow-hidden">
                  <h3 className="text-2xl font-black tracking-tight truncate">
                    {scannedTicket?.names || "Guest"}
                  </h3>
                  <p className="text-white/50 text-sm font-medium truncate">
                    #{scannedTicket?.qrcode_number || "TICKET"}
                  </p>
                </div>
              </div>
            )}
            ...
            <div
              className={`w-full p-5 rounded-2xl text-center font-black text-xl tracking-wide shadow-inner mb-4 ${result === "fail" ? "bg-red-500/10 text-red-400 border border-red-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"}`}
            >
            ...
            {result === "vip" && (
              <div className="mt-5 pt-5 border-t border-white/10 space-y-3">
                <p className="text-xs uppercase tracking-widest text-white/50 mb-1 font-bold">
                  VIP Privileges
                </p>
                <div className="bg-white/5 rounded-2xl p-4 space-y-3 border border-white/10">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/70 font-medium">Parking Access</span>
                    <span className="font-bold text-emerald-400">Yes</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/70 font-medium">License Plate</span>
                    <span className="font-mono bg-black/50 px-2.5 py-1 rounded-lg text-[#FFD700] font-bold tracking-widest">
                      RAA 123 A
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-white/70 font-medium">Backstage Pass</span>
                    <span className="font-bold text-emerald-400">Yes</span>
                  </div>
                </div>
              </div>
        ...
        {result === "staff" && (
          <div className="animate-in slide-in-from-bottom-8 duration-300">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xs font-black text-primary tracking-widest uppercase bg-primary/10 px-3 py-1 rounded-full border border-primary/20">
                Staff Verification
              </h2>
              <button
            ...
            <div className="bg-gradient-to-br from-slate-900 to-black border border-white/10 rounded-[2rem] p-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="relative z-10 flex gap-5 items-center">
                ...
                <div>
                  ...
                  <p className="text-primary font-black uppercase text-sm tracking-wider mt-1">
                    {scannedStaff?.role || "Staff"}
                  </p>
                  <p className="text-white/30 font-mono text-[11px] mt-1.5">
            ...
            <div className="bg-slate-900 border border-blue-500/30 rounded-[2rem] p-6 shadow-[0_10px_30px_rgba(59,130,246,0.15)]">
              ...
              <div className="flex items-center justify-center gap-8 mb-8">
                <button
                  ...
                  className="h-16 w-16 rounded-full bg-white/5 flex items-center justify-center active:bg-white/10 active:scale-95 transition-all border border-white/10"
                >
              ...
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 active:scale-[0.98] disabled:opacity-50 text-white font-black py-4 rounded-2xl flex items-center justify-center transition-all shadow-[0_10px_20px_rgba(37,99,235,0.3)] text-lg"
              >
                {processingTx
                  ? "Processing..."
        ...
        {result === "idle" && (
          <>
            ...
            <button
              ...
              className="flex-none bg-emerald-500/20 text-emerald-400 px-6 py-4 rounded-full text-sm font-bold whitespace-nowrap border border-emerald-500/30 shadow-lg snap-start"
            >
              Simulate Ticket
            </button>
            <button
              onClick={() => setResult("voucher")}
              className="flex-none bg-blue-500/20 text-blue-400 px-6 py-4 rounded-full text-sm font-bold whitespace-nowrap border border-blue-500/30 shadow-lg snap-start"
            >
              Simulate Wallet
            </button>
            <button
              onClick={() => handleManualScan("STAFF-123")}
              className="flex-none bg-primary/20 text-primary px-6 py-4 rounded-full text-sm font-bold whitespace-nowrap border border-primary/30 shadow-lg snap-start"
            >
              Simulate Badge
            </button>
          </>
