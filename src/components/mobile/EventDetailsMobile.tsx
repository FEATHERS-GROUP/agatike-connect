...
import { Calendar, Clock, MapPin, Star, Users, Heart, Share2, Plus, Minus, Shield, ChevronLeft } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-32">
      ...
      <section className="relative h-[65vh] w-full overflow-hidden">
        <Link to="/explore" className="absolute top-safe-top left-4 z-30 h-10 w-10 bg-black/40 backdrop-blur-md rounded-full flex items-center justify-center border border-white/10 mt-4">
      ...
      <div className="px-4 pt-6 space-y-8">
        ...
        <div className="flex items-center justify-between bg-card/60 backdrop-blur rounded-3xl p-3 border border-border/40">
          <div className="flex items-center gap-3">
            <img src={event.cover} className="h-12 w-12 rounded-full object-cover border border-border" alt={event.organizer || event.host || event.cinema || 'Host'} />
            <div>
              <p className="font-semibold leading-tight">{event.organizer || event.host || event.cinema}</p>
              <p className="text-xs text-muted-foreground">@{event.organizerHandle || "host"}</p>
            </div>
          </div>
          <Button size="sm" className="rounded-full h-8 px-4 font-bold">Follow</Button>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm text-primary">{(event.attendees || event.spots || 0).toLocaleString()} going</span>
          </div>
          <div className="flex -space-x-3">
            {Array.from({ length: 6 }).map((_, i) => (
              <img key={i} src={`https://i.pravatar.cc/100?img=${i+20}`} className="h-10 w-10 rounded-full border-2 border-background" />
            ))}
                ...
                {tier === t.id && (
                  <div className="flex items-center justify-between mt-4 pt-4 border-t border-border/40">
                    ...
                    <div className="flex items-center gap-3 bg-background rounded-full px-2 py-1">
                      <button className="h-8 w-8 flex items-center justify-center rounded-full bg-secondary text-foreground" onClick={(e) => { e.stopPropagation(); setQty(Math.max(1, qty - 1)); }}><Minus className="h-3 w-3" /></button>
                      <span className="w-4 text-center font-bold text-sm">{qty}</span>
                      <button className="h-8 w-8 flex items-center justify-center rounded-full bg-secondary text-foreground" onClick={(e) => { e.stopPropagation(); setQty(qty + 1); }}><Plus className="h-3 w-3" /></button>
                    </div>
                  </div>
                )}
              </div>
