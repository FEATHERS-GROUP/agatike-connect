    ...
    <div className="min-h-screen bg-background pb-28">
      ...
      <div className="relative h-72 w-full">
        ...
        <div className="absolute top-0 left-0 right-0 pt-safe-top p-4 flex items-center justify-between z-10">
          <Link to="/venues" className="w-10 h-10 rounded-full bg-background/20 backdrop-blur-md flex items-center justify-center text-white active:scale-95 transition-transform border border-white/10">
      ...
      <div className="px-4 pt-4 space-y-6">
        <div>
          ...
          <div className="flex flex-col gap-2 text-sm text-muted-foreground font-medium">
            ...
            <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-primary" /> {venue.openTime} - {venue.closeTime}</span>
          </div>
        </div>
        ...
        <div className="border-t border-border/40 pt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {['Free WiFi', 'Parking', 'Wheelchair', 'Cafeteria'].map((amenity, i) => (
              <span key={i} className="px-3 py-1.5 bg-secondary/50 rounded-lg text-xs font-medium text-muted-foreground border border-border/40">
                {amenity}
              </span>
            ))}
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 pb-safe-bottom bg-background/80 backdrop-blur-xl border-t border-border/40 z-30">
        <div className="flex items-center justify-between gap-4 max-w-md mx-auto">
          ...
          <Link to="/venues/checkout/$venueId" params={{ venueId: venue.id }} className="flex-1">
            <button className="w-full h-12 rounded-xl text-sm font-bold text-primary-foreground shadow-[var(--shadow-glow)] active:scale-[0.98] transition-transform" style={{ background: "var(--gradient-primary)" }}>
              Book Ticket
            </button>
          </Link>
        </div>
