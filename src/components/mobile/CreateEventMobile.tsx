    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-muted-foreground mb-8">
          "{data.title}" is now live and ready for tickets.
        </p>
        <Link to="/events/$eventId" params={{ eventId: "1" }} className="w-full">
          <Button className="w-full h-12 rounded-full font-bold text-lg mb-3">
            View Event Page
          </Button>
        </Link>
        <Link to="/dashboard" className="w-full">
          <Button variant="outline" className="w-full h-12 rounded-full font-bold text-lg">
            Back to Dashboard
          </Button>
        </Link>
      </div>
          ...
          {step === 2 && (
            <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
              {tickets.map((t) => (
                ...
                >
                  {t.type === "vip" && (
                    <div className="absolute top-0 right-0 bg-primary text-primary-foreground px-3 py-1 rounded-bl-xl font-bold text-[10px] uppercase">
                      VIP
                    </div>
                  )}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setTickets(
                            tickets.map((x) =>
                              x.id === t.id ? { ...x, quantity: Number(e.target.value) } : x,
                            ),
                          )
                        }
                        placeholder="Qty"
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="rounded-3xl border border-border/40 overflow-hidden bg-card">
                <div className="aspect-[4/3] bg-secondary relative">
                  {data.coverPreview && (
                    <img src={data.coverPreview} className="w-full h-full object-cover" />
                  )}
