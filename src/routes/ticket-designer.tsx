...
export const Route = createFileRoute("/ticket-designer")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Design premium, branded tickets for your events, movies and experiences.",
      },
    ],
  }),
...
function TicketDesignerPage() {
  const [template, setTemplate] = useState<Template>("concert");
  const [palette, setPalette] = useState(palettes[0]);
  const [font, setFont] = useState(fonts[0]);
  const [tier, setTier] = useState("VIP");
  const [title, setTitle] = useState("Afrobeats Night Live");
  const [subtitle, setSubtitle] = useState("Eko Convention Centre · Lagos");
  const [date, setDate] = useState("Sat, 14 Sep 2026");
  const [time, setTime] = useState("21:00");
  const [seat, setSeat] = useState("Sec A · Row 12 · Seat 36");
    ...
    <div className="min-h-screen bg-secondary/30">
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-border/60 bg-background/80 px-6 py-3 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-muted-foreground">Dashboard / Tickets</p>
            <h1 className="text-lg font-semibold">Ticket Designer</h1>
          </div>
        </div>
...
function TicketPreview(props: {
  ...
  return (
    <div
      className="relative flex w-[720px] max-w-full overflow-hidden rounded-[28px] text-white shadow-2xl"
      style={{
        fontFamily: font.css,
