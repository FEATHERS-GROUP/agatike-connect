  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-24 md:pb-0">
            ...
            {cinemaMovies.map((movie) => (
              ...
              >
                ...
                <div className="flex-1 flex flex-col">
                  <h3 className="font-bold text-base md:text-lg leading-tight mb-1 group-hover:text-primary transition-colors">{movie.title}</h3>
                  <p className="text-xs text-muted-foreground mb-3">{movie.genre} • {movie.duration}</p>
                  
                  <div className="mt-auto">
                    <div className="flex flex-wrap gap-1 mb-3">
                      {movie.showtimes.slice(0, 3).map((t) => (
                        <span key={t} className="px-2 py-1 bg-secondary text-[10px] font-semibold rounded-md border border-border/60">
                          {t}
                        </span>
                      ))}
                    ...
                    <div className="flex items-center justify-between mt-2 pt-3 border-t border-border/40">
                      ...
                      <Button size="sm" variant="secondary" className="h-7 text-xs rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground group-hover:bg-primary group-hover:text-primary-foreground">
                        Book
                      </Button>
                    </div>
                  </div>
      ...
      <Drawer open={!!selectedMovie} onOpenChange={(open) => !open && setSelectedMovie(null)}>
        <DrawerContent className="max-h-[90vh] p-0 border-border/60 mx-auto md:max-w-2xl" aria-describedby="cinema-movie-desc">
          {selectedMovie && (
            <div className="overflow-y-auto hide-scrollbar pb-safe">
              <DrawerHeader className="sr-only">
                <DrawerTitle>{selectedMovie.title}</DrawerTitle>
                <DrawerDescription id="cinema-movie-desc">{selectedMovie.synopsis}</DrawerDescription>
              </DrawerHeader>
              ...
              <div className="px-5 -mt-8 relative z-10">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground font-medium mb-4">
                  ...
                  <span className="w-1 h-1 rounded-full bg-border" />
                  <span className="border border-border/60 px-1.5 py-0.5 rounded">{selectedMovie.rating}</span>
                </div>
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button variant="outline" size="icon" className="h-14 w-14 rounded-2xl border-border/60 bg-secondary/50">
