      ...
      <div className="relative bg-primary text-primary-foreground pt-16 pb-28 px-6 md:px-12 overflow-hidden border-b border-primary/20">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <Link
            ...
            className="inline-flex items-center gap-2 text-primary-foreground/70 hover:text-primary-foreground transition-colors mb-8 font-medium bg-black/10 hover:bg-black/20 px-4 py-2 rounded-full backdrop-blur-sm text-sm"
          >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-lg md:text-xl text-primary-foreground/80 font-medium flex items-center gap-3">
                <span className="bg-white/20 px-3 py-1 rounded-md backdrop-blur-md">
                  {space?.name}
                </span>
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-4 md:px-12 -mt-16 mb-20 relative z-10">
        ...
        >
          ...
          <TabsList className="flex flex-row md:flex-col justify-start h-auto bg-background/60 dark:bg-background/40 backdrop-blur-xl p-2 rounded-2xl border border-border/50 shadow-lg w-full md:w-64 shrink-0 overflow-x-auto">
            <TabsTrigger
              value="overview"
              className="w-full justify-start py-3.5 px-4 rounded-xl data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md transition-all text-base font-medium mb-1"
            >
            ...
            <TabsTrigger
              value="visitor"
              className="w-full justify-start py-3.5 px-4 rounded-xl data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md transition-all text-base font-medium"
            >
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Manage your current billing plan and status.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="relative overflow-hidden bg-secondary border border-border p-7 rounded-3xl shadow-xl transform transition-transform hover:scale-[1.02] duration-300">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-3xl md:text-4xl font-extrabold tracking-tight mb-8 text-transparent bg-clip-text bg-gradient-to-r from-foreground to-muted-foreground">
                  ...
                  <div className="bg-secondary/40 p-7 rounded-3xl border border-border/50 flex flex-col justify-center">
                    <p className="text-sm text-muted-foreground font-bold uppercase tracking-widest mb-4">
                      Status & Renewal
                    </p>
                    <div className="flex items-center gap-3 mb-6">
                      <span className={`relative flex h-4 w-4`}>
                        <span
                          className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${subscription.status === "active" ? "bg-green-500" : "bg-red-500"}`}
                        ></span>
                        <span
                          className={`relative inline-flex rounded-full h-4 w-4 ${subscription.status === "active" ? "bg-green-500" : "bg-red-500"}`}
                        ></span>
                      </span>
                    ...
                    {subscription.next_billing_date && (
                      <div className="bg-background/50 dark:bg-background/30 rounded-2xl p-4 border border-border/30">
                        ...
                        <p className="font-bold text-lg">
                          {format(new Date(subscription.next_billing_date), "MMMM do, yyyy")}
                        </p>
                      </div>
                    )}
                  </div>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          if (price === 0 || subscription?.booking_type === "visitor") {
                            if (
                              confirm("Are you sure you want to cancel this free subscription?")
                            ) {
                            ...
                            if (
                              confirm(
                                `Are you sure you want to cancel this subscription? WARNING: You will only receive a 50% refund (${(price * 0.5).toLocaleString()} ${space?.workspace?.currency}).`,
                              )
                            ) {
                              cancelMutation.mutate();
                            }
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-muted-foreground text-base">
                    Upgrade your plan to unlock different benefits. Your current payment of{" "}
                    <span className="font-bold text-foreground bg-primary/10 px-2 py-0.5 rounded-md">
                      {currentPrice.toLocaleString()} {space?.workspace?.currency}
                    </span>{" "}
                    will be prorated against the new plan!
                  </p>
                </div>
                ...
                {otherPlans.length === 0 ? (
                  <div className="text-center py-16 bg-secondary/30 rounded-3xl border border-border/40 border-dashed">
                    ...
                    <p className="font-medium text-muted-foreground max-w-sm mx-auto">
                      There are no other plans available at this space.
                    </p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    {otherPlans.map((plan: any, i: number) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="group relative overflow-hidden bg-background hover:bg-gradient-to-b hover:from-background hover:to-primary/5 p-7 rounded-3xl border-2 border-border/40 hover:border-primary/50 transition-all duration-300 cursor-pointer shadow-sm hover:shadow-2xl hover:-translate-y-1"
                          ...
                          <div className="mt-auto pt-6 border-t border-border/50 relative z-10">
                            <div className="flex justify-between items-center mb-6 bg-secondary/50 p-4 rounded-2xl">
                              ...
                              <span className="text-xl font-black text-primary">
                                {amountDue.toLocaleString()} {space?.workspace?.currency}
                              </span>
                            </div>
                            <Button
                              size="lg"
                              className="w-full rounded-2xl h-14 font-bold text-base group-hover:bg-primary group-hover:text-primary-foreground shadow-md group-hover:shadow-xl transition-all"
                            >
                              Proceed to Upgrade
                            </Button>
                          </div>
            ...
            >
              <div className="space-y-8 max-w-2xl">
                <div>
                  ...
                  <p className="text-muted-foreground text-base">
                    Generate a 1-day pass for a guest. They will receive a QR code ticket via email
                    instantly.
                  </p>
                </div>
