    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Define an area (e.g. VIP Lounge, Main Gate) to assign staff to.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label>Section Name</Label>
            <Input placeholder="e.g. Backstage" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="space-y-2">
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Assign a role and section. A secure Digital Badge will be generated automatically.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.section_id} onValueChange={(val) => setFormData({...formData, section_id: val})}>
              <SelectTrigger><SelectValue placeholder="No specific section" /></SelectTrigger>
              <SelectContent>
...
function StaffView() {
  ...
  const { data: sections = [] } = useQuery({
    ...
    queryFn: async () => {
      try { return await getEventSections({ data: { event_id: eventId } } as any); }
      catch { return [{ id: "1", name: "VIP Lounge" }, { id: "2", name: "Main Bar" }]; }
    }
  });
  ...
  const { data: staff = [] } = useQuery({
    ...
    queryFn: async () => {
      try { 
        ...
        return [
          { id: "1", user_id: "User A", role: "Manager", status: "active", badge_qr_string: "STAFF-XYZ123", section: null },
          { id: "2", user_id: "User B", role: "Bartender", status: "active", badge_qr_string: "STAFF-ABC987", section: { name: "VIP Lounge" } },
        ]; 
      }
    }
  });
    ...
    <div className="space-y-6 max-w-6xl mx-auto w-full">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage roles, assign sections, and generate digital badges.</p>
        </div>
      ...
      <Tabs defaultValue="staff" className="w-full">
          ...
          <div className="rounded-2xl border border-border/60 bg-card overflow-hidden shadow-[var(--shadow-card)]">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-border/60">
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="font-semibold text-foreground">User {s.user_id.substring(0,6)}</p>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        s.status === 'active' ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'
                      }`}>
                        {s.status}
                      </span>
                    </td>
                ...
                {staff.length === 0 && (
                  <tr><td colSpan={5} className="p-8 text-center text-muted-foreground">No staff members assigned yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <TabsContent value="sections">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {sections.map((s: any) => (
              <div key={s.id} className="rounded-xl border border-border/60 bg-card p-5 shadow-sm hover:shadow-md transition-all">
      ...
      <Sheet open={!!selectedStaff} onOpenChange={(open) => !open && setSelectedStaff(null)}>
        <SheetContent className="sm:max-w-md w-full overflow-y-auto">
          <SheetHeader className="mb-6">
            ...
            <SheetDescription>This is how the badge looks on the staff member's phone.</SheetDescription>
          </SheetHeader>
          ...
          {selectedStaff && (
            <div className="flex flex-col items-center mt-4">
              <div className="relative w-full aspect-[1/1.5] rounded-3xl overflow-hidden shadow-2xl bg-gradient-to-b from-slate-900 to-black border border-slate-800">
                ...
                <div className="relative z-10 flex flex-col h-full p-8">
                  <div className="flex-grow flex flex-col items-center justify-center text-center mt-8">
                    ...
                    <h2 className="text-white text-2xl font-bold tracking-tight">User {selectedStaff.user_id.substring(0,6)}</h2>
                    <p className="text-primary font-semibold tracking-widest uppercase mt-2 text-sm">{selectedStaff.role}</p>
                    ...
                    <p className="text-slate-500 font-mono text-xs mt-3">{selectedStaff.badge_qr_string}</p>
                  </div>
              ...
              <div className="mt-8 flex gap-3 w-full">
                <Button className="flex-1" variant="outline" onClick={() => toast.success("Copied secure link!")}>
                  Copy Link
                </Button>
