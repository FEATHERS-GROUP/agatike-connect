...
import { getProduct, createProductOrders } from "@/api/products";
...
function CheckoutPage() {
          ...
          objects: [
            {
              ...
              amount_paid: total,
              decrptions: bookingRef, // Maps to booking_ref in webhook
              qr_code_string: crypto.randomUUID(),
              ticket_id: null,
  ...
  // 4. Polling for PawaPay Completion
  useEffect(() => {
      ...
      >
        <div className="max-w-md w-full bg-secondary/30 border border-border/40 p-8 rounded-3xl text-center space-y-6 animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-muted-foreground">
            Thank you, {buyerName || "Guest"}! Your order for <span className="font-semibold text-foreground">{qty}x {product?.name}</span> has been confirmed.
          </p>
  ...
  if (isPollingPawaPay) {
    return (
      <div 
        ...
        style={{ fontFamily: `${fontFamily}, sans-serif` }}
      >
        <Smartphone className="h-20 w-20 text-primary mb-8 animate-pulse" style={{ color: themeColor }} />
        <h1 className="text-3xl font-bold mb-4">Check Your Phone</h1>
        <p className="text-lg text-muted-foreground mb-10 max-w-sm mx-auto">
          We've sent a payment request to your mobile number. Please enter your PIN to confirm the payment of <strong>RWF {total.toLocaleString()}</strong>.
        </p>
          ...
          <div>
            ...
            <div className="space-y-8">
              ...
              <section className="space-y-5 pt-6 border-t border-border/40">
                ...
                <RadioGroup value={selectedPaymentGroup} onValueChange={setSelectedPaymentGroup} className="space-y-3">
                  <div className={`flex items-center space-x-3 border p-5 rounded-2xl cursor-pointer transition-colors ${selectedPaymentGroup === "momo" ? "border-primary bg-primary/5" : "border-border/60 hover:bg-secondary/20"}`} onClick={() => setSelectedPaymentGroup("momo")}>
                    <RadioGroupItem value="momo" id="momo" style={selectedPaymentGroup === "momo" ? { color: themeColor } : {}} />
                    <Label htmlFor="momo" className="flex flex-1 items-center justify-between cursor-pointer">
                      ...
                      <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">MTN / Airtel</span>
                    </Label>
                  </div>
                  <div className={`flex items-center space-x-3 border p-5 rounded-2xl cursor-pointer transition-colors ${selectedPaymentGroup === "card" ? "border-primary bg-primary/5" : "border-border/60 hover:bg-secondary/20"}`} onClick={() => setSelectedPaymentGroup("card")}>
                    <RadioGroupItem value="card" id="card" style={selectedPaymentGroup === "card" ? { color: themeColor } : {}} />
                    <Label htmlFor="card" className="flex flex-1 items-center justify-between cursor-pointer">
                      <span className="flex items-center gap-3 font-medium text-base">
                        <CreditCard className="w-5 h-5 text-muted-foreground" />
                        Credit / Debit Card
                      </span>
                      <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Visa / MC</span>
                    </Label>
                  </div>
                </RadioGroup>
              </section>
        ...
        <div className="max-w-sm mx-auto space-y-8 sticky top-12">
          ...
          <div className="flex gap-5">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <h3 className="font-bold text-lg line-clamp-2 leading-tight">{product?.name || "Product"}</h3>
              <div className="space-y-1 mt-2">
                {size && <p className="text-sm text-muted-foreground">Size: <span className="font-medium text-foreground">{size}</span></p>}
                {color && (
                  <p className="text-sm text-muted-foreground flex items-center gap-2">
                    Color: 
                    <span className="w-3 h-3 rounded-full border border-border shadow-sm inline-block" style={{ backgroundColor: color }} />
                  </p>
                )}
              </div>
            </div>
