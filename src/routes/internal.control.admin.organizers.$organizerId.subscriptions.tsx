  ...
  return (
    <div className="flex items-center justify-between px-5 py-3 border-t border-[#333333] bg-[#111111]">
      ...
      <div className="flex items-center gap-2">
        <button onClick={onPrev} disabled={page === 1} className="p-1.5 border border-[#333333] text-[#797775] hover:text-white hover:border-[#555555] disabled:opacity-30 transition-colors">
          <ChevronLeft className="h-3.5 w-3.5" />
        </button>
        <span className="text-xs text-[#797775]">{page} / {pages}</span>
        <button onClick={onNext} disabled={page === pages} className="p-1.5 border border-[#333333] text-[#797775] hover:text-white hover:border-[#555555] disabled:opacity-30 transition-colors">
...
function TxDrawer({ tx, onClose }: { tx: any; onClose: () => void }) {
  ...
  const Field = ({ label, value, mono, copyKey }: { label: string; value?: string | null; mono?: boolean; copyKey?: string }) => {
    if (!value) return (
      <div>
    ...
    return (
      <div>
        ...
        <div className="flex items-start gap-1.5">
          <p className={`text-[#cccccc] text-sm break-all ${mono ? "font-mono text-xs text-[#569cd6]" : ""}`}>{value}</p>
          {copyKey && (
            <button onClick={() => copyToClipboard(value, copyKey)} className="shrink-0 mt-0.5 text-[#797775] hover:text-white transition-colors">
              {copied === copyKey ? <Check className="h-3 w-3 text-[#84c87e]" /> : <Copy className="h-3 w-3" />}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="relative ml-auto z-10 w-full max-w-lg h-full bg-[#1b1b1c] border-l border-[#333333] flex flex-col font-sans overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#333333] shrink-0">
          <div className="flex items-center gap-3">
            <div className={`h-8 w-8 flex items-center justify-center ${isCredit ? "bg-[#84c87e]/10 text-[#84c87e]" : "bg-[#f43f5e]/10 text-[#f43f5e]"}`}>
              {isCredit ? <ArrowDownLeft className="h-4 w-4" /> : <ArrowUpRight className="h-4 w-4" />}
            </div>
            <div>
              <p className="text-white font-medium text-sm capitalize">{(tx.type || "").replace(/_/g, " ")}</p>
              <p className="text-[#797775] text-xs">{tx.workspaceName}</p>
            </div>
          </div>
          <button onClick={onClose} className="text-[#797775] hover:text-white transition-colors p-1">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-5 border-b border-[#333333] bg-[#111111] shrink-0">
          ...
          <p className={`text-3xl font-bold mb-1 ${isCredit ? "text-[#84c87e]" : "text-white"}`}>
            {isCredit ? "+" : "−"}{fmtAmt(tx.amount, tx.currency)}
          </p>
          {tx.net_amount && tx.net_amount !== tx.amount && (
            <p className="text-[#797775] text-sm">Net payout: {fmtAmt(tx.net_amount, tx.currency)}</p>
          )}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
          ...
          <div className="h-px bg-[#333333]" />
          <div className="grid grid-cols-2 gap-4">
            <Field label="Created At" value={tx.created_at ? new Date(tx.created_at).toLocaleString("en-US") : null} />
            <Field label="Updated At" value={tx.updated_at ? new Date(tx.updated_at).toLocaleString("en-US") : null} />
          </div>
          ...
          {tx.raw_callback_data && (
            <>
              <div className="h-px bg-[#333333]" />
              <div>
                <p className="text-[#797775] text-xs uppercase tracking-wider mb-2">Raw Callback Data</p>
...
function OrganizerSubscriptions() {
  ...
  const pendingCount = transactions.filter((t: any) => (t.status || "").toLowerCase() === "pending").length;
  const approvedSims = feeSimulations.filter((s: any) => s.decision === "approved").length;
  const avgMargin = feeSimulations.length > 0
    ? feeSimulations.reduce((acc: number, s: any) => acc + (parseFloat(s.expected_margin) || 0), 0) / feeSimulations.length
    : 0;

  const tabs: { id: Tab; label: string; count: number; icon: React.ReactNode }[] = [
    { id: "subscriptions", label: "Platform Plans", count: subscriptions.length, icon: <CreditCard className="h-3.5 w-3.5" /> },
    { id: "transactions", label: "Wallet Transactions", count: transactions.length, icon: <Activity className="h-3.5 w-3.5" /> },
    { id: "simulations", label: "Fee Simulations", count: feeSimulations.length, icon: <BarChart3 className="h-3.5 w-3.5" /> },
  ];
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-[#333333] border border-[#333333]">
          {[
            ...
            },
            {
              ...
              valueColor: activeSubscription?.next_billing_date && new Date(activeSubscription.next_billing_date) < new Date()
                ? "text-[#f43f5e]"
                : "text-white",
            },
            {
            ...
            <div key={i} className="bg-[#1a1a1a] px-5 py-5">
              ...
              <p className={`font-bold text-lg leading-tight mb-0.5 ${kpi.valueColor}`}>{kpi.value}</p>
        ...
        <div className="flex border-b border-[#333333] bg-[#111111]">
          {tabs.map((t) => (
            ...
            >
              {t.icon}
              {t.label}
              <span className={`text-[11px] px-1.5 py-0.5 rounded-sm ${activeTab === t.id ? "bg-[#f97316]/20 text-[#f97316]" : "bg-[#333333] text-[#797775]"}`}>
                {t.count}
              </span>
            </button>
          ))}
                ...
                <table className="w-full text-left text-[13px] whitespace-nowrap">
                  <thead>
                    <tr className="bg-[#2d2d30] text-[#797775] text-xs uppercase tracking-wider">
                      {["Plan", "Status", "Amount", "Start Date", "Next Billing", "Svc Fee", "Max Withdrawals/wk"].map((h) => (
                        <th key={h} className="py-2.5 px-5 font-medium border-b border-[#333333]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#333333]">
                    {subscriptions.length === 0 ? (
                      <tr><td colSpan={7} className="py-10 text-center text-[#797775] italic">No subscriptions.</td></tr>
                    ) : subscriptions.map((s: any, i: number) => (
                      <tr key={s.id} className={`hover:bg-[#252526] transition-colors ${i % 2 === 0 ? "bg-[#1a1a1a]" : "bg-[#111111]"}`}>
                        <td className="py-3 px-5">
                          <div className="flex items-center gap-2">
                            <Sparkles className="h-3.5 w-3.5 text-[#dcdcaa] shrink-0" />
                            <span className="font-medium text-white">{s.pricing_plan?.name || "Unknown Plan"}</span>
                          </div>
                        </td>
                        <td className="py-3 px-5"><StatusPill status={s.status} /></td>
                        <td className="py-3 px-5 text-[#dcdcaa] font-medium">
                          {s.amount > 0 ? `${s.pricing_plan?.currency || "RWF"} ${s.amount}` : "Free"}
                        </td>
                        <td className="py-3 px-5 text-[#797775]">
                          {s.start_date ? new Date(s.start_date).toLocaleDateString("en-US") : "—"}
                        </td>
                        <td className="py-3 px-5">
                          {s.next_billing_date ? (
                            <span className={new Date(s.next_billing_date) < new Date() ? "text-[#f43f5e]" : "text-[#cccccc]"}>
                              {new Date(s.next_billing_date).toLocaleDateString("en-US")}
                            </span>
                          ) : "—"}
                        </td>
                        <td className="py-3 px-5 text-[#cccccc]">{s.pricing_plan?.customer_service_fee_percentage ?? "—"}%</td>
                        <td className="py-3 px-5 text-[#cccccc]">{s.pricing_plan?.max_withdrawals_per_week ?? "∞"}/wk</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="border border-[#333333]">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-[13px] whitespace-nowrap">
                  <thead>
                    <tr className="bg-[#2d2d30] text-[#797775] text-xs uppercase tracking-wider">
                      {["", "Type", "Workspace", "Amount", "Net Amount", "Currency", "Provider Ref", "Status", "Date", ""].map((h, i) => (
                        <th key={i} className="py-2.5 px-4 font-medium border-b border-[#333333]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#333333]">
                    {txSlice.length === 0 ? (
                      <tr><td colSpan={10} className="py-10 text-center text-[#797775] italic">
                        {txSearch ? "No transactions match." : "No transactions found."}
                      </td></tr>
                    ) : txSlice.map((t: any, i: number) => {
                      const isCredit = CREDIT_TYPES.has(t.type);
                      return (
                        <tr key={t.id} className={`hover:bg-[#252526] transition-colors ${i % 2 === 0 ? "bg-[#1a1a1a]" : "bg-[#111111]"}`}>
                          {/* Direction icon */}
                          <td className="py-3 px-4">
                            <div className={`h-7 w-7 flex items-center justify-center ${isCredit ? "bg-[#84c87e]/10 text-[#84c87e]" : "bg-[#f43f5e]/10 text-[#f43f5e]"}`}>
                              {isCredit ? <ArrowDownLeft className="h-3.5 w-3.5" /> : <ArrowUpRight className="h-3.5 w-3.5" />}
                            </div>
                          </td>
                          {/* Type */}
                          <td className="py-3 px-4">
                            <p className="text-[#cccccc] capitalize">{(t.type || "").replace(/_/g, " ")}</p>
                            {t.description && <p className="text-[#797775] text-xs truncate max-w-[140px]">{t.description}</p>}
                          </td>
                          {/* Workspace */}
                          <td className="py-3 px-4">
                            <div className="flex items-center gap-1.5 text-[#797775] text-xs">
                              <Building2 className="h-3 w-3 shrink-0" />{t.workspaceName}
                            </div>
                          </td>
                          {/* Amount */}
                          <td className={`py-3 px-4 font-semibold ${isCredit ? "text-[#84c87e]" : "text-[#cccccc]"}`}>
                            {isCredit ? "+" : "−"}{fmtAmt(t.amount, t.currency)}
                          </td>
                          {/* Net Amount */}
                          ...
                          {/* Currency */}
                          <td className="py-3 px-4 font-mono text-[#797775] text-xs">{t.currency || "—"}</td>
                          {/* Provider reference */}
                          <td className="py-3 px-4">
                            {t.provider_reference ? (
                              <span className="font-mono text-xs text-[#569cd6]">{String(t.provider_reference).substring(0, 14)}{String(t.provider_reference).length > 14 ? "…" : ""}</span>
              ...
              <Pagination
                page={txPage} total={filteredTx.length}
                onPrev={() => setTxPage((p) => Math.max(1, p - 1))}
                onNext={() => setTxPage((p) => Math.min(Math.ceil(filteredTx.length / PAGE_SIZE), p + 1))}
              />
            </div>
          </div>
        )}
            ...
            <div className="border border-[#333333]">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-[13px] whitespace-nowrap">
                  <thead>
                    <tr className="bg-[#2d2d30] text-[#797775] text-xs uppercase tracking-wider">
                      {["Transaction ID", "Decision", "Collection Cost", "Disbursement Cost", "Net Margin", "Date"].map((h) => (
                        <th key={h} className="py-2.5 px-5 font-medium border-b border-[#333333]">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#333333]">
                    {simSlice.length === 0 ? (
                      <tr><td colSpan={6} className="py-10 text-center text-[#797775] italic">
                      ...
                      return (
                        <tr key={s.transaction_id} className={`hover:bg-[#252526] transition-colors ${i % 2 === 0 ? "bg-[#1a1a1a]" : "bg-[#111111]"}`}>
                          <td className="py-3 px-5 font-mono text-[#569cd6] text-xs">
                            {String(s.transaction_id || "—").substring(0, 18)}…
                          </td>
                          <td className="py-3 px-5"><StatusPill status={s.decision} /></td>
                          <td className="py-3 px-5 text-[#f43f5e]">{fmtAmt(parseFloat(s.expected_collection_cost) || 0)}</td>
                          <td className="py-3 px-5 text-[#dcdcaa]">{fmtAmt(parseFloat(s.expected_disbursement_cost) || 0)}</td>
                          <td className={`py-3 px-5 font-semibold ${margin >= 0 ? "text-[#84c87e]" : "text-[#f43f5e]"}`}>
                            {margin >= 0 ? "+" : ""}{fmtAmt(margin)}
                          </td>
                          <td className="py-3 px-5 text-[#797775]">
              ...
              <Pagination
                page={simPage} total={filteredSims.length}
                onPrev={() => setSimPage((p) => Math.max(1, p - 1))}
                onNext={() => setSimPage((p) => Math.min(Math.ceil(filteredSims.length / PAGE_SIZE), p + 1))}
              />
            </div>
          </div>
        )}
