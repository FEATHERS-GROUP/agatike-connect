  ...
  return (
    <div className="min-h-screen bg-background text-foreground pb-24 md:pb-0 md:max-w-md md:mx-auto md:border-x md:border-border/40 lg:max-w-none lg:border-x-0 lg:mx-0 shadow-xl lg:shadow-none">
      ...
      <section className="relative overflow-hidden border-b border-border/60">
        <img src={activeMovie.cover} alt="" className="absolute inset-0 h-full w-full object-cover opacity-30 blur-sm" />
        ...
        <div className="relative mx-auto grid max-w-7xl gap-6 px-4 md:px-6 py-6 md:gap-8 md:py-16 md:grid-cols-[260px_1fr]">
          <img src={activeMovie.cover} alt={activeMovie.title} className="aspect-[2/3] w-[180px] md:w-full rounded-2xl object-cover shadow-[var(--shadow-card)] mx-auto md:mx-0" />
          <div className="text-center md:text-left">
            ...
            <p className="mt-2 text-sm text-muted-foreground">{activeMovie.genre} · {activeMovie.duration} · {activeMovie.rating}</p>
            <p className="mt-4 max-w-xl text-sm text-muted-foreground mx-auto md:mx-0">{activeMovie.synopsis}</p>
            <div className="mt-6 flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm">
              <span className="inline-flex items-center gap-1 text-muted-foreground"><MapPin className="h-4 w-4" /> {activeMovie.cinema}, {activeMovie.city}</span>
            ...
            <div className="mt-5 flex overflow-x-auto hide-scrollbar gap-2 pb-2 justify-center md:justify-start">
              {activeMovie.showtimes.map((t) => (
                <button key={t} className="rounded-2xl border border-border bg-background px-5 py-3 text-sm font-medium hover:border-primary hover:bg-accent transition shrink-0">
                  {t}
                </button>
              ))}
            </div>
            <div className="mt-6 flex flex-wrap justify-center md:justify-start gap-2">
              <Button asChild className="rounded-full shadow-[var(--shadow-glow)] w-full md:w-auto" style={{ background: "var(--gradient-primary)" }}>
                <Link to="/book/$eventId" params={{ eventId: activeMovie.id }}>
                  <Ticket className="mr-2 h-4 w-4" /> Reserve seat — {activeMovie.currency || '$'}{activeMovie.price}
                </Link>
              </Button>
              <Button variant="outline" className="rounded-full w-full md:w-auto mt-2 md:mt-0">Watch trailer</Button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 md:px-6 pb-20">
        <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4">
          ...
          <Button variant="outline" className="rounded-full w-full md:w-auto">List your cinema</Button>
        </div>
