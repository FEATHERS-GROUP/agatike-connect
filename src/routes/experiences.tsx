...
export const Route = createFileRoute("/experiences")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Join hikes, run clubs, surf camps and wellness retreats across Africa." },
      { property: "og:title", content: "Experiences — Agatike" },
      { property: "og:description", content: "Hike, run, surf and explore with curated local hosts." },
    ],
  }),
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 text-primary-foreground md:py-24">
          ...
          <h1 className="mt-4 max-w-3xl text-4xl font-semibold md:text-5xl">Hike, run, surf — book it like a ticket.</h1>
          <p className="mt-3 max-w-xl opacity-90">Local hosts running curated outdoor experiences and recurring clubs across the continent.</p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {list.map((x) => (
            <article key={x.id} className="group overflow-hidden rounded-3xl border border-border/60 bg-card shadow-[var(--shadow-card)] transition hover:-translate-y-1">
              <div className="relative aspect-[4/3] overflow-hidden">
                ...
                <span className="absolute top-3 left-3 rounded-full bg-background/90 px-3 py-1 text-xs backdrop-blur">{x.category}</span>
                <span className="absolute top-3 right-3 inline-flex items-center gap-1 rounded-full bg-background/90 px-2.5 py-1 text-xs font-semibold backdrop-blur"><Star className="h-3 w-3 fill-primary text-primary" /> {x.rating}</span>
              </div>
              <div className="p-5">
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {x.city}</span>
                  <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" /> {x.duration}</span>
                  <span className="inline-flex items-center gap-1"><Users className="h-3 w-3" /> {x.spots} spots</span>
                </div>
