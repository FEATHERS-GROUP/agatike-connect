...
export const Route = createFileRoute("/internal/control/admin/organizers/$organizerId/venues")({
  loader: async ({ params }) => {
    const data = await getAdminOrganizerVenues({ data: { organizerId: params.organizerId } } as any);
    return data;
  },
...
function OrganizerVenuesAndSpaces() {
  ...
  const tabs: { key: Tab; label: string; icon: any; count: number; color: string }[] = [
    { key: "venues", label: "Rentable Venues", icon: MapPin, count: venues?.length || 0, color: "text-[#569cd6]" },
    { key: "spaces", label: "Spaces", icon: LayoutGrid, count: spaces?.length || 0, color: "text-[#c586c0]" },
  ];
    ...
    <div className="space-y-4 font-sans text-sm pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-2 border-b border-[#333333] gap-4">
        <h2 className="text-lg font-medium text-white flex items-center gap-2">
          {activeTab === "venues" ? <MapPin className="h-5 w-5 text-[#569cd6]" /> : <LayoutGrid className="h-5 w-5 text-[#c586c0]" />}
          Venues & Spaces
        </h2>
        ...
        <div className="overflow-x-auto">
          ...
          {activeTab === "venues" && (
            <table className="w-full text-left text-[13px] whitespace-nowrap">
              ...
              <tbody className="divide-y divide-[#333333] text-[#cccccc]">
                {filteredVenues.length === 0 ? <EmptyRow cols={8} label="No rentable venues found." /> : filteredVenues.map((v: any) => (
                  <tr key={v.id} className="hover:bg-[#2d2d30] transition-colors">
                    ...
                    <td className="py-2 px-4">
                      {v.status === "Active" 
                        ...
                        : <span className="text-xs px-2 py-0.5 rounded-sm bg-[#797775]/10 text-[#797775]">{v.status || "—"}</span>
                      }
                    </td>
                    <td className="py-2 px-4">
                      <Sheet>
                          ...
                          <ScrollArea className="flex-1 p-6">
                            <div className="space-y-6">
                              ...
                              <div className="flex items-start gap-4">
                                {v.cover_url ? (
                                  <img src={v.cover_url} alt="Cover" className="h-16 w-16 rounded-md object-cover bg-[#2d2d30]" />
                                ) : (
                                ...
                                <div>
                                  <h3 className="text-lg font-medium text-white">{v.name || "Untitled Venue"}</h3>
                                  <div className="text-sm text-[#797775] capitalize">{v.type || "Uncategorized"}</div>
                              ...
                              <div className="grid grid-cols-1 gap-6">
                                ...
                                <DetailBlock icon={Map} label="Location">
                                  {v.address ? (
                                    <>
                                      <div>{v.address}</div>
                                      <div className="text-[#cccccc]">{[v.city, v.country].filter(Boolean).join(", ")}</div>
                                    </>
                                  ) : (
                                    <span className="italic text-[#797775]">No address provided</span>
                                  )}
                                </DetailBlock>

                                <DetailBlock icon={Users} label="Capacity">
                                  {v.capacity ? `${v.capacity} people` : <span className="italic text-[#797775]">Not specified</span>}
                                </DetailBlock>

                                <DetailBlock icon={Banknote} label="Rental Information">
                                  <div className="capitalize">
                                    Model: <span className="text-[#cccccc]">{v.rental_model || "—"}</span>
                                  </div>
                                  <div className="capitalize">
                                    Type: <span className="text-[#cccccc]">{v.rental_type || "—"}</span>
                                  </div>
                                  <div>
                                    Currency: <span className="text-[#dcdcaa]">{v.currency || "USD"}</span>
                                  </div>
                                </DetailBlock>
                                ...
                                <DetailBlock icon={Info} label="Description">
                                  {v.description ? (
                                    <div className="whitespace-pre-wrap text-[#cccccc] leading-relaxed">{v.description}</div>
                                  ) : (
                                    <span className="italic text-[#797775]">No description provided</span>
                                  )}
                                </DetailBlock>

                                {v.instructions && (
                                  <DetailBlock icon={FileText} label="Instructions">
                                    <div className="whitespace-pre-wrap text-[#cccccc] leading-relaxed">{v.instructions}</div>
                                  </DetailBlock>
                                )}
                                  ...
                                  {v.amenities && Array.isArray(v.amenities) && v.amenities.length > 0 ? (
                                    <div className="flex flex-wrap gap-1.5">
                                      {v.amenities.map((a: string, i: number) => (
                                        <span key={i} className="px-2 py-0.5 rounded-full bg-[#2d2d30] text-xs text-[#cccccc] capitalize">
                                          {a}
                                        </span>
                                      ))}
                                    </div>
          ...
          {activeTab === "spaces" && (
            <table className="w-full text-left text-[13px] whitespace-nowrap">
              ...
              <tbody className="divide-y divide-[#333333] text-[#cccccc]">
                {filteredSpaces.length === 0 ? <EmptyRow cols={7} label="No spaces found." /> : filteredSpaces.map((s: any) => (
                  <tr key={s.id} className="hover:bg-[#2d2d30] transition-colors">
                    <td className="py-2 px-4 font-mono text-[#797775] text-xs">{String(s.id).substring(0, 8)}...</td>
                    <td className="py-2 px-4 font-medium text-white">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="py-2 px-4"><WorkspaceCell name={s.workspaceName} /></td>
                    <td className="py-2 px-4 capitalize text-[#dcdcaa]">{s.type || "—"}</td>
                    <td className="py-2 px-4">
                      {s.status === "Active" 
                        ? <span className="text-xs px-2 py-0.5 rounded-sm bg-[#84c87e]/10 text-[#84c87e]">Active</span>
                        : <span className="text-xs px-2 py-0.5 rounded-sm bg-[#797775]/10 text-[#797775]">{s.status || "—"}</span>
                      }
                    </td>
                    <td className="py-2 px-4 text-[#797775]">{s.created_at ? new Date(s.created_at).toLocaleDateString('en-US') : "—"}</td>
                    <td className="py-2 px-4">
                      <Sheet>
                        <SheetTrigger asChild>
                          <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#333333] hover:bg-[#444444] text-white text-xs rounded transition-colors">
                        ...
                        <SheetContent className="bg-[#1e1e1e] border-[#333333] text-white w-full sm:max-w-md p-0 flex flex-col font-sans">
                          ...
                          <ScrollArea className="flex-1 p-6">
                            <div className="space-y-6">
                              ...
                              <div className="flex items-start gap-4">
                                {s.cover_url ? (
                                  <img src={s.cover_url} alt="Cover" className="h-16 w-16 rounded-md object-cover bg-[#2d2d30]" />
                                ) : (
                                  <div className="h-16 w-16 rounded-md bg-[#2d2d30] flex items-center justify-center">
                                ...
                                <div>
                                  <h3 className="text-lg font-medium text-white">{s.name || "Untitled Space"}</h3>
                                  <div className="text-sm text-[#797775] capitalize">{s.type || "Uncategorized"}</div>
                                  <div className="mt-1 flex items-center gap-2">
                                    <span className="text-xs px-2 py-0.5 rounded-sm bg-[#333333] text-[#cccccc] font-mono">
                              ...
                              <div className="grid grid-cols-1 gap-6">
                                ...
                                <DetailBlock icon={Map} label="Locations">
                                  {s.locations && Array.isArray(s.locations) && s.locations.length > 0 ? (
                                    <ul className="list-disc pl-4 space-y-1 text-[#cccccc]">
                                      {s.locations.map((loc: any, i: number) => (
                                        <li key={i}>
                                          <span className="font-medium text-white">{loc.name}</span>
                                          {loc.address && <div className="text-xs text-[#797775]">{loc.address}</div>}
                                        </li>
                                      ))}
                                    </ul>
                                  ) : (
                                    <span className="italic text-[#797775]">No locations provided</span>
                                  )}
                                </DetailBlock>
                                  ...
                                  {s.plans && Array.isArray(s.plans) && s.plans.length > 0 ? (
                                    <div className="space-y-2">
                                      {s.plans.map((plan: any, i: number) => (
                                        <div key={i} className="bg-[#2d2d30] p-2 rounded border border-[#333333] text-xs">
                                          <div className="font-medium text-white">{plan.name}</div>
                                          <div className="text-[#dcdcaa]">{s.currency || "USD"} {plan.price} / {plan.interval}</div>
                                        </div>
                                      ))}
                                    </div>
                                  ) : (
                                    <span className="italic text-[#797775]">No plans configured</span>
                                  )}
                                </DetailBlock>

                                <DetailBlock icon={Info} label="Description">
                                  {s.description ? (
                                    <div className="whitespace-pre-wrap text-[#cccccc] leading-relaxed">{s.description}</div>
                                  ) : (
                                    <span className="italic text-[#797775]">No description provided</span>
                                  )}
                                </DetailBlock>
                                ...
                                {(s.rsvp_form_id || s.show_rsvp_form_button) && (
                                  <DetailBlock icon={List} label="RSVP Form">
                                    <div className="flex items-center gap-2">
                                      <span className="text-[#cccccc]">{s.rsvp_form_button_text || "RSVP"}</span>
                                      {s.rsvp_form_id && (
