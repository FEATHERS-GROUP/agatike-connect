...
const INITIAL_SERVICES: Service[] = [
  { id: "db-supabase", name: "Supabase Database", category: "database", status: "checking", uptime: "99.99%", lastChecked: null, description: "Primary PostgreSQL cluster" },
  { id: "api-supabase-realtime", name: "Supabase Realtime", category: "api", status: "checking", uptime: "99.95%", lastChecked: null, description: "Websocket realtime events" },
  { id: "api-supabase-auth", name: "Supabase Auth", category: "api", status: "checking", uptime: "100%", lastChecked: null, description: "Identity and session management" },
  { id: "api-core", name: "Core Application API", category: "api", status: "checking", uptime: "99.9%", lastChecked: null, description: "Main application endpoints" },
  { id: "api-firebase", name: "Firebase Services", category: "api", status: "checking", uptime: "99.99%", lastChecked: null, description: "Storage and cloud messaging" },
  { id: "api-resend", name: "Resend Email API", category: "api", status: "checking", uptime: "99.98%", lastChecked: null, description: "Transactional email delivery" },
  { id: "pay-pawapay", name: "Pawapay Gateway", category: "payment", status: "checking", uptime: "99.7%", lastChecked: null, description: "Mobile money processing" },
  { id: "pay-stripe", name: "Stripe Gateway", category: "payment", status: "checking", uptime: "99.99%", lastChecked: null, description: "Credit card processing" },
];
...
function ServiceCard({ service, onRefresh }: { service: Service; onRefresh: (id: string) => void }) {
  return (
    <div className="flex flex-col p-4 rounded-xl border border-[#333333] bg-[#1b1b1c] hover:border-[#444444] transition-colors group">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg bg-[#252526] border border-[#333333] ${
            service.category === 'database' ? 'text-indigo-400' :
            service.category === 'api' ? 'text-sky-400' : 'text-orange-400'
          }`}>
      ...
      <div className="mt-auto pt-4 border-t border-[#333333]/50 flex items-center justify-between">
        ...
        >
          <LucideIcons.RefreshCw className={`w-3.5 h-3.5 ${service.status === 'checking' ? 'animate-spin' : ''}`} />
        </button>
      </div>
    </div>
  );
        ...
        <div className="flex items-center gap-4">
          <div className="text-right text-sm">
            ...
            <div className="font-medium flex items-center justify-end gap-2 mt-0.5">
              {globalStatus === "operational" && <span className="text-emerald-500">All Systems Normal</span>}
              {globalStatus === "degraded" && <span className="text-amber-500">Minor Issues Detected</span>}
              {globalStatus === "down" && <span className="text-red-500">System Outage</span>}
              {globalStatus === "checking" && <span className="text-blue-500">Running Diagnostics...</span>}
            </div>
          </div>
          ...
          >
            <LucideIcons.RefreshCw className={`w-4 h-4 ${globalStatus === "checking" ? 'animate-spin' : ''}`} />
            Run Diagnostics
          </button>
        </div>
      </div>
