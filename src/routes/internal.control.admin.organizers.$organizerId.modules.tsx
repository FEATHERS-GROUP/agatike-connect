...
import { getAdminOrganizerModulesData, updateAdminWorkspaceModules } from "@/api/admin_organizer_control";
import { Blocks, Building2, Check, X, RefreshCw, LayoutGrid, CheckSquare, Square } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

export const Route = createFileRoute("/internal/control/admin/organizers/$organizerId/modules")({
  loader: async ({ params }) => {
    const data = await getAdminOrganizerModulesData({ data: { organizerId: params.organizerId } } as any);
    return data;
  },
...
function OrganizerModules() {
  ...
  const startEditing = (wsId: string, currentModules: any) => {
    ...
    if (Array.isArray(currentModules)) {
      ...
      try { parsed = JSON.parse(currentModules); } catch { /* ignore */ }
    }
  ...
  const saveModules = async () => {
    ...
    setLoading(true);
    try {
      await updateAdminWorkspaceModules({ data: { workspaceId: editingWs, moduls: selectedModules } } as any);
  ...
  return (
    <div className="font-sans text-sm pb-10">
      ...
      <p className="text-[#797775] text-xs mb-6 max-w-2xl">
        Manage which platform modules are enabled for each workspace. Enabling a module grants access to its features in the workspace dashboard.
      </p>
        ...
        {workspaces.map((ws: any) => {
          ...
          if (isEditing) {
            ...
            if (Array.isArray(ws.moduls)) activeModuleIds = ws.moduls;
              ...
              try { activeModuleIds = JSON.parse(ws.moduls); } catch { /* ignore */ }
            }
          }
                  ...
                  <div key={category}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      {categorizedModules[category].map((mod: any) => {
                          ...
                          return (
                            ...
                            >
                              <div className="mt-0.5 shrink-0">
                                {isActive ? <CheckSquare className="h-4 w-4 text-[#84c87e]" /> : <Square className="h-4 w-4 text-[#555555]" />}
                              </div>
                              <div>
                                <p className={`text-sm font-medium ${isActive ? "text-[#84c87e]" : "text-[#cccccc]"}`}>{mod.label}</p>
                              </div>
                            </button>
                          );
                        }
                        ...
                        return (
                          <div key={mod.id} className="p-2 border border-[#333333] bg-[#2d2d30]/50 rounded-sm flex items-center gap-2">
