...
function AdminLeadsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      ...
      default: return "bg-[#333] text-[#ccc] border-[#444]";
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#111] text-[#ccc]">
      <div className="p-6 border-b border-[#333] shrink-0 bg-[#161616]">
        ...
        <p className="text-[12px] text-[#888] mt-1">Review and manage inbound contact and pricing plan requests.</p>
      </div>
        ...
        <div className="bg-[#161616] border border-[#333] rounded-lg overflow-hidden">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-[#2a2a2a]">
              {leads?.map((lead) => (
                <tr key={lead.id} className="hover:bg-[#1a1a1a] transition-colors">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-[#888] flex items-center gap-1 mt-0.5">
                  ...
                  <td className="px-4 py-3 text-[#aaa]">
                    {lead.company && <div className="flex items-center gap-1 text-white"><Building2 className="h-3 w-3 text-[#666]" /> {lead.company}</div>}
                    <div className="text-xs text-[#888] mt-1">{lead.country || "—"} • {lead.language || "—"}</div>
                  </td>
              ...
              {leads?.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-16 text-center text-[#666]">No leads found.</td>
                </tr>
              )}
            </tbody>
          </table>
...
function LeadModal({ lead, onClose }: { lead: Lead; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="bg-[#161616] border border-[#333] rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto">
          <div className="p-5 grid grid-cols-2 gap-6 border-b border-[#333]">
            <div>
              <div className="text-xs font-medium text-[#888] uppercase mb-1">Contact</div>
              <div className="text-white font-medium">{lead.name}</div>
              <div className="text-sm text-[#aaa]">{lead.email}</div>
              {lead.phone && <div className="text-sm text-[#aaa]">{lead.phone}</div>}
            </div>
            <div>
              <div className="text-xs font-medium text-[#888] uppercase mb-1">Company Info</div>
              <div className="text-white font-medium">{lead.company || "—"}</div>
              <div className="text-sm text-[#aaa]">{lead.country || "—"} • {lead.language || "—"}</div>
            </div>
          </div>

          <div className="p-5 border-b border-[#333] bg-[#111]">
            <div className="text-xs font-medium text-[#888] uppercase mb-2 flex items-center gap-1"><MessageSquare className="h-3 w-3"/> Message from Lead</div>
            <div className="bg-[#1a1a1a] border border-[#333] rounded p-4 text-sm text-[#ddd] whitespace-pre-wrap">
              {lead.message || <span className="text-[#666] italic">No message provided.</span>}
            </div>
          </div>

          <div className="p-5 space-y-4">
            <div>
              <label className="block text-xs font-medium text-[#888] uppercase mb-1.5">Status</label>
              <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full bg-[#111] border border-[#333] rounded-md px-3 py-2 text-sm text-white focus:border-[#f97316] outline-none">
            ...
            <div>
              <label className="block text-xs font-medium text-[#888] uppercase mb-1.5">Internal Notes</label>
              <textarea 
                rows={4}
                value={notes} 
                onChange={(e) => setNotes(e.target.value)} 
                placeholder="Add private notes about this lead..."
                className="w-full bg-[#111] border border-[#333] rounded-md px-3 py-2 text-sm text-white focus:border-[#f97316] outline-none resize-none" 
              />
            </div>
          </div>
