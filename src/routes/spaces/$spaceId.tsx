...
import { MapPin, Clock, CheckCircle2, Instagram, Facebook, Twitter, Globe, Phone, Star, ChevronLeft } from "lucide-react";
  ...
  const mockReviews = [
    {
      ...
      body: space.type === "gym" ? "Incredible facilities and super clean. The classes are great too!" : "Best wifi in Kigali. The coffee is an added bonus for long working sessions.",
      is_verified: true,
    },
    {
      id: "rev2",
      ...
      body: space.type === "gym" ? "Gets a bit crowded in the evenings, but the equipment is top notch." : "Really nice layout. Sometimes the hot desks get taken quickly if you arrive past 10am.",
      is_verified: false,
    }
  ];
                  ...
                  return (
                    <div className="p-5 rounded-2xl border border-border/40 bg-card shadow-sm animate-in fade-in duration-300">
                      ...
                      <div className="mt-6 lg:hidden h-[250px] w-full rounded-xl overflow-hidden border border-border/40 shadow-inner relative bg-secondary/20">
                        {isClient && (
                          <Suspense fallback={<div className="h-full w-full flex items-center justify-center text-muted-foreground text-sm font-medium">Loading map...</div>}>
                            <VenueMap 
                              tourStops={mapStops} 
