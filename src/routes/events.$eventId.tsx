...
import { events, ticketTiers, merch } from "@/lib/mock-data";
...
export const Route = createFileRoute("/events/$eventId")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.event.title} — Agatike` },
      ...
      <section className="relative h-[60vh] min-h-[420px] w-full overflow-hidden">
        ...
        <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-10">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            ...
            <span className="inline-flex items-center gap-1"><Star className="h-4 w-4 fill-primary text-primary" /> {event.rating}</span>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 pb-20 pt-10 lg:grid-cols-[1fr_400px]">
        ...
        <div className="space-y-10">
          <div className="flex items-center justify-between rounded-2xl border border-border/60 bg-card p-4">
            <div className="flex items-center gap-3">
              <img src={event.cover} className="h-12 w-12 rounded-full object-cover" alt={event.organizer} />
              <div>
                <p className="text-sm text-muted-foreground">Hosted by</p>
                <p className="font-semibold">{event.organizer} <span className="text-xs text-muted-foreground">@{event.organizerHandle}</span></p>
              </div>
            </div>
          ...
          <div>
            ...
            <p className="mt-3 text-muted-foreground leading-relaxed">{event.description} Expect curated sound, immersive lighting and a crowd that brings the energy. Doors open one hour before showtime — bring an ID and your good vibes.</p>
          </div>
          ...
          <div>
            <h2 className="text-xl font-semibold">People going · {event.attendees.toLocaleString()}</h2>
            <div className="mt-4 flex -space-x-3">
              ...
              <div className="ml-3 grid h-10 place-items-center rounded-full bg-secondary px-3 text-xs font-medium">+ {(event.attendees - 8).toLocaleString()}</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-4 aspect-[16/9] overflow-hidden rounded-2xl border border-border/60 bg-secondary">
              <div className="h-full w-full bg-[linear-gradient(135deg,oklch(0.95_0.02_60),oklch(0.85_0.05_50))] flex items-center justify-center text-muted-foreground">
                <MapPin className="mr-2 h-5 w-5" /> {event.venue}, {event.city}
              </div>
            </div>
          </div>
        ...
        <aside className="lg:sticky lg:top-24 h-fit">
          <div className="rounded-3xl border border-border/60 bg-card p-6 shadow-[var(--shadow-card)]">
            ...
            <Button className="mt-4 h-12 w-full rounded-2xl text-base shadow-[var(--shadow-glow)]" style={{ background: "var(--gradient-primary)" }}>
              Buy with Apple Pay
            </Button>
            <Button variant="outline" className="mt-2 h-12 w-full rounded-2xl">Checkout with card</Button>
          ...
          <div className="mt-4 flex items-center justify-between rounded-2xl border border-border/60 bg-card p-4 text-sm">
            <span className="inline-flex items-center gap-2 text-muted-foreground"><Users className="h-4 w-4" /> {event.attendees.toLocaleString()} going</span>
