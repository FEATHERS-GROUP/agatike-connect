...
function SettingsPage() {
  ...
  const DESKTOP_TABS = [
    {
      group: "Account",
      items: [
        { id: "general", label: "General Info", desc: "Name, email, phone", icon: User, color: "text-primary bg-primary/10" },
        { id: "avatar", label: "Avatar", desc: "Profile picture", icon: ImageIcon, color: "text-blue-500 bg-blue-500/10" },
        { id: "interests", label: "Interests", desc: "Event categories", icon: Heart, color: "text-rose-500 bg-rose-500/10" },
      ],
    },
    {
      group: "App Settings",
      items: [
        { id: "security", label: "Security", desc: "Update password", icon: Lock, color: "text-emerald-500 bg-emerald-500/10" },
        { id: "preferences", label: "Preferences", desc: "Dark / Light mode", icon: Monitor, color: "text-amber-500 bg-amber-500/10" },
      ],
    },
    {
      group: "Legal",
      items: [
        { id: "terms", label: "Terms & Conditions", desc: "Legal agreements", icon: FileText, color: "text-purple-500 bg-purple-500/10" },
        { id: "refunds", label: "Refund Policy", desc: "Refund rules", icon: FileText, color: "text-blue-500 bg-blue-500/10" },
        { id: "privacy", label: "Privacy Policy", desc: "How we use your data", icon: Lock, color: "text-indigo-500 bg-indigo-500/10" },
      ],
    },
    {
      group: "Danger Zone",
      items: [
        { id: "delete", label: "Delete Account", desc: "Permanently deactivate", icon: Trash2, color: "text-destructive bg-destructive/10" },
      ],
    },
  ];
  ...
  const getDesktopTabMeta = () => DESKTOP_TABS.flatMap((g) => g.items).find((i) => i.id === desktopTab);
  ...
  const renderDesktopContent = () => {
    ...
    const content = (() => {
      switch (desktopTab) {
            ...
            <form onSubmit={handleUpdateGeneral} className="space-y-5">
              {isOtpStep ? (
                <div className="flex flex-col gap-4 text-center py-8 max-w-sm mx-auto">
                  ...
                  <div className="flex flex-col gap-3 text-left">
                    <Label>One-Time Password</Label>
                    <Input value={otpInput} onChange={(e) => setOtpInput(e.target.value)} placeholder="Enter 6-digit OTP" className="bg-background/50 rounded-xl text-center text-lg tracking-widest font-mono" maxLength={6} />
                  </div>
                  <Button type="button" variant="ghost" onClick={() => setIsOtpStep(false)} className="mt-2 text-muted-foreground">Cancel</Button>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-5 max-w-2xl">
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    ...
                    <div className="flex flex-col gap-2">
                      <Label>Gender</Label>
                      <div className="relative">
                        <select value={general.gender} onChange={(e) => setGeneral({ ...general, gender: e.target.value })} className="flex appearance-none h-10 w-full rounded-xl border border-input bg-background/50 px-3 pr-10 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                  ...
                  <div className="flex flex-col gap-2">
                    <Label>Country</Label>
                    <div className="relative">
                      <select value={general.country} onChange={(e) => setGeneral({ ...general, country: e.target.value })} className="flex appearance-none h-10 w-full rounded-xl border border-input bg-background/50 px-3 pr-10 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
              ...
              <Button type="submit" disabled={isUpdatingGeneral} className="rounded-xl px-8" style={{ background: "var(--gradient-primary)" }}>
        ...
        case "avatar":
          return (
            <div className="space-y-5">
              <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
                {AVATAR_STYLES.map((style) => (
                  <button key={style} onClick={() => setSelectedStyle(style)} className={`text-xs px-3 py-1.5 rounded-full whitespace-nowrap capitalize transition-colors ${selectedStyle === style ? "bg-primary text-primary-foreground font-bold" : "bg-secondary text-muted-foreground hover:text-foreground"}`}>{style}</button>
                ))}
              </div>
              <div className="grid grid-cols-4 gap-3">
                {generatedAvatars.map((url, idx) => (
                  <button key={idx} onClick={() => setStagedAvatar(url)} disabled={isUpdatingAvatar} className={`aspect-square rounded-2xl p-2 border transition-all hover:scale-105 ${stagedAvatar === url ? "bg-primary/20 border-primary shadow-sm ring-2 ring-primary ring-offset-2 ring-offset-background" : "bg-secondary/30 border-border/40 hover:border-primary/50"}`}>
                    <img src={url} alt={`Avatar ${idx}`} className="w-full h-full object-contain" />
                  </button>
                ))}
              </div>
              <div className="flex gap-3">
                {stagedAvatar && (
                  <Button onClick={() => handleSelectAvatar(stagedAvatar)} disabled={isUpdatingAvatar} className="rounded-xl" style={{ background: "var(--gradient-primary)" }}>
                    {isUpdatingAvatar ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save Selected Avatar"}
                  </Button>
                )}
                <Button variant="secondary" onClick={() => setSeed(Math.random().toString(36).substring(7))} className="rounded-xl">
                  <RefreshCw className="h-4 w-4 mr-2" /> Generate More
                </Button>
              </div>
            </div>
        ...
        case "interests":
          return (
            <div className="space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                {INTEREST_OPTIONS.map((interest) => {
                  const isSelected = selectedInterests.some((i) => { if (typeof i !== "string") return false; const d = i.toLowerCase(); const o = interest.toLowerCase(); return d === o || d + "s" === o || d === o + "s"; });
                  return (
                    <button key={interest} onClick={() => setSelectedInterests(isSelected ? selectedInterests.filter((i) => typeof i === "string" && i.toLowerCase() !== interest.toLowerCase()) : [...selectedInterests, interest])} className={`px-4 py-2 rounded-full text-sm font-semibold border transition-all ${isSelected ? "bg-primary/20 border-primary text-primary" : "bg-secondary border-border/40 text-muted-foreground hover:text-foreground"}`}>
                      {interest}
                    </button>
                  );
                })}
              ...
              {(() => {
                ...
                const isChanged = normalizedSelected.length !== normalizedInitial.length || normalizedSelected.some((i) => !normalizedInitial.includes(i));
                return (<Button onClick={handleUpdateInterests} disabled={isUpdatingInterests || !isChanged} className="rounded-xl px-8 transition-all" style={{ background: !isChanged ? undefined : "var(--gradient-primary)" }}>{isUpdatingInterests ? <Loader2 className="h-4 w-4 animate-spin" /> : "Save Interests"}</Button>);
              })()}
            </div>
          );
        case "security":
            ...
            <form onSubmit={handleUpdatePassword} className="space-y-5 max-w-md">
              {isOtpStep ? (
                <div className="flex flex-col gap-4 text-center py-8">
                  <div className="mx-auto w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mb-2"><Lock className="w-8 h-8 text-primary" /></div>
                  <h3 className="text-xl font-bold">Verification Required</h3>
                  <p className="text-sm text-muted-foreground">We've sent a 6-digit OTP to <strong>{user?.email}</strong>. Please enter it below to securely change your password.</p>
                  <div className="flex flex-col gap-3 text-left">
                    <Label>One-Time Password</Label>
                    <Input value={otpInput} onChange={(e) => setOtpInput(e.target.value)} placeholder="Enter 6-digit OTP" className="bg-background/50 rounded-xl text-center text-lg tracking-widest font-mono" maxLength={6} />
                  </div>
              ...
              <Button type="submit" disabled={isUpdatingPassword || (!isOtpStep && (!password || !confirmPassword))} className="rounded-xl px-8" style={{ background: "var(--gradient-primary)" }}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4">
                {[{ id: "light", label: "Light", icon: Sun }, { id: "dark", label: "Dark", icon: Moon }, { id: "system", label: "System", icon: Monitor }].map((t) => (
                  <button key={t.id} onClick={() => setTheme(t.id as any)} className={`flex flex-col items-center justify-center gap-2 p-6 rounded-2xl border transition-all w-32 ${theme === t.id ? "bg-primary/10 border-primary text-primary font-bold shadow-sm" : "bg-card border-border/60 text-muted-foreground hover:bg-secondary"}`}>
        ...
        case "terms":
          return <div className="h-[60vh] overflow-y-auto"><TermsAndConditions /></div>;
        case "refunds":
          return <div className="h-[60vh] overflow-y-auto"><RefundPolicy /></div>;
        case "privacy":
          return <div className="h-[60vh] overflow-y-auto"><PrivacyPolicy /></div>;
        case "delete":
          return (
            <div className="space-y-6 max-w-md">
              <div className="flex flex-col items-center text-center gap-3 py-4">
                <div className="h-16 w-16 rounded-full bg-destructive/10 flex items-center justify-center"><Trash2 className="h-8 w-8 text-destructive" /></div>
                <h3 className="text-xl font-bold text-destructive">Delete Account</h3>
                <p className="text-sm text-muted-foreground">This will permanently deactivate your account. You will be immediately logged out and will <strong>not</strong> be able to log back in.</p>
              </div>
              <div className="bg-destructive/5 border border-destructive/20 rounded-2xl p-4 flex gap-3">
                <AlertTriangle className="h-5 w-5 text-destructive shrink-0 mt-0.5" />
                <div className="text-sm text-destructive/80 space-y-1">
              ...
              <div className="flex flex-col gap-2">
                <Label className="text-sm">Type your handle <strong className="text-foreground">@{user?.handle}</strong> to confirm:</Label>
                <Input value={deleteConfirmHandle} onChange={(e) => setDeleteConfirmHandle(e.target.value)} placeholder={`@${user?.handle}`} className="bg-background/50 rounded-xl" />
              </div>
              <Button variant="destructive" className="w-full rounded-xl" disabled={isDeletingAccount || deleteConfirmHandle !== user?.handle} onClick={handleDeleteAccount}>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="md:hidden flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-4 space-y-6">
          ...
          <div className="bg-card rounded-3xl border border-border/40 shadow-[var(--shadow-card)] overflow-hidden">
            ...
            <div className="divide-y divide-border/40">
              {[
                { id: "general", label: "General Info", desc: "Name, Email, Phone", icon: User, color: "bg-primary/10 text-primary" },
          ...
          <div className="bg-card rounded-3xl border border-border/40 shadow-[var(--shadow-card)] overflow-hidden">
            ...
            <div className="divide-y divide-border/40">
              {[
                { id: "security", label: "Security", desc: "Update your password", icon: Lock, color: "bg-emerald-500/10 text-emerald-500" },
                { id: "preferences", label: "Preferences", desc: "Dark mode, Light mode", icon: Monitor, color: "bg-amber-500/10 text-amber-500" },
                { id: "terms", label: "Terms & Conditions", desc: "Read our legal agreements", icon: FileText, color: "bg-purple-500/10 text-purple-500" },
                { id: "refunds", label: "Refund Policy", desc: "Read our refund rules", icon: FileText, color: "bg-blue-500/10 text-blue-500" },
                { id: "privacy", label: "Privacy Policy", desc: "How we handle your data", icon: Lock, color: "bg-indigo-500/10 text-indigo-500" },
              ].map((item) => (
          ...
          <div className="bg-destructive/5 border border-destructive/20 rounded-3xl overflow-hidden">
            ...
            <button onClick={() => { setDeleteConfirmHandle(""); setActiveModal("delete"); }} className="w-full flex items-center justify-between p-4 hover:bg-destructive/5 transition-colors text-left">
              <div className="flex items-center gap-4">
                <div className="h-10 w-10 rounded-full bg-destructive/10 flex items-center justify-center text-destructive"><Trash2 className="h-5 w-5" /></div>
                <div>
        ...
        {/* Mobile Modal */}
        <Dialog open={!!activeModal} onOpenChange={(open) => { if (!open) { setActiveModal(null); setIsOtpStep(false); setOtpInput(""); } }}>
          <DialogContent className="w-full h-[100dvh] max-w-none p-0 overflow-hidden bg-card border-none rounded-none flex flex-col">
            <DialogHeader className="p-4 border-b border-border/40 bg-muted/20 flex flex-row items-center gap-3 space-y-0 text-left">
              <button onClick={() => { setActiveModal(null); setIsOtpStep(false); setOtpInput(""); }} className="p-2 -ml-2 rounded-full hover:bg-secondary transition-colors">
                <ArrowLeft className="h-5 w-5" />
              </button>
        ...
        <aside className="w-72 shrink-0 border-r border-border/40 bg-card/40 backdrop-blur sticky top-0 h-screen flex flex-col overflow-y-auto pt-6 pb-8">
          ...
          <div className="px-5 mb-8">
            <div className="flex items-center gap-3 p-4 rounded-2xl bg-background/60 border border-border/40">
              {user?.profile ? (
                <img src={user.profile} alt={user.username} className="h-12 w-12 rounded-full object-cover border-2 border-border/40" />
              ) : (
          ...
          <nav className="flex-1 px-3 space-y-6">
            {DESKTOP_TABS.map((group) => (
              <div key={group.group}>
                <p className="px-3 mb-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground/60">{group.group}</p>
                <div className="space-y-0.5">
                  {group.items.map((item) => {
                    ...
                    return (
                      <button
                        key={item.id}
                        onClick={() => { setDesktopTab(item.id); setIsOtpStep(false); setOtpInput(""); }}
                      ...
                      >
                        <div className={`h-8 w-8 rounded-lg flex items-center justify-center shrink-0 transition-colors ${isActive ? item.color : "bg-secondary/60"}`}>
          ...
          <div className="px-5 pt-4 border-t border-border/40">
            <button onClick={() => navigate({ to: "/profile" })} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="flex-1 flex flex-col min-h-screen">
          ...
          <main className="flex-1 p-10">
            ...
            <div className="mb-8 pb-6 border-b border-border/40">
              <div className="flex items-center gap-4">
                {desktopTabMeta && (
                  <div className={`h-12 w-12 rounded-2xl flex items-center justify-center shrink-0 ${desktopTabMeta.color}`}>
