  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.specs?.linked_assets) {
        const events = product.specs.linked_assets.filter((a: any) => a.type === "event").map((a: any) => a.id);
        const venues = product.specs.linked_assets.filter((a: any) => a.type === "venue").map((a: any) => a.id);
