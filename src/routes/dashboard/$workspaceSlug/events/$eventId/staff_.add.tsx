...
function AddStaffPage() {
  ...
  const { canAddEventStaff } = useSubscriptionLimits(
  ...
  const [noAccountInputs, setNoAccountInputs] = useState([
    { first_name: "", last_name: "", email: "", phone: "", id_passport: "", profile_image: "" },
  ]);
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      const generatePin = () => {
        ...
        const numbers =
          "01234567899876543210234567890134567890124567890123567890123467890123457890123456890123456890123456790123456780123456789";
        while (true) {
          let randomNums = "";
          for (let i = 0; i < 5; i++) {
      ...
      if (registrationType === "account") {
        ...
        if (validProfiles.length === 0)
          throw new Error("No valid profiles provided (First Name and Email are required).");
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-12 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-2">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className={`h-2 flex-1 rounded-full transition-colors ${step >= i ? "bg-primary" : "bg-secondary"}`}
          />
        ))}
      </div>
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-sm text-muted-foreground text-center">
                  Search for users who already have an account on Agatike using their handle or
                  email.
                </p>
              </button>
              <button
              ...
              >
                ...
                <p className="text-sm text-muted-foreground text-center">
                  Add external staff by manually entering their name and contact information.
                </p>
              </button>
            </div>
            <div className="flex justify-end pt-4">
              <Button
                onClick={handleNext}
                className="bg-primary text-primary-foreground font-bold shadow-[var(--shadow-glow)]"
              >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {accountInputs.map((input, index) => (
                <div
                  key={index}
                  className="flex items-start gap-4 p-4 border border-border/50 rounded-xl bg-secondary/20"
                >
                  <div className="flex-1 space-y-2">
                    ...
                    {input.error && (
                      <p className="text-xs text-red-500 flex items-center">
                        <X className="h-3 w-3 mr-1" /> {input.error}
                      </p>
                    )}
                    {input.user && (
                        ...
                        <div className="flex items-center gap-3 p-3 bg-card rounded-lg border border-border shadow-sm">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-1">
                              @{input.user.handle} • {input.user.email}
                            </p>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <p className="text-[10px] text-muted-foreground">
                              Overrides the username on their badge.
                            </p>
                          </div>
                          <div className="space-y-2">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
              {noAccountInputs.map((input, index) => (
                <div
                  key={index}
                  className="p-4 border border-border/50 rounded-xl bg-secondary/20 relative"
                >
                  {noAccountInputs.length > 1 && (
                    <button
                      onClick={() =>
                        setNoAccountInputs(noAccountInputs.filter((_, i) => i !== index))
                      }
        ...
        {step === 3 && (
          <div className="space-y-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                These settings will be applied to all {getValidCount()} users in this batch.
              </p>
            </div>
            ...
            <div className="space-y-10 bg-secondary/10 border border-border/50 rounded-xl p-6 sm:p-8">
              <div className="space-y-4">
                <div>
                  <Label className="text-base font-bold flex items-center gap-2">
                    <Shield className="h-5 w-5 text-primary" /> Event Role
                  </Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Select the primary job title for these staff members.
                  </p>
                </div>
                <div className="max-w-md">
                  <Select
                    value={permissions.role}
                    onValueChange={(val) => setPermissions({ ...permissions, role: val })}
                  >
              ...
              <div className="space-y-4 pt-8 border-t border-border/50">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Select the features this staff member can access on their mobile app.
                  </p>
                </div>
                  ...
                  {[
                    {
                      id: "SCAN_TICKETS",
                      label: "Scan Tickets",
                      desc: "Access the scanner for entry",
                      icon: ScanLine,
                      color: "text-blue-500",
                      bg: "bg-blue-500/10",
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          if (isSelected) {
                            setPermissions({
                              ...permissions,
                              app_permissions: permissions.app_permissions.filter(
                                (id) => id !== perm.id,
                              ),
                            });
                          } else {
                            setPermissions({
                              ...permissions,
                              app_permissions: [...permissions.app_permissions, perm.id],
                            });
                          }
                        }}
                      ...
                      >
                        <div className="flex flex-col items-start gap-4">
                          <div
                            className={`p-3 rounded-xl shrink-0 transition-colors ${isSelected ? perm.bg + " " + perm.color : "bg-muted text-muted-foreground"}`}
                          >
                            <perm.icon className="h-6 w-6" />
                          </div>
                          <div>
                            <h4
                              className={`font-semibold tracking-tight ${isSelected ? "text-foreground" : "text-foreground/80"}`}
                            >
                              {perm.label}
                            </h4>
              ...
              <div className="space-y-4 pt-8 border-t border-border/50">
                <div>
                  <Label className="text-base font-bold">
                    Allowed Sections (Where they can go)
                  </Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Define physical access control for this batch of staff members.
                  </p>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                  ...
                  >
                    <Shield
                      className={`h-6 w-6 ${permissions.allowed_sections.includes("*") ? "animate-pulse" : ""}`}
                    />
                  ...
                  {sections.map((sec) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (isSelected) {
                            setPermissions({
                              ...permissions,
                              allowed_sections: permissions.allowed_sections.filter(
                                (id) => id !== sec.id,
                              ),
                            });
                          } else {
                            setPermissions({
                              ...permissions,
                              allowed_sections: [...permissions.allowed_sections, sec.id],
                            });
                          }
                        }}
                      ...
                      >
                        <MapPin
                          className={`h-5 w-5 ${isSelected ? "text-primary" : "text-muted-foreground opacity-70"}`}
                        />
                  ...
                  {sections.length === 0 && (
                    <div className="col-span-full">
                      <p className="text-sm text-muted-foreground italic p-4 text-center border border-dashed rounded-xl">
          ...
          <div className="space-y-8">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                You are about to add {getValidCount()} staff member(s).
              </p>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <p className="font-medium">
                    {permissions.app_permissions.length > 0
                      ? `${permissions.app_permissions.length} Features Enabled`
                      : "No Features"}
                  </p>
                </div>
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <p className="font-medium">
                    {permissions.allowed_sections.includes("*")
                      ? "All Access"
                      : permissions.allowed_sections.length > 0
                        ? `${permissions.allowed_sections.length} Sections Selected`
                        : "No Sections"}
                  </p>
                </div>
              </div>
