...
export const Route = createFileRoute(
  "/dashboard/$workspaceSlug/events/$eventId/products_/$productId/edit",
)({
      ...
      if (!isNaN(limit)) {
        ...
        if (currentTotal + newStock > limit) {
          toast.error("Stock Limit Exceeded", {
            description: `You only have ${limit - currentTotal} stock remaining for sizes.`,
          });
          return;
        }
      }
      ...
      sizes.map((size) => {
        if (size.name === sizeName) {
          ...
          if (currentColorTotal + newStock > size.stock) {
            toast.error("Size Stock Exceeded", {
              description: `You only have ${size.stock - currentColorTotal} stock remaining for colors within the '${sizeName}' size.`,
            });
            return size;
          }
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (formData.type === "physical") {
        ...
        payload.available_colors = null; // Deprecated, explicitly clear it
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.type === "physical" && formData.stock_limit) {
      ...
      if (!isNaN(limit)) {
        ...
        if (totalSizes > limit) {
          toast.error("Stock Limit Exceeded", {
            description: "The total stock assigned to sizes exceeds the global Total Stock limit.",
          });
          return;
        }
        ...
        for (const size of sizes) {
          ...
          if (totalColors > size.stock) {
            toast.error("Size Stock Exceeded", {
              description: `The colors under size '${size.name}' exceed the stock allocated for that size.`,
            });
            return;
          }
        }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            <div className="rounded-3xl border border-border/40 bg-card p-6 shadow-[var(--shadow-card)] flex flex-col gap-6">
              <h3 className="text-sm font-bold text-foreground uppercase tracking-widest border-b border-border/20 pb-4">
                Basic Information
              </h3>
            ...
            {formData.type === "physical" && (
              <div className="rounded-3xl border border-border/40 bg-card p-8 shadow-[var(--shadow-card)] space-y-10">
                ...
                <div className="space-y-3">
                  ...
                  >
                    <SelectTrigger className="h-14 rounded-2xl bg-secondary/20 border-border/40 focus:bg-background text-base">
                      <SelectValue placeholder="Select category..." />
                    </SelectTrigger>
                    <SelectContent className="rounded-2xl border-border/40 shadow-xl">
                      <SelectItem value="tshirts" className="py-3">
                        T-Shirts
                      </SelectItem>
                      <SelectItem value="caps" className="py-3">
                        Caps / Hats
                      </SelectItem>
                      <SelectItem value="jumpers" className="py-3">
                        Jumpers / Hoodies
                      </SelectItem>
                      <SelectItem value="clothes" className="py-3">
                        Clothes / Apparel
                      </SelectItem>
                      <SelectItem value="accessories" className="py-3">
                        Accessories
                      </SelectItem>
                      <SelectItem value="books" className="py-3">
                        Books / Media
                      </SelectItem>
                      <SelectItem value="art" className="py-3">
                        Art / Portraits
                      </SelectItem>
                      <SelectItem value="bottles" className="py-3">
                        Bottles / Drinks
                      </SelectItem>
                      <SelectItem value="jewelry" className="py-3">
                        Jewelry / Rings
                      </SelectItem>
                ...
                <div className="space-y-4">
                  <Label className="text-xs uppercase tracking-widest font-semibold text-muted-foreground">
                    Available Variants & Sub-variants
                  </Label>
                  ...
                  <div className="flex flex-col gap-6 mb-4">
                    {sizes.map((size) => (
                      <div
                        key={size.name}
                        className="flex flex-col bg-secondary/20 rounded-3xl border border-border/30 shadow-sm overflow-hidden"
                      >
                        {/* Size Header */}
                        <div className="flex justify-between items-center bg-secondary/60 px-5 py-4 border-b border-border/30">
                          ...
                          <div className="flex items-center gap-6">
                            <span className="text-muted-foreground text-xs uppercase tracking-widest font-semibold">
                              Variant Stock: <span className="text-foreground">{size.stock}</span>
                            </span>
                            <button
                              type="button"
                              onClick={() => setSizes(sizes.filter((s) => s.name !== size.name))}
                              className="text-muted-foreground hover:text-red-500 transition-colors p-2 hover:bg-red-500/10 rounded-full"
                            >
                        ...
                        <div className="p-5 space-y-4">
                          {size.colors.length > 0 && (
                            <div className="flex flex-col gap-2">
                              {size.colors.map((color) => (
                                <div
                                  key={color.name}
                                  className="flex justify-between items-center bg-background px-4 py-3 rounded-2xl border border-border/30 shadow-sm"
                                >
                                  <span className="font-bold text-sm">{color.name}</span>
                                  <div className="flex items-center gap-4">
                                    <span className="text-muted-foreground text-xs uppercase tracking-widest font-semibold">
                                      Stock: <span className="text-foreground">{color.stock}</span>
                                    </span>
                                    <button
                                      type="button"
                                      onClick={() => removeColorFromSize(size.name, color.name)}
                                      className="text-muted-foreground hover:text-red-500 transition-colors p-1.5 hover:bg-red-500/10 rounded-full"
                                    >
                          ...
                          <div className="flex gap-3 items-center bg-background p-1.5 rounded-2xl border border-border/20">
                            <Input
                              ...
                              onKeyDown={(e) =>
                                e.key === "Enter" && (e.preventDefault(), addColorToSize(size.name))
                              }
                            ...
                            <Button
                              type="button"
                              variant="secondary"
                              onClick={() => addColorToSize(size.name)}
                              className="rounded-xl h-9 px-4 text-xs font-semibold bg-primary/10 text-primary hover:bg-primary/20"
                            >
                              Add Sub-variant
                            </Button>
                          </div>
                  ...
                  <div className="flex gap-3 items-center bg-secondary/10 p-2 rounded-[1.25rem] border border-border/20">
                    <Input
                      placeholder="Add a new variant (e.g. XL, Hardcover, 500ml)"
                      value={sizeInput}
                      onChange={(e) => setSizeInput(e.target.value)}
                      onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addSize())}
                      className="h-12 rounded-xl flex-1 bg-transparent border-none shadow-none focus-visible:ring-0 px-4"
                    />
                    ...
                    <Input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addSize())}
                      className="h-12 rounded-xl w-32 bg-transparent border-none shadow-none focus-visible:ring-0 px-4"
                    />
                    <Button
                      type="button"
                      variant="secondary"
                      onClick={addSize}
                      className="rounded-xl h-10 px-6 font-semibold bg-primary/10 text-primary hover:bg-primary/20"
                    >
                      Add Variant
                    </Button>
                  </div>
                ...
                <div className="space-y-4 pt-4 border-t border-border/20">
                  ...
                  <div className="space-y-3">
                    {specs.map((spec, idx) => (
                      <div
                        key={idx}
                        className="flex gap-3 items-center bg-secondary/10 p-2 rounded-[1.25rem] border border-border/20"
                      >
                        <Input
                        ...
                        <Button
                          ...
                          className="text-muted-foreground hover:text-red-500 p-2 hover:bg-red-500/10 rounded-full h-10 w-10"
                        >
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={addSpec}
                    className="w-full h-14 rounded-2xl border-dashed border-border/60 hover:bg-secondary/30 transition-colors font-medium"
                  >
