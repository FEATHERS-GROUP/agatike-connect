...
import { getSpaceSubscriptionsBySpaceId, renewSpaceSubscription, cancelSpaceSubscription } from "@/api/space_subscriptions";
import {
  RefreshCw, UserCheck, Search, Download, Eye, ReceiptText,
  AlertCircle, Clock, XCircle, Loader2, CheckCircle2,
} from "lucide-react";
...
function SpaceSubscriptionsPage() {
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      toast.success(`Renewed! Next billing: ${new Date(result.newNextBillingDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}`);
                    ...
                    return (
                      <tr key={sub.id} className="hover:bg-secondary/5 transition-colors">
                        ...
                        <td className="px-6 py-4 text-xs whitespace-nowrap">
                          <span className={
                            renewalState === "expiring_soon" ? "text-amber-500 font-bold" :
                            renewalState === "grace_period" ? "text-rose-500 font-bold" :
                            renewalState === "on_hold" ? "text-muted-foreground line-through" :
                            "text-foreground font-semibold"
                          }>
                            {nextBillingDate}
                          </span>
                          {renewalState === "expiring_soon" && daysLeft !== null && (
                            <div className="text-[10px] text-amber-500 mt-0.5">{daysLeft}d left</div>
                          )}
                          {renewalState === "grace_period" && daysLeft !== null && (
                            <div className="text-[10px] text-rose-500 mt-0.5">{Math.abs(daysLeft)}d overdue</div>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${statusClass}`}>
                            {statusLabel}
                          </span>
                        </td>
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold mt-0.5">
                        {selectedSub.start_date
                          ? new Date(selectedSub.start_date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })
                          : new Date(selectedSub.created_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Next Billing</p>
                      <p className={`text-sm font-semibold mt-0.5 ${
                        renewalState === "expiring_soon" ? "text-amber-500" :
                        renewalState === "grace_period" ? "text-rose-500" :
                        "text-orange-500"
                      }`}>
                        {selectedSub.next_billing_date
                          ? new Date(selectedSub.next_billing_date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })
                          : "—"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Status</p>
                      <span className={`inline-flex items-center mt-0.5 px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${statusClass}`}>
                        {statusLabel}
                      </span>
                    </div>
                  </div>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            Renew now to avoid any service interruption. An invoice will be generated automatically.
                          </p>
                        </div>
                      </div>
                      <Button
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p className="text-sm font-semibold text-rose-600 dark:text-rose-400">
                            {Math.abs(daysLeft!)} day{Math.abs(daysLeft!) !== 1 ? "s" : ""} overdue — Grace period
                          </p>
                          <p className="text-xs text-muted-foreground mt-0.5">
                            This subscription is past its billing date. Renew to continue access, or cancel to end it.
                            After 7 days the membership will be automatically put on hold.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            This subscription has been placed on hold because it was not renewed within the grace period.
