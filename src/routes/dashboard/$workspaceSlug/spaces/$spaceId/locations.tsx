    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {space.locations?.map((loc: any, idx: number) => (
          <div
            key={idx}
            className="bg-card border border-border/60 rounded-3xl overflow-hidden shadow-sm flex flex-col"
          >
              ...
              <div className="grid grid-cols-1 gap-2">
                {loc.opening_hours ? (
                  Object.entries(loc.opening_hours).map(([day, h]: any) => (
                    <div
                      key={day}
                      className="flex justify-between items-center text-sm p-2 rounded-lg hover:bg-secondary/20 transition-colors"
                    >
        ...
        {(!space.locations || space.locations.length === 0) && (
          <div className="col-span-full p-12 text-center bg-secondary/20 border border-dashed border-border/60 rounded-3xl">
            ...
            <p className="text-muted-foreground">
              This space doesn't have any physical locations yet.
            </p>
          </div>
        )}
      </div>
