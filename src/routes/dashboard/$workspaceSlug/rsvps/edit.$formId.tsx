...
export const Route = createFileRoute("/dashboard/$workspaceSlug/rsvps/edit/$formId")({
...
function EditFormPage() {
  ...
  const { workspaceSlug, formId } = Route.useParams();
  const [coverFile, setCoverFile] = useState<File | null>(null);

  const { data: formDetails, isLoading } = useQuery({
  ...
  const {
    ...
    defaultValues: { title: "", description: "", cover_image_url: "", fields: [] },
  });
  ...
  useEffect(() => {
    if (formDetails) {
      reset({
        ...
        cover_image_url: formDetails.cover_image_url || "",
        fields: (formDetails.form_fields || []).map((f: any) => ({
          id: f.id,
          label: f.label,
  ...
  const mutation = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      return updateCustomFormAndFields({
        data: {
          id: formId,
          title: values.title,
