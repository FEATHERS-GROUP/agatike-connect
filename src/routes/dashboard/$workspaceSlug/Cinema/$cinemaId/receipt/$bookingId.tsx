    ...
    <div className="min-h-screen bg-secondary/30 pb-20 print:min-h-0 print:bg-white print:pb-0 print:m-0">
      ...
      <div className="p-4 flex items-center justify-between max-w-sm mx-auto print:hidden">
        <Button
          ...
          onClick={() =>
            navigate({
              to: `/dashboard/$workspaceSlug/Cinema/$cinemaId/bookings`,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 print:block print:space-y-0 max-w-6xl mx-auto px-4 pb-12 print:px-0 print:pb-0">
        {Array.from({ length: booking.quantity }).map((_, index) => (
          <div
            key={index}
            className={`w-full max-w-[320px] mx-auto bg-white text-black p-6 shadow-sm border border-border/40 print:shadow-none print:border-none print:max-w-none print:w-full print:p-0 print:m-0 print:break-inside-avoid ${index < booking.quantity - 1 ? "print:break-after-page" : ""}`}
          >
            {/* Header */}
            <div className="text-center border-b border-black/20 pb-4 mb-4 space-y-1">
              {cinema.logo_url && (
                <img
                  src={cinema.logo_url}
                  alt="Logo"
                  className="h-12 object-contain mx-auto mb-2 grayscale"
                />
              )}
            ...
            <div className="text-center space-y-0.5 mb-6">
              <p className="text-[10px] uppercase font-bold text-black/60 tracking-widest">
                Receipt
              </p>
            ...
            <div className="mb-4">
              <div className="flex justify-between text-sm mb-2">
                <span className="font-bold">
                  1x {ticket_tier?.name || "Ticket"} (Ticket {index + 1} of {booking.quantity})
                </span>
