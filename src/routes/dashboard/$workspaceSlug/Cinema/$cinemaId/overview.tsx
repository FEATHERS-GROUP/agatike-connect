...
import { Film, MapPin, Ticket, TrendingUp, Users, Loader2, Tag, Calendar, ChevronDown, BarChart3 } from "lucide-react";
...
function CinemaOverview() {
  ...
  const upcomingSchedules = (cinema.schedules || [])
    .filter((s: any) => {
      if (!s.start_time) return false;
      const showStart = new Date(`${s.show_date}T${s.start_time}`);
      const isToday = showStart.getFullYear() === now.getFullYear() && 
          ...
          {nextSchedules.length === 0 ? (
            ...
            <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
              {nextSchedules.map((schedule: any) => (
                <div key={schedule.id} className="min-w-[140px] max-w-[140px] shrink-0 snap-start space-y-2 opacity-80 hover:opacity-100 transition-opacity">
                  <div className="aspect-[2/3] rounded-xl overflow-hidden bg-secondary border border-border/40 shadow-sm relative">
                    <img 
                      src={schedule.movie?.cover_url || "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?auto=format&fit=crop&q=80&w=400"} 
                      alt={schedule.movie?.title}
                    ...
                    <div className="absolute top-2 right-2 bg-secondary text-foreground text-[10px] font-bold px-2 py-0.5 rounded-full border border-border/60 shadow-sm">
                      {new Date(schedule.show_date).toLocaleDateString() === now.toLocaleDateString() ? "Today" : new Date(schedule.show_date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}
                    </div>
                    <div className="absolute bottom-2 left-2 right-2 bg-black/70 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-lg text-center truncate shadow-sm border border-white/10">
                      {schedule.screen?.name || "Screen 1"}
                    </div>
                  ...
                  <div>
                    <p className="font-bold text-sm truncate" title={schedule.movie?.title}>{schedule.movie?.title}</p>
                    <p className="text-xs text-primary font-semibold">{schedule.start_time.substring(0, 5)}</p>
                  </div>
                </div>
              ))}
            </div>
