...
function CreateMovieWizard() {
  ...
  const handleSearchOMDb = async () => {
    ...
    setIsSearching(true);
    try {
      const res = await fetch(`https://www.omdbapi.com/?s=${encodeURIComponent(searchQuery)}&type=movie&apikey=${OMDB_API_KEY}`);
    ...
    try {
      ...
      if (data.Response === "True") {
        setForm((p: any) => ({
          ...
          language: data.Language && data.Language !== "N/A" ? data.Language.split(",")[0] : "English",
        }));
  ...
  return (
    <div className="flex w-full h-screen bg-background">
      ...
      <div className="hidden lg:flex w-80 flex-col border-r border-border/60 bg-secondary/10 p-6 shadow-[4px_0_24px_rgba(0,0,0,0.02)] h-full shrink-0">
        <button
          onClick={() => navigate({ to: "/dashboard/$workspaceSlug/Cinema/movies", params: { workspaceSlug } })}
              ...
              <div className="flex flex-col items-center justify-center min-h-[calc(100vh-10rem)] animate-in fade-in zoom-in-95 duration-500 w-full max-w-4xl mx-auto">
                <div className="text-center max-w-2xl mx-auto mb-12 mt-[-4rem]">
                  ...
                  <p className="text-muted-foreground text-lg">
                    You can search the global movie database to automatically fill in details, or build it manually from scratch.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-3xl mx-auto">
                  ...
                  >
                    ...
                    <p className="text-muted-foreground text-sm">
                      Search OMDb and instantly import the poster, synopsis, duration, genres, and ratings.
                    </p>
                  </button>
                  ...
                  >
                    ...
                    <p className="text-muted-foreground text-sm">
                      Start with a blank form and manually type in all the details and upload your own poster.
                    </p>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-muted-foreground">
                    Search the global database to automatically fill in the movie's poster, plot, rating, and details.
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {isSearching && !searchSelected ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
                    Search
                  </Button>
                </div>
                    ...
                    {searchResults.map((res: any) => (
                      ...
                      >
                        <div className="w-16 h-24 shrink-0 rounded-lg bg-secondary overflow-hidden">
                          {res.Poster !== "N/A" ? (
                            <img src={res.Poster} alt={res.Title} className="w-full h-full object-cover" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center"><Film className="h-6 w-6 text-muted-foreground" /></div>
                          )}
                        </div>
                        <div className="flex-1 py-1">
                          <h4 className="font-bold text-sm leading-tight mb-1 group-hover:text-primary transition-colors">{res.Title}</h4>
                ...
                <div className="relative py-4 mt-8">
                  ...
                  <div className="relative flex justify-center"><span className="bg-card px-4 text-xs font-medium text-muted-foreground">OR</span></div>
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2 md:col-span-2">
                    ...
                    <Input autoFocus value={form.title} onChange={(e) => set("title", e.target.value)} placeholder="e.g. Inception" className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <Label>Synopsis</Label>
                    <Textarea value={form.synopsis} onChange={(e) => set("synopsis", e.target.value)} placeholder="Brief description..." className="rounded-xl min-h-[120px] resize-none" />
                  </div>
                  <div className="space-y-2">
                    <Label>Genre</Label>
                    <Input value={form.genre} onChange={(e) => set("genre", e.target.value)} placeholder="Action, Sci-Fi..." className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-2">
                    <Label>Duration (minutes)</Label>
                    <Input type="number" value={form.duration_minutes} onChange={(e) => set("duration_minutes", parseInt(e.target.value))} className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-2">
                    <Label>Release Date</Label>
                    <Input type="date" value={form.release_date} onChange={(e) => set("release_date", e.target.value)} className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-2">
                    <Label>Language</Label>
                    <Input value={form.language} onChange={(e) => set("language", e.target.value)} placeholder="English" className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-2 md:col-span-2">
                    <Label>Rating</Label>
                    <Input value={form.rating} onChange={(e) => set("rating", e.target.value)} placeholder="PG-13, R, etc." className="rounded-xl h-11" />
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground">Posters, trailers, and production details.</p>
                </div>

                <div className="space-y-2">
                  <Label className="flex items-center gap-1.5">Poster / Cover Image</Label>
                  <div className="flex gap-2">
                    <Input value={form.cover_url} onChange={(e) => set("cover_url", e.target.value)} placeholder="https://... or upload" className="rounded-xl h-11 flex-1" />
                    <div className="relative">
                      <Input
                        ...
                        onChange={async (e) => {
                          ...
                          try {
                            const url = await uploadFileToStorage(e.target.files[0], "movies/covers");
                            set("cover_url", url);
                      ...
                      />
                      <Button type="button" variant="outline" className="h-11 rounded-xl px-4" disabled={isUploadingCover}>
                  ...
                  {form.cover_url && (
                    <div className="mt-3 rounded-2xl overflow-hidden border border-border/60 aspect-[2/3] w-48 bg-secondary relative">
                      <img src={form.cover_url} alt="Cover preview" className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={form.trailer_url} onChange={(e) => set("trailer_url", e.target.value)} placeholder="https://youtube.com/watch?v=..." className="rounded-xl h-11" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                  <div className="space-y-2">
                    <Label>Director</Label>
                    <Input value={form.director} onChange={(e) => set("director", e.target.value)} className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-2">
                    <Label>Distributor (Production)</Label>
                    <Input value={form.distributor} onChange={(e) => set("distributor", e.target.value)} className="rounded-xl h-11" />
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground">What premium formats does this movie support?</p>
                </div>
                ...
                <div className="flex gap-4 flex-wrap pt-4">
                  <label className="flex items-center gap-3 cursor-pointer p-5 border border-border/60 rounded-2xl hover:bg-secondary/50 flex-1 transition-colors">
                    <input type="checkbox" checked={form.is_3d} onChange={(e) => set("is_3d", e.target.checked)} className="rounded border-border text-primary focus:ring-primary h-5 w-5" />
                    <div>
                      <span className="block font-bold">3D Format</span>
                      <span className="block text-xs text-muted-foreground">Stereoscopic 3D rendering</span>
                    </div>
                  </label>
                  <label className="flex items-center gap-3 cursor-pointer p-5 border border-border/60 rounded-2xl hover:bg-secondary/50 flex-1 transition-colors">
                    <input type="checkbox" checked={form.is_imax} onChange={(e) => set("is_imax", e.target.checked)} className="rounded border-border text-primary focus:ring-primary h-5 w-5" />
                    <div>
                      <span className="block font-bold">IMAX Format</span>
                      <span className="block text-xs text-muted-foreground">High resolution, larger aspect ratio</span>
                    </div>
                  </label>
                </div>
              </div>
            ...
            {step === 4 && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground">Final check before adding this movie to your global catalog.</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-6 p-6 rounded-3xl bg-secondary/30 border border-border/40">
                  <div className="w-32 aspect-[2/3] shrink-0 rounded-xl overflow-hidden bg-secondary border border-border/40">
                    {form.cover_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center"><Film className="h-8 w-8 text-muted-foreground opacity-50" /></div>
                    )}
                  </div>
                  <div className="flex-1">
                    <h3 className="text-2xl font-black leading-tight mb-2">{form.title || "Untitled Movie"}</h3>
                    <div className="flex flex-wrap gap-2 text-sm text-muted-foreground mb-4 font-medium">
                      <span className="px-2 py-0.5 rounded bg-primary/10 text-primary">{form.rating}</span>
                      <span>•</span>
                      <span>{form.duration_minutes} min</span>
                      <span>•</span>
                      <span>{form.genre}</span>
                    </div>
                    <p className="text-sm line-clamp-3 text-muted-foreground mb-4">{form.synopsis}</p>
                    <div className="flex gap-2">
                      {form.is_3d && <span className="px-2 py-0.5 rounded text-xs font-bold bg-white text-black">3D</span>}
                      {form.is_imax && <span className="px-2 py-0.5 rounded text-xs font-bold bg-white text-black">IMAX</span>}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <Button
                ...
                onClick={() => {
                  if (step === 0) {
                    ...
                    else navigate({ to: "/dashboard/$workspaceSlug/Cinema/movies", params: { workspaceSlug } });
                  } else {
                    setStep(step - 1);
                  }
                }}
