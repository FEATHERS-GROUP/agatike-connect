    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex gap-2">
          ...
          <Button
            className="rounded-xl gap-2 h-10 text-white"
      ...
      <div className="flex items-start justify-center">
        ...
        <div className="flex-1 overflow-x-auto bg-slate-100 p-8 rounded-3xl shadow-inner min-h-[800px] flex items-start justify-center print:bg-white print:p-0 print:shadow-none print:rounded-none">
          <div
            className={cn(
              "bg-white text-slate-900 w-[210mm] min-h-[297mm] shadow-2xl p-[20mm] flex flex-col relative print:shadow-none print:w-full print:h-full print:p-0",
              theme.pageBorder,
            )}
          >
            {theme.watermark && (
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center opacity-[0.03] overflow-hidden">
                <span className="text-[150px] font-black -rotate-45 whitespace-nowrap">
                  {theme.watermark}
                </span>
              </div>
            )}
            ...
            <div className="flex justify-between items-start mb-12 relative z-10">
              <div className="space-y-2">
                <h1
                  className={cn("text-4xl font-black uppercase tracking-widest", theme.textAccent)}
                >
                  {titleLabel}
                </h1>
            ...
            <div className="grid grid-cols-2 gap-12 mb-12 relative z-10">
              <div
                className={cn("p-4 rounded-xl", isPO ? "bg-slate-100 border border-slate-200" : "")}
              >
                <h3
                  className={cn(
                    "text-xs font-bold uppercase tracking-wider mb-3",
                    isPO ? "text-slate-700" : "text-slate-400",
                  )}
                >
                  {billToLabel}
                </h3>
                <div className="space-y-1">
                  ...
                  {invoice.client_company && (
                    <p className="text-slate-600">{invoice.client_company}</p>
                  )}
            ...
            <div className="mb-8 flex-1 relative z-10">
              <div
                className={cn(
                  "grid grid-cols-[1fr_80px_120px_120px] gap-4 pb-2 mb-4",
                  theme.tableHeaderBg,
                  isPO ? "p-3 rounded-t-lg" : "",
                )}
              >
                <div
                  className={cn(
                ...
                <div
                  className={cn(
                    "text-xs font-bold uppercase tracking-wider text-right",
                    isPO ? "text-slate-200" : "",
                  )}
                >
                  Unit Price
              ...
              <div className="space-y-2">
                {items.map((item: any, idx: number) => (
                  <div
                    key={idx}
                    className={cn(
                      "grid grid-cols-[1fr_80px_120px_120px] gap-4 items-center group py-2",
                      isPO ? "border-b border-slate-100" : "",
                    )}
                  >
            ...
            <div className="grid grid-cols-2 gap-12 mt-auto relative z-10">
              ...
              <div
                className={cn(
                  "p-6 rounded-2xl flex flex-col justify-center space-y-3",
                  theme.bgLight,
                )}
              >
                ...
                <div className="flex justify-between text-sm text-slate-600">
                  <span className="flex items-center gap-2">Tax ({invoice.tax_rate}%)</span>
                  <span className="font-semibold">
                ...
                <div
                  className={cn(
                    "flex justify-between text-xl font-black pt-3 border-t-2",
                    theme.borderAccent,
                    theme.textAccent,
                  )}
                >
