...
function SubscriptionsPage() {
  ...
  useEffect(() => {
    async function loadSub() {
      ...
      try {
        const sub = await getActiveSubscription({ data: { organizer_id: activeWorkspace.orgnizer_id } } as any);
        if (sub) {
          setSubscription(sub);
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 py-8 px-4">
        ...
        <div className="flex flex-col items-center justify-center py-16 bg-card border border-border/60 rounded-3xl shadow-sm text-center px-4">
          ...
          <Button asChild size="lg" className="rounded-full px-8 shadow-md transition-transform hover:scale-105" style={{ background: "var(--gradient-primary)" }}>
  ...
  const nextBillingDate = subscription.next_billing_date 
    ? new Date(subscription.next_billing_date).toLocaleDateString("en-US", { year: 'numeric', month: 'long', day: 'numeric' })
    : "N/A";
      ...
      <section className="space-y-4">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-background to-secondary/30 p-8 sm:p-10 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <p className="text-muted-foreground">
                <span className="text-3xl font-medium text-foreground tracking-tight">
                  {isFree ? "Free" : `${plan.currency === 'USD' ? '$' : plan.currency + ' '}${subscription.amount}`}
                </span> 
                {!isFree && <span className="text-lg"> / {plan.billing_cycle === "yearly" ? "yr" : "mo"}</span>}
              </p>
              {!isFree && (
                <p className="text-sm text-muted-foreground pt-1">
                  Your next charge will be on <span className="font-medium text-foreground">{nextBillingDate}</span>.
                </p>
              )}
            </div>
          ...
          <div className="relative z-10 mt-10 pt-8 border-t border-border/40">
            <h4 className="text-sm font-bold mb-6 uppercase tracking-wider text-muted-foreground">What's Included</h4>
