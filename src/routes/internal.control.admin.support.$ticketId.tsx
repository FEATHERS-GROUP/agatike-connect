...
const STATUS_CONFIG: Record<string, { label: string; icon: any; color: string; bg: string }> = {
  ...
  closed: { label: "Closed", icon: XCircle, color: "text-gray-500 dark:text-[#888]", bg: "bg-gray-100 dark:bg-[#222] border-gray-200 dark:border-[#333]" },
};
      ...
      <div className="max-w-6xl w-full mx-auto flex flex-col gap-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 md:p-6 rounded-xl border border-gray-200 dark:border-[#333] bg-gray-50 dark:bg-[#161616] shadow-sm relative overflow-hidden">
          <div className="flex items-start gap-4 relative z-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[11px] font-bold text-gray-500 dark:text-[#888] uppercase tracking-wider">Ticket</span>
                <span className="text-[11px] font-mono text-gray-500 dark:text-[#888] bg-gray-100 dark:bg-[#222] px-1.5 py-0.5 rounded border border-gray-200 dark:border-[#333]">#{ticket.id.slice(0, 8).toUpperCase()}</span>
              </div>
              <h1 className="text-xl md:text-2xl font-bold leading-tight text-gray-900 dark:text-white">{ticket.subject}</h1>
            </div>
          </div>
          <div className="flex items-center gap-3 relative z-10">
            ...
            <button onClick={() => refetch()} className="p-2 bg-gray-100 dark:bg-[#222] border border-gray-200 dark:border-[#333] shadow-sm rounded-md hover:bg-gray-200 dark:bg-[#333] transition-colors text-gray-700 dark:text-[#ccc]" title="Refresh">
                    ...
                    {ticket.comments?.map((comment: SupportTicketComment) => {
                      ...
                      return (
                        <div key={comment.id} className="relative group">
                          {/* Timeline Node */}
                          <div className={`absolute -left-[30px] sm:-left-[38px] top-0 h-6 w-6 sm:h-7 sm:w-7 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold border-2 border-gray-200 dark:border-[#161616] z-10 shadow-sm ${
            ...
            <div className="rounded-xl border border-[#f97316]/30 bg-[#f97316]/5 shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Assigned To</label>
                ...
                {/* Priority */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Severity / Priority</label>
                  <select
                ...
                {/* Status */}
                <div className="space-y-1.5">
                  <label className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Ticket Status</label>
                  <select
                    value={ticket.status}
            ...
            <div className="rounded-xl border border-gray-200 dark:border-[#333] bg-gray-50 dark:bg-[#161616] shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-200 dark:divide-[#333]">
                ...
                <div className="p-4 flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Organizer</span>
                  <div className="flex flex-col gap-1">
                    <span className="text-sm font-medium text-gray-900 dark:text-white">{(ticket as any).organizer?.name || 'Unknown'}</span>
                    <span className="text-xs text-gray-500 dark:text-[#888]">{(ticket as any).organizer?.email}</span>
                  </div>
                </div>

                {/* Organizer ID */}
                <div className="p-4 flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Organizer ID</span>
                  <span className="text-xs font-mono text-gray-500 dark:text-[#888] break-all">{(ticket as any).organizer_id}</span>
                </div>
                
                {/* System ID */}
                <div className="p-4 flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">System Ticket ID</span>
                  <span className="text-xs font-mono text-gray-500 dark:text-[#888] break-all">{ticket.id}</span>
                </div>

                {/* Category */}
                <div className="p-4 flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Category</span>
                ...
                {ticket.subscription_plan_name && (
                  <div className="p-4 flex flex-col gap-1.5">
                    <span className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Related Plan</span>
                ...
                {/* Created */}
                <div className="p-4 flex flex-col gap-1.5">
                  <span className="text-[11px] font-semibold text-gray-500 dark:text-[#888] uppercase tracking-wider">Opened On</span>
                  <span className="text-sm font-medium text-gray-700 dark:text-[#ccc]">{new Date(ticket.created_at).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
                </div>
              </div>
            </div>
