...
function ConfirmDialog({ modal, onClose }: { modal: ConfirmModal; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Dialog */}
      <div className="relative z-10 bg-gray-50 dark:bg-[#1e1e1e] border border-gray-300 dark:border-[#444444] rounded-sm shadow-2xl w-full max-w-md mx-4 p-6 font-sans">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-gray-600 dark:text-[#797775] hover:text-gray-900 dark:hover:text-white transition-colors"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="flex items-center gap-3 justify-end">
          <button
            onClick={onClose}
            disabled={loading}
            className="px-4 py-2 text-sm border border-gray-200 dark:border-[#333333] text-gray-600 dark:text-[#797775] hover:text-gray-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-[#555555] rounded-sm transition-colors disabled:opacity-50"
          >
            Cancel
          </button>
          <button
      ...
      <div className="space-y-0 font-sans text-sm pb-10">
        ...
        <section className="border-b border-gray-200 dark:border-[#333333]">
          <div className="py-5">
            ...
            <div className="flex items-center justify-between bg-gray-100 dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333333] px-5 py-4">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="text-gray-900 dark:text-white font-medium">{overview?.name}</p>
                  <p className="text-gray-600 dark:text-[#797775] text-xs">{overview?.email}</p>
                </div>
                <span
        ...
        <section className="border-b border-gray-200 dark:border-[#333333]">
          <div className="py-5">
            ...
            {activeSubscription ? (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-gray-200 dark:bg-[#333333] border border-gray-200 dark:border-[#333333]">
                {[
                  {
                    label: "Plan",
                  ...
                  },
                  {
                    ...
                    subColor: "text-gray-600 dark:text-[#797775]",
                  ...
                  <div key={i} className="bg-gray-100 dark:bg-[#1a1a1a] px-5 py-4">
                    <div className="flex items-center gap-1.5 text-gray-600 dark:text-[#797775] text-xs uppercase tracking-wider mb-2">
                      {card.icon}
                      {card.label}
                    </div>
                    <p
                      className={`font-semibold text-base mb-0.5 ${card.valueClass || "text-gray-900 dark:text-white"}`}
                    >
                      {card.value}
                    </p>
              ...
              <div className="bg-gray-100 dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333333] px-5 py-6 text-center text-gray-600 dark:text-[#797775] italic text-sm">
                <Clock className="h-5 w-5 mx-auto mb-2 text-gray-500 dark:text-[#444]" />
                No active subscription.
              </div>
            )}

            {subscriptions.length > 1 && (
              <div className="mt-3 border border-gray-200 dark:border-[#333333] divide-y divide-gray-200 dark:divide-[#333333]">
                {subscriptions.slice(1, 4).map((s: any) => (
                  <div
                    key={s.id}
                    className="flex items-center justify-between bg-white dark:bg-[#111111] px-4 py-2.5 text-xs"
                  >
                    <span className="text-gray-700 dark:text-[#cccccc] font-medium">
                      {s.pricing_plan?.name || "Unknown Plan"}
                    </span>
                    <span className="text-gray-600 dark:text-[#797775]">
                      {new Date(s.start_date).toLocaleDateString("en-US")}
                    </span>
                    <span
                      className={`capitalize ${s.status === "active" ? "text-[#84c87e]" : "text-gray-600 dark:text-[#797775]"}`}
                    >
                      {s.status}
                    </span>
                  </div>
        ...
        <section className="border-b border-gray-200 dark:border-[#333333]">
          <div className="py-5">
            <div className="flex items-start gap-3 mb-4">
              <div className="h-8 w-8 rounded-sm bg-[#569cd6]/10 flex items-center justify-center shrink-0">
                <Timer className="h-4 w-4 text-[#569cd6]" />
              </div>
              <div>
                <h3 className="text-gray-900 dark:text-white font-medium text-sm">
                  Extend Free Trial
                ...
                <p className="text-gray-600 dark:text-[#797775] text-xs mt-0.5">
                  Admin-only. Push the trial expiry up to a max of 30 days from sign-up.
                </p>
              </div>
            </div>
            ...
            {trialSub && trialStart ? (
              <div className="border border-gray-200 dark:border-[#333333] bg-gray-100 dark:bg-[#1a1a1a]">
                ...
                <div className="px-5 pt-4 pb-3 border-b border-gray-200 dark:border-[#333333]">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs text-gray-600 dark:text-[#797775] flex items-center gap-1">
              ...
              <div className="bg-gray-100 dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333333] px-5 py-4 flex items-center gap-3 text-sm text-gray-600 dark:text-[#797775] italic">
                <CalendarPlus className="h-4 w-4 shrink-0 text-gray-500 dark:text-[#444]" />
                Organizer is on a paid plan — trial controls disabled.
              </div>
            )}
          </div>
        </section>
        ...
        <section className="border-b border-gray-200 dark:border-[#333333]">
          <div className="py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-px bg-gray-200 dark:bg-[#333333] border border-gray-200 dark:border-[#333333]">
                  ...
                  <button
                    ...
                    className={`text-left p-4 transition-all relative ${
                      isSelected
                        ...
                        : isActive
                          ? "bg-[#84c87e]/5"
                          : "bg-gray-100 dark:bg-[#1a1a1a] hover:bg-gray-100 dark:hover:bg-[#252526]"
                    }`}
                  >
                    {isActive && (
              ...
              <div className="mt-3 flex items-center justify-between bg-[#569cd6]/5 border border-[#569cd6]/30 px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 text-xs border border-gray-200 dark:border-[#333333] text-gray-600 dark:text-[#797775] hover:text-gray-900 dark:hover:text-white hover:border-gray-300 dark:hover:border-[#555555] rounded-sm transition-colors"
                  >
                    Clear
                  </button>
                  <button
                    onClick={promptPlanChange}
                    className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-[#569cd6] text-gray-900 dark:text-white hover:bg-[#4a8cc0] rounded-sm transition-colors font-medium"
                  >
        ...
        <section>
          <div className="py-5">
            ...
            {workspaces.length === 0 ? (
              <div className="bg-gray-100 dark:bg-[#1a1a1a] border border-gray-200 dark:border-[#333333] text-center py-8 text-gray-600 dark:text-[#797775] italic text-sm">
                No workspaces found.
              </div>
            ) : (
              <div className="border border-gray-200 dark:border-[#333333] divide-y divide-gray-200 dark:divide-[#333333]">
                {workspaces.map((ws: any, i: number) => {
                  ...
                  return (
                    <div
                      key={ws.id}
                      className={`flex items-center gap-4 px-5 py-4 ${i % 2 === 0 ? "bg-gray-100 dark:bg-[#1a1a1a]" : "bg-white dark:bg-[#111111]"}`}
                    >
                      {/* Workspace identity */}
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        {ws.logo ? (
                          <img
                            src={ws.logo}
                            alt="logo"
                            className="h-8 w-8 object-cover border border-gray-200 dark:border-[#333333] shrink-0"
                          />
                        ) : (
                          <div className="h-8 w-8 bg-gray-50 dark:bg-[#252526] border border-gray-200 dark:border-[#333333] flex items-center justify-center shrink-0">
                            <Building2 className="h-4 w-4 text-gray-600 dark:text-[#797775]" />
                          </div>
                        )}
                      ...
                      <div className="shrink-0 text-center hidden sm:block">
                        <p className="text-gray-600 dark:text-[#797775] text-xs mb-0.5">Current</p>
                        <span className="text-gray-700 dark:text-[#cccccc] font-mono font-semibold text-sm">
                          {currentCurrency}
                        </span>
                      </div>
                      ...
                      <div className="relative shrink-0">
                        <select
                          ...
                          className="appearance-none bg-gray-50 dark:bg-[#252526] border border-gray-200 dark:border-[#333333] text-gray-900 dark:text-white text-sm px-3 py-2 pr-8 focus:outline-none focus:border-[#569cd6] transition-colors cursor-pointer"
                        >
                          {CURRENCIES.map((c) => (
                        ...
                        <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-600 dark:text-[#797775] pointer-events-none" />
                      </div>
