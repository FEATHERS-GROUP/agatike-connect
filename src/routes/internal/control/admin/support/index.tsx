...
function Initials({ name, size = "h-7 w-7" }: { name: string; size?: string }) {
  ...
  return (
    <div
      className={`${size} rounded-full ${avatarColor(name)} flex items-center justify-center text-white text-[10px] font-bold shrink-0`}
    >
      {letters}
    </div>
  );
        ...
        <div className="grid grid-cols-4 gap-3 mb-4 shrink-0">
          {[
            ...
            >
              ...
              <div className="text-[11px] text-gray-500 dark:text-[#666] font-medium">
                {s.label}
              </div>
            </div>
          ))}
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          {activeTab === "documentation" ? (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {/* Category */}
                        <span
                          className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${CATEGORY_COLORS[ticket.category] || CATEGORY_COLORS.other}`}
                        >
            ...
            (() => {
              ...
              const allSelected =
                pageTickets.length > 0 && pageTickets.every((t) => selectedIds.has(t.id));

              const toggleAll = () => {
                if (allSelected) {
                  setSelectedIds((prev) => {
                    const s = new Set(prev);
                    pageTickets.forEach((t) => s.delete(t.id));
                    return s;
                  });
                } else {
                  setSelectedIds((prev) => {
                    const s = new Set(prev);
                    pageTickets.forEach((t) => s.add(t.id));
                    return s;
                  });
                }
              };
                      ...
                      <thead>
                        <tr className="bg-gray-50 dark:bg-[#161616] border-b border-gray-200 dark:border-[#2a2a2a]">
                          ...
                          {[
                            ...
                            <th
                              key={h}
                              className="px-3 py-3 text-[11px] font-semibold text-gray-500 dark:text-[#888] whitespace-nowrap tracking-wide"
                            >
                              {h}
                            </th>
                          ))}
                        ...
                        {pageTickets.length === 0 ? (
                          ...
                          pageTickets.map((ticket: any) => {
                            ...
                            const customerName =
                              ...
                              ticket.organizer?.email?.split("@")[0] ||
                              "—";
                            const assigneeName = ticket.assignedAdmin
                              ? ticket.assignedAdmin.name ||
                                ticket.assignedAdmin.email?.split("@")[0] ||
                                "Admin"
                              : null;
                              ...
                              >
                                ...
                                >
                                  <div className="flex items-center gap-1.5">
                                    {/* Flag icon */}
                                    <svg
                                      className={`h-3.5 w-3.5 shrink-0 ${pFlag.color}`}
                                      viewBox="0 0 16 16"
                                      fill="currentColor"
                                    >
                                    ...
                                    <span className={`text-[12px] font-medium ${pFlag.color}`}>
                                      {pFlag.label}
                                    </span>
                                  </div>
                                </td>
                                ...
                                >
                                  <div
                                    className={`inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded border ${statusCfg.bg} ${statusCfg.color} cursor-default`}
                                  >
                                    {statusCfg.label}
                                ...
                                >
                                  {assigneeName ? (
                                    ...
                                    <span className="text-[11px] text-gray-400 dark:text-[#555] italic">
                                      —
                                    </span>
                                  )}
                                </td>
