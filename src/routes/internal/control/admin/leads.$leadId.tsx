...
function LeadDetailsPage() {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-gray-500">
        ...
        <button
          ...
          className="mt-4 text-[#f97316] underline"
        >
          Go back to Leads
        </button>
      </div>
  ...
  const allCommunications = [...initialMessage, ...communications].sort(
      ...
      <div className="p-4 sm:p-6 w-full space-y-6">
        ...
        <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="px-6 py-4 border-t border-b border-gray-100 dark:border-[#222] grid grid-cols-2 md:grid-cols-6 gap-6 bg-gray-50/50 dark:bg-[#161616]/50">
            <div>
              <div className="text-[11px] font-medium text-gray-500 dark:text-[#888] uppercase tracking-wider mb-1">
                Company
              </div>
              <div className="text-sm font-medium text-gray-900 dark:text-[#eee]">
                {lead.company || "—"}
              </div>
            </div>
            <div>
              ...
              <div
                className="text-sm font-medium text-gray-900 dark:text-[#eee] truncate max-w-[150px]"
                title={lead.email}
              >
                {lead.email}
              </div>
            ...
            <div className="col-span-2">
              ...
              <div className="flex gap-2 flex-wrap items-center h-6">
                {tags.map((tag: string, i: number) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 bg-[#f97316]/10 border border-[#f97316]/20 text-[#f97316] font-semibold text-[10px] uppercase rounded-md flex items-center gap-1"
                  >
                    {tag}
                    <button
                ...
                {STATUS_STAGES.map((s, idx) => {
                  ...
                  return (
                    ...
                    >
                      {(isActive || isPassed) && (
                        <CheckCircle2
                          className={`h-3.5 w-3.5 mr-1.5 ${isActive ? "text-white" : "text-[#f97316]"}`}
                        />
                      )}
                      {getStatusLabel(s)}
                    </button>
                        ...
                        <React.Suspense
                          fallback={
                            <div className="h-[120px] w-full flex items-center justify-center">
                              <Loader2 className="animate-spin text-[#f97316] h-6 w-6" />
                            </div>
                          }
                        >
                          <ReactQuill
                      ...
                      {attachments.length > 0 && (
                        <div className="flex flex-wrap gap-2 px-5 pb-3">
                          {attachments.map((file, i) => (
                            <div
                              key={i}
                              className="flex items-center gap-2 bg-gray-100 dark:bg-[#222] px-3 py-1.5 rounded-full text-xs text-gray-700 dark:text-[#aaa]"
                            >
                ...
                <div className="space-y-4">
                  {allCommunications.length === 0 ? (
                    ...
                    allCommunications.map((msg: any) => (
                      <div
                        key={msg.id}
                        className="bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded-xl overflow-hidden shadow-sm"
                      >
                        <div
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="h-8 w-8 rounded-full bg-gray-200 dark:bg-[#333] overflow-hidden flex items-center justify-center shrink-0">
                              {msg.type === "sent" ? (
                                <img
                                  src={`https://api.dicebear.com/7.x/shapes/svg?seed=AgatikeTeam`}
                                  alt="Team"
                                  className="h-full w-full object-cover"
                                />
                              ) : (
                                <img
                                  src={`https://api.dicebear.com/7.x/shapes/svg?seed=${encodeURIComponent(lead.email)}&backgroundColor=10b981`}
                                  alt={lead.name}
                                  className="h-full w-full object-cover"
                                />
                              )}
                        ...
                        <div className="p-5">
                          {msg.subject && (
                            <div className="text-sm font-bold text-gray-900 dark:text-white mb-3 pb-3 border-b border-gray-100 dark:border-[#222]">
                              {msg.subject}
                            </div>
                          )}
                          <div
            ...
            {activeTab === "notes" && (
              <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded-xl p-6 shadow-sm min-h-[400px]">
                ...
                {updateProfileMutation.isPending && (
                  <div className="text-xs text-[#f97316] mt-2 flex items-center gap-1">
            ...
            <div className="bg-white dark:bg-[#111] border border-gray-200 dark:border-[#333] rounded-xl overflow-hidden shadow-sm">
              ...
              {isEditingDetails ? (
                <form
                  ...
                  className="p-5 space-y-3 bg-gray-50/30 dark:bg-[#161616]/30"
                >
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, email: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, phone: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, company: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, job_title: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={editDetailsForm.country}
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, country: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={editDetailsForm.language}
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, language: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      value={editDetailsForm.cc_emails}
                      onChange={(e) =>
                        setEditDetailsForm({ ...editDetailsForm, cc_emails: e.target.value })
                      }
                      className="w-full border border-gray-200 dark:border-[#333] bg-white dark:bg-[#111] rounded px-2 py-1.5 text-xs text-gray-900 dark:text-white outline-none"
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <button
                      ...
                      className="px-3 py-1.5 text-xs text-gray-600 dark:text-[#aaa] hover:bg-gray-200 dark:hover:bg-[#333] rounded"
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      disabled={updateProfileMutation.isPending}
                      className="flex items-center gap-2 px-3 py-1.5 text-xs bg-[#f97316] text-white rounded hover:bg-[#ea580c]"
                    >
                      {updateProfileMutation.isPending ? (
                ...
                <div className="p-5 space-y-4">
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-[#222] border-dashed">
                    ...
                    <span className="text-sm font-medium text-gray-900 dark:text-[#eee]">
                      {lead.email}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-[#222] border-dashed">
                ...
                <button className="flex items-center justify-between p-3 hover:bg-gray-50 dark:hover:bg-[#1a1a1a] rounded-lg transition-colors text-left group">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-500 dark:text-[#888]">
                        Add to automated sales follow-ups
                      </div>
                    </div>
                  </div>
