...
import {
  ...
  updatePaymentProviderFeeAdmin,
  createPaymentProviderFeeAdmin,
...
function AdminProviderFeesPage() {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: { id: string; updates: Record<string, any> }) =>
      updatePaymentProviderFeeAdmin({ data }),
    onSuccess: () => {
  ...
  const uniqueCountries = useMemo(
    () => Array.from(new Set(fees.map((f: any) => f.country_code).filter(Boolean))).sort(),
    [fees],
  );
  const uniqueCategories = useMemo(
    () => Array.from(new Set(fees.map((f: any) => f.category).filter(Boolean))).sort(),
    [fees],
  );
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-xs text-muted-foreground flex items-center gap-2 mt-0.5">
                      <span className="font-mono bg-gray-200 dark:bg-[#333333] text-gray-700 dark:text-[#cccccc] px-1.5 rounded">
                        {fee.country_code}
                      </span>
                      {fee.category && (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex flex-col gap-1 text-xs">
                      <div className="flex justify-between w-32">
                        <span className="text-muted-foreground">Percentage:</span>
                        <span className="text-gray-900 dark:text-white font-medium">
                          {fee.disbursement_percentage}%
                        </span>
                      </div>
                      <div className="flex justify-between w-32">
                        <span className="text-muted-foreground">Fixed Fee:</span>
                        <span className="text-gray-900 dark:text-white font-medium">
                          {fee.disbursement_fixed_fee}
                        </span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-2 py-1 rounded-full text-[10px] font-medium uppercase tracking-wider ${fee.is_tiered ? "bg-blue-500/10 text-blue-400 border border-blue-500/20" : "bg-gray-200 dark:bg-[#333333] text-muted-foreground border border-gray-300 dark:border-[#444]"}`}
                    >
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-4">
          <div className="text-sm text-muted-foreground">
            ...
            {Math.min(currentPage * ITEMS_PER_PAGE, filteredFees.length)} of {filteredFees.length}{" "}
            entries
          </div>
          ...
          <div className="bg-gray-50 dark:bg-[#1b1b1c] border border-gray-200 dark:border-[#333333] rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-[#333333] bg-white dark:bg-[#111111]">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">
                Provider Details: {editingProvider.network}
              </h2>
              <button
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setFormState({ ...formState, network: e.target.value })}
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    onChange={(e) => setFormState({ ...formState, country_code: e.target.value })}
                    className="w-full bg-white dark:bg-[#111111] border border-gray-200 dark:border-[#333333] rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white"
                  >
                    {COUNTRIES.map((c) => (
                ...
                <div className="space-y-1.5 col-span-2">
                  ...
                  <select
                    value={formState.category || "mobile"}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={formState.collection_percentage}
                      onChange={(e) =>
                        setFormState({ ...formState, collection_percentage: e.target.value })
                      }
                      className="w-full bg-gray-50 dark:bg-[#1b1b1c] border border-gray-200 dark:border-[#333333] rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={formState.collection_fixed_fee}
                      onChange={(e) =>
                        setFormState({ ...formState, collection_fixed_fee: e.target.value })
                      }
                      className="w-full bg-gray-50 dark:bg-[#1b1b1c] border border-gray-200 dark:border-[#333333] rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white"
                    />
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={formState.disbursement_percentage}
                      onChange={(e) =>
                        setFormState({ ...formState, disbursement_percentage: e.target.value })
                      }
                      className="w-full bg-gray-50 dark:bg-[#1b1b1c] border border-gray-200 dark:border-[#333333] rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input
                      type="number"
                      step="0.01"
                      value={formState.disbursement_fixed_fee}
                      onChange={(e) =>
                        setFormState({ ...formState, disbursement_fixed_fee: e.target.value })
                      }
                      className="w-full bg-gray-50 dark:bg-[#1b1b1c] border border-gray-200 dark:border-[#333333] rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-white"
                    />
                  </div>
