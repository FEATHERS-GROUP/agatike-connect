...
export const Route = createFileRoute("/internal/control/admin/organizers/$organizerId/projects")({
  loader: async ({ params }) => {
    const data = await getAdminOrganizerProjects({
...
function OrganizerProjects() {
  ...
  const filteredBadges = (badges || []).filter(
    (b: any) =>
      (b.logo_text || "").toLowerCase().includes(q) ||
                  ...
                  filteredTickets.map((t: any) => (
                    <tr key={t.id} className="hover:bg-gray-200 dark:hover:bg-[#2d2d30] transition-colors">
                      ...
                      <td className="py-2 px-4">
                        <WorkspaceCell name={t.workspaceName} />
                      </td>
                      <td className="py-2 px-4 capitalize text-[#dcdcaa]">{t.template || "—"}</td>
                      <td className="py-2 px-4">
                        {t.deleted ? (
                          <span className="text-xs px-2 py-0.5 rounded-sm bg-[#f43f5e]/10 text-[#f43f5e]">
                            Deleted
                          </span>
                        ) : (
                      ...
                      <td className="py-2 px-4">
                        {b.accent_color ? (
                          <span className="flex items-center gap-1.5">
                            <span
                              className="inline-block h-3 w-3 rounded-full border border-gray-300 dark:border-[#444]"
                              style={{ backgroundColor: b.accent_color }}
                            />
                            <span className="text-xs font-mono text-gray-600 dark:text-[#797775]">
                              {b.accent_color}
                            </span>
                          </span>
                        ) : (
                          <span className="text-gray-600 dark:text-[#797775]">—</span>
                        )}
                      </td>
                    </tr>
                  ))
                ...
                {filteredPages.length === 0 ? (
                  ...
                  filteredPages.map((p: any) => (
                    <tr key={p.id} className="hover:bg-gray-200 dark:hover:bg-[#2d2d30] transition-colors">
                      ...
                      <td className="py-2 px-4">
                        <WorkspaceCell name={p.workspaceName} />
                      </td>
                      <td className="py-2 px-4">
                        {p.slug ? (
                          <span className="flex items-center gap-1.5 font-mono text-xs text-[#569cd6]">
                            /{p.slug}
                            <ExternalLink className="h-3 w-3 text-gray-600 dark:text-[#797775]" />
                          </span>
                        ) : (
                          <span className="text-gray-600 dark:text-[#797775]">—</span>
                        )}
                      ...
                      <td className="py-2 px-4">
                        {p.is_published ? (
                          <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-sm bg-[#84c87e]/10 text-[#84c87e]">
                            <Globe className="h-3 w-3" /> Published
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-sm bg-gray-200 dark:bg-[#797775]/10 text-gray-600 dark:text-[#797775]">
