        ...
        <div className="space-y-5 opacity-80 hover:opacity-100 transition-opacity">
          {notifications.slice(2).map((n) => {
            ...
            return (
              <div
                key={n.id}
                className="flex items-start gap-4 p-3 rounded-2xl bg-card/50 border border-border/40 transition-all active:scale-[0.98]"
              >
                <div className="relative shrink-0">
                  <div className="h-14 w-14 rounded-xl overflow-hidden bg-secondary">
                    <img
                  ...
                  {n.type === "rating" ? (
                    <div className="flex gap-1 mb-1">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className="h-5 w-5 text-muted-foreground/30 hover:text-amber-500 hover:fill-amber-500 cursor-pointer transition-colors"
                        />
                      ))}
                    </div>
                  ) : (
