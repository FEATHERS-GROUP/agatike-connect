...
function PostCommunityPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-24 md:pb-8 md:max-w-xl md:mx-auto shadow-xl relative">
      {/* Header */}
      <div className={`flex items-center justify-between px-4 pb-3 pt-safe-top fixed top-0 left-0 right-0 mx-auto w-full md:max-w-xl z-40 transition-colors duration-300 min-h-[60px] ${scrolled ? 'bg-background/90 backdrop-blur-md border-b border-border/40' : 'bg-gradient-to-b from-black/60 to-transparent border-b border-transparent'}`}>
        <div className="flex items-center gap-3">
          <button onClick={() => router.history.back()} className={`p-2 -ml-2 rounded-full transition-colors ${scrolled ? 'text-foreground hover:bg-secondary' : 'text-white hover:bg-white/20 backdrop-blur-sm'}`}>
            <ArrowLeft className="h-6 w-6" />
          </button>
          {scrolled ? (
            <div className="flex items-center gap-2 animate-in fade-in duration-300">
              <img src={organizerEvent.cover} className="w-8 h-8 rounded-full object-cover border border-border" />
              <div className="flex flex-col">
                <span className="font-bold text-sm leading-none">{organizerEvent.organizer}</span>
                <span className="text-[10px] text-muted-foreground">@{organizerEvent.organizerHandle}</span>
              </div>
            </div>
          ) : (
        ...
        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="relative">
              <img src={organizerEvent.cover} alt={organizerEvent.organizer} className="w-12 h-12 rounded-full border-2 border-background object-cover" />
            ...
            <div>
              ...
              <p className="text-xs text-white/80 drop-shadow-sm">@{organizerEvent.organizerHandle}</p>
            </div>
          </div>
          <button 
            onClick={() => setIsFollowing(!isFollowing)}
      ...
      <div className="w-full pt-4">
        <h3 className="px-4 text-sm font-bold text-muted-foreground mb-4 flex items-center gap-2">
          Community Conversations <span className="bg-secondary text-foreground px-2 py-0.5 rounded-full text-[10px]">{post.comments}</span>
        </h3>
        
        <div className="px-4 space-y-5">
          {post.commentsList && post.commentsList.map((comment) => (
            <div key={comment.id} className="flex gap-3">
              <img src={comment.avatar} alt={comment.handle} className="w-8 h-8 rounded-full object-cover shrink-0" />
      ...
      <div className="fixed bottom-0 left-0 right-0 md:max-w-xl md:mx-auto bg-background/90 backdrop-blur-md border-t border-border/40 p-4 pb-safe-bottom z-40">
        {isFollowing ? (
          <div className="flex gap-3 items-center">
            <img src="https://i.pravatar.cc/150?u=me" alt="You" className="w-8 h-8 rounded-full object-cover shrink-0" />
          ...
          <div className="flex flex-col items-center justify-center py-2">
            <p className="text-sm text-muted-foreground font-medium mb-2">You must follow the organizer to join the conversation</p>
            <button 
