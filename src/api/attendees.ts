...
export const addEventAttendees = createServerFn({ method: "POST" })
  ...
  .handler(async (ctx) => {
    ...
    `;
      ...
      const updates: { id: string; new_sold: number; new_remaining: number }[] = [];
      for (const tid of ticketIds) {
        ...
        const currentRemaining = parseInt(dbTier.remaining) || 0;
        const qty = qtyByTier[tid];
    ...
    `;
      ...
      updates.forEach((u, i) => {
        ...
        update_${i}: update_event_tickets_by_pk(
          pk_columns: { id: "${u.id}" },
          _set: { sold: "${u.new_sold}", remaining: "${u.new_remaining}" }
        ) {
          id
        }
      `;
      ...
      if (totalCost > 0 && eventId) {
        ...
        `;
          const wsData = await hasuraRequest<{ events_by_pk: { workspace_id: string } }>(
            GET_EVENT_WORKSPACE,
            { id: eventId },
          );
