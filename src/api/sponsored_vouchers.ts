...
export async function processSponsoredVouchersForAttendees(attendees: any[]) {
  ...
  const workspaceIds = [...new Set(dbTickets.map(t => t.workspace_id).filter(Boolean))];

  const GET_BATCHES = `
    query GetTicketLinkedBatches($events: [uuid!], $workspaces: [uuid!]) {
      sponsored_voucher_batches(
        where: {
          generation_type: { _eq: "ticket_linked" },
          is_active: { _eq: true },
          _or: [
            { event_id: { _in: $events } },
  ...
  if (vouchersToInsert.length > 0) {
    ...
    try {
      ...
      console.log(`[Vouchers] Successfully generated ${vouchersToInsert.length} sponsored vouchers for attendees.`);
    } catch (e) {
