...
export const updatePricingPlanAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async (ctx) => {
    ...
    if (typeof formattedUpdates.features === "string") {
      try {
        formattedUpdates.features = JSON.parse(formattedUpdates.features);
      } catch (e) {}
    }
    if (typeof formattedUpdates.modules_included === "string") {
      try {
        formattedUpdates.modules_included = JSON.parse(formattedUpdates.modules_included);
      } catch (e) {}
    }
    if (typeof formattedUpdates.usage_limits === "string") {
      try {
        formattedUpdates.usage_limits = JSON.parse(formattedUpdates.usage_limits);
      } catch (e) {}
    }
...
export const createPricingPlanAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async (ctx) => {
    ...
    if (typeof formattedData.features === "string") {
      try {
        formattedData.features = JSON.parse(formattedData.features);
      } catch (e) {}
    }
    if (typeof formattedData.modules_included === "string") {
      try {
        formattedData.modules_included = JSON.parse(formattedData.modules_included);
      } catch (e) {}
    }
    if (typeof formattedData.usage_limits === "string") {
      try {
        formattedData.usage_limits = JSON.parse(formattedData.usage_limits);
      } catch (e) {}
    }
