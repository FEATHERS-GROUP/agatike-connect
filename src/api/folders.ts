...
export const getWorkspaceFolders = createServerFn({ method: "POST" }).handler(async (ctx) => {
  const { workspace_id, module_type } = (ctx as any).data as {
    workspace_id: string;
    module_type: string;
  };
  const query = `
  ...
  `;
  const res = await hasuraRequest<{ workspace_folders: any[] }>(query, {
    workspace_id,
    module_type,
  });
...
export const createWorkspaceFolder = createServerFn({ method: "POST" }).handler(async (ctx) => {
  const { workspace_id, name, module_type } = (ctx as any).data as {
    workspace_id: string;
    name: string;
    module_type: string;
  };
  ...
  `;
  const res = await hasuraRequest<{ insert_workspace_folders_one: any }>(query, {
    workspace_id,
    name,
    module_type,
  });
