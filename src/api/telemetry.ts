...
export const recordHeartbeat = createServerFn({ method: "POST" })
  .validator((d: { sessionId: string; path: string; userAgent: string; visibilityState: string }) => d)
  .handler(async (ctx) => {
    ...
    try {
      ...
      await db.runTransaction(async (t: any) => {
        ...
        if (!doc.exists) {
          ...
          const secondsElapsed = Math.floor((currentDate.getTime() - lastActiveDate.getTime()) / 1000);
          ...
          const addDuration = (secondsElapsed > 0 && secondsElapsed < 300 && visibilityState === "visible") 
            ? secondsElapsed 
            : 0;
...
export const getTelemetryStats = createServerFn({ method: "POST" })
  .handler(async () => {
    try {
      const { getFirebaseAdmin } = await import("@/lib/firebase.server");
      const { db } = getFirebaseAdmin();
      ...
      const snapshot = await db.collection("platform_telemetry")
      ...
      const avgDurationSeconds = sessions.length > 0 ? Math.floor(totalDuration / sessions.length) : 0;
      ...
      const chart = Object.keys(chartData).map(key => ({
        time: key,
        users: chartData[key]
      ...
      const topPaths = Object.keys(pathCount).map(key => ({
        path: key,
      ...
      return { activeNow: 0, totalToday: 0, avgDurationSeconds: 0, chart: [], userTypes: [], topPaths: [], recentSessions: [] };
    }
  });
