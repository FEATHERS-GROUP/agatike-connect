    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-6 items-start">
        ...
        <div className="space-y-6 print:hidden sticky top-6">
          <div className="bg-card border border-border/60 rounded-3xl p-5 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(["quote", "proforma", "invoice", "receipt", "purchase_order", "credit_note"] as InvoiceType[]).map((t) => (
                  <button
                    key={t}
                    onClick={() => setInvoiceType(t)}
                    className={cn(
                      "h-10 rounded-xl font-semibold text-xs capitalize transition-all border",
                      invoiceType === t
          ...
          <div className="bg-card border border-border/60 rounded-3xl p-5 space-y-4">
             <h3 className="font-bold text-base border-b border-border/60 pb-2">Client Details</h3>
             <div className="space-y-3">
                <div className="space-y-1">
                   <Label className="text-xs">Name *</Label>
                   <Input value={client.name} onChange={(e) => setClient({ ...client, name: e.target.value })} className="h-9 rounded-lg text-xs" placeholder="Client Name" />
                </div>
                <div className="space-y-1">
                   <Label className="text-xs">Email</Label>
                   <Input value={client.email} onChange={(e) => setClient({ ...client, email: e.target.value })} className="h-9 rounded-lg text-xs" placeholder="Email Address" />
                </div>
                <div className="space-y-1">
                   <Label className="text-xs">Company</Label>
                   <Input value={client.company} onChange={(e) => setClient({ ...client, company: e.target.value })} className="h-9 rounded-lg text-xs" placeholder="Company Name" />
                </div>
                <div className="space-y-1">
                   <Label className="text-xs">Address</Label>
                   <Input value={client.address} onChange={(e) => setClient({ ...client, address: e.target.value })} className="h-9 rounded-lg text-xs" placeholder="Billing Address" />
                </div>
             </div>
          </div>
          ...
          <div className="bg-card border border-border/60 rounded-3xl p-5 space-y-4">
             ...
             <div className="space-y-3">
                <div className="space-y-1">
                   ...
                   <Input type="number" value={taxRate} onChange={(e) => setTaxRate(Number(e.target.value))} className="h-9 rounded-lg text-xs" min={0} max={100} />
                </div>
             </div>
          </div>
          ...
          <div className="bg-white text-slate-900 w-[210mm] min-h-[297mm] shadow-2xl p-[20mm] flex flex-col relative print:shadow-none print:w-full print:h-full print:p-0">
            ...
            <div className="flex justify-between items-start mb-12">
              <div className="space-y-2">
                <h1 className="text-4xl font-black uppercase tracking-widest text-slate-800 focus-within:ring-2 ring-primary rounded px-2 -ml-2">
                  {invoiceType === "quote" ? "Quote" : 
                   invoiceType === "proforma" ? "Proforma" : 
                   invoiceType === "purchase_order" ? "Purchase Order" : 
                 ...
                 {logoUrl ? (
                    <>
                       ...
                       <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100">
                          <Button variant="destructive" size="icon" className="h-6 w-6 rounded-full" onClick={() => setLogoUrl("")}>
                    ...
                    <div className="flex flex-col items-center justify-center p-4 text-slate-400">
                       {isUploading["logo"] ? <Loader2 className="w-5 h-5 animate-spin" /> : <Upload className="w-5 h-5 mb-1 text-slate-300" />}
                       <span className="text-xs font-medium">Upload Logo (5MB)</span>
                    </div>
                 )}
                 {!logoUrl && <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handleUpload(e, "logo", setLogoUrl)} />}
              </div>
            </div>
               ...
               <div className="flex flex-col items-end text-right">
                  <div className="space-y-3 w-full max-w-[200px]">
                     <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                        ...
                        <input type="date" value={issueDate} onChange={(e) => setIssueDate(e.target.value)} className="bg-transparent border-0 text-sm font-medium text-right focus:ring-0 p-0" />
                     </div>
                     <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                        <span className="text-xs font-bold uppercase text-slate-400">Due Date</span>
                        <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="bg-transparent border-0 text-sm font-medium text-right focus:ring-0 p-0" />
                     </div>
                  </div>
               </div>
            </div>
                  ...
                  {items.map((item, idx) => (
                     <div key={idx} className="grid grid-cols-[1fr_80px_120px_120px_40px] gap-4 items-center group">
                        ...
                        <div className="flex justify-end opacity-0 group-hover:opacity-100 transition-opacity print:hidden">
                           <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-destructive" onClick={() => removeItem(idx)} disabled={items.length === 1}>
            ...
            <div className="grid grid-cols-2 gap-12 mt-auto">
               <div className="space-y-6">
                  ...
                  <div>
                     <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Payment Terms</h3>
                     <textarea
                        rows={2}
                        value={paymentTerms}
            ...
            <div className="grid grid-cols-2 gap-12 mt-12 pt-8 border-t border-slate-100">
                  ...
                  <div className="w-full max-w-[200px] h-24 mx-auto flex flex-col items-center justify-center border-b-2 border-dashed border-slate-300 relative group hover:bg-slate-50 transition-colors cursor-pointer rounded-t-lg">
                     {signatureUrl ? (
                        <>
                           ...
                           <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100">
                              <Button variant="destructive" size="icon" className="h-6 w-6 rounded-full" onClick={() => setSignatureUrl("")}>
                                 <Trash2 className="h-3 w-3" />
                              </Button>
                           </div>
                        </>
                        ...
                        <div className="flex flex-col items-center justify-center p-4 text-slate-400">
                           {isUploading["signature"] ? <Loader2 className="w-5 h-5 animate-spin" /> : <ImageIcon className="w-5 h-5 mb-1 text-slate-300" />}
                           <span className="text-xs font-medium text-center">Upload Signature<br/>(Optional)</span>
                        </div>
                     )}
                     {!signatureUrl && <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handleUpload(e, "signature", setSignatureUrl)} />}
                  </div>
               </div>
               ...
               <div>
                  ...
                  <div className="w-24 h-24 mx-auto flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-full relative group hover:bg-slate-50 transition-colors cursor-pointer">
                     {stampUrl ? (
                        <>
                           <img src={stampUrl} alt="Stamp" className="w-full h-full object-contain p-2 rounded-full" />
                           <div className="absolute -top-2 -right-2 opacity-0 group-hover:opacity-100">
                              <Button variant="destructive" size="icon" className="h-6 w-6 rounded-full shadow-lg" onClick={() => setStampUrl("")}>
                                 <Trash2 className="h-3 w-3" />
                              </Button>
                           </div>
                        </>
                        ...
                        <div className="flex flex-col items-center justify-center p-2 text-slate-400">
                           {isUploading["stamp"] ? <Loader2 className="w-5 h-5 animate-spin" /> : <ImageIcon className="w-5 h-5 mb-1 text-slate-300" />}
                           <span className="text-[10px] font-medium text-center leading-tight">Add Stamp</span>
                        </div>
                     )}
                     {!stampUrl && <input type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer rounded-full" onChange={(e) => handleUpload(e, "stamp", setStampUrl)} />}
                  </div>
               </div>
            </div>
