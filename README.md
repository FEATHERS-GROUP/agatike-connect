...
- **Heartbeat Mechanism:** The platform utilizes a frontend telemetry hook (`useTelemetry.ts`) that generates a unique session ID for the user's browser tab. It periodically sends "heartbeats" to the backend (`recordHeartbeat` in `telemetry.ts`), capturing the active path, user agent, visibility state, and session duration.
...
- **Analytics Dashboard:** Admins can view this telemetry data on the Moderation Dashboard (`/internal/control/admin/moderation`). It parses active heartbeats, categorizes anonymous vs. registered users, highlights top user paths, and visualizes hourly usage trends.
