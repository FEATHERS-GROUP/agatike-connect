...
desktop = desktop.replace(/\{!isSubdomain && <Navbar \/>\}/, "{!hideLayout && !isSubdomain && <Navbar />}");
desktop = desktop.replace(/\{!isSubdomain && <Footer \/>\}/, "{!hideLayout && !isSubdomain && <Footer />}");
...
mobile = mobile.replace(/\{!isSubdomain && \(\n[ \t]*<div\n[ \t]*className=\{`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 transition-all duration-300 \$\{/g, "{!hideLayout && !isSubdomain && (\n        <div\n          className={`fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-4 py-3 transition-all duration-300 ${");
...
route = route.replace(/<VenueDetailsMobile venue=\{venue\} \/>/, "<VenueDetailsMobile venue={venue} hideLayout={!!subdomainSlug} />");
route = route.replace(/<VenueDetailsDesktop venue=\{venue\} \/>/, "<VenueDetailsDesktop venue={venue} hideLayout={!!subdomainSlug} />");
