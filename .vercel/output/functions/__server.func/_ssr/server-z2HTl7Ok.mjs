...
import {
  j as defineHandlerCallback,
  I as resolveManifestAssetLink,
  J as resolveManifestCssLink,
  p as getManifestScriptFormat,
  K as rootRouteId,
  ...
  F as mergeHeaders,
  m as executeRewriteInput,
  i as defaultSerovalPlugins,
  E as makeSerovalPlugin,
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-CWfzzk10.mjs");
...
function appendUniqueManifestAssetLink(target, link) {
  ...
  if (target) {
    for (const item of target)
      if ((typeof item === "string" ? item : item.href) === href) return target;
  }
  return [...(target ?? []), link];
}
function addClientEntryToManifest(manifest2, clientEntry) {
  ...
  const scripts = rootScripts.some((script) => script.attrs?.src === clientEntry)
    ? rootScripts
    : [...rootScripts, buildClientEntryScriptTag(clientEntry, getManifestScriptFormat(manifest2))];
  manifest2.routes = {
    ...manifest2.routes,
    __root__: {
      ...rootRoute,
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./router-EgqkzaPB.mjs").then((n) => n.r),
    import("./start-CZevyPw5.mjs"),
