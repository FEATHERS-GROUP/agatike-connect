...
let content = fs.readFileSync('src/routes/dashboard/$workspaceSlug/users/$userId.edit.tsx', 'utf-8');
...
// 7. Remove Step 6 UI
content = content.replace(/\{\/\* ========== STEP 6: Security ========== \*\/\}([\s\S]*?)STEP 7: Review & Confirm/, 'STEP 7: Review & Confirm');

// 8. Rename Step 7 to Step 6
content = content.replace(/STEP 7: Review & Confirm ========== \*\/\}\s*\{step === 7 && \(/m, 'STEP 6: Review & Confirm ========== */}\n        {step === 6 && (');

// 9. Remove password success box in Review
content = content.replace(/<div className="p-4 bg-amber-500\/10 border border-amber-500\/20 rounded-2xl flex gap-3">([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>\s*\)}/m, '</div>\n            </div>\n          </div>\n        )}');

// 10. Fix text "Add User" and "Create User"
content = content.replace(/<h2 className="text-xl font-bold">Add New User<\/h2>/g, '<h2 className="text-xl font-bold">Edit User</h2>');
content = content.replace(/<h1 className="text-sm font-bold">Add User<\/h1>/g, '<h1 className="text-sm font-bold">Edit User</h1>');
content = content.replace(/Double-check the details before creating the user account\./g, 'Double-check the details before updating the user account.');
