...
function EditUserPage() {
  ...
  useEffect(() => {
    if (existingUser) {
      ...
      setSelectedWorkspaces(existingUser.workspaces?.includes("ALL") ? [] : (existingUser.workspaces || []));
  ...
  function getRoutesForModule(module: any) {
    ...
    return [{ id: `${module.id}:view`, label: `View ${module.label}`, path: `/${module.href || module.id}` }];
  }

  const activeModulesIds = activeWorkspace?.modules || [];

  const legacyIdMap: Record<string, string> = {
    Dashboard: "dashboard", Events: "events", Tickets: "tickets", RSVPs: "rsvps",
    Attendees: "rsvps", Scanning: "scanner", "Products & Add-ons": "products&add-ons",
    Merchandise: "merchandise", "VIP Access": "vip", Campaigns: "campaigns",
    "Venue Listings": "venue_listings", "Venue Designer": "venue_designer",
    Experiences: "experiences", Analytics: "analytics", Users: "users",
  ...
  const handleSubmit = () => {
    ...
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { toast.error("A valid email is required"); return; }
    if (!isAllWorkspaces && selectedWorkspaces.length === 0) { toast.error("Please select at least one workspace"); return; }
    if (!isAllRoutes && selectedRoutes.length === 0) { toast.error("Please select at least one route access"); return; }
    if (isTemporary && !expiresAt) { toast.error("Please set an expiration date for temporary access"); return; }
  ...
  if (!existingUser) {
    return (
      <div className="flex flex-col h-screen w-full items-center justify-center space-y-4">
        ...
        <Button onClick={() => navigate({ to: `/dashboard/${workspaceSlug}/users` })}>Back to Users</Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 w-full max-w-4xl mx-auto pb-32">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-1">Modify access and permissions for {existingUser.name}</p>
      </div>
      ...
      <div className="space-y-12">
        ...
        <section className="space-y-6">
          ...
          <div className="flex flex-col items-center gap-4 p-8 rounded-3xl border border-border/60 bg-card">
            ...
            <div className="flex gap-2">
              <Button type="button" variant="outline" size="sm" className="rounded-xl gap-2" onClick={() => setIsAvatarPickerOpen(true)}>
              ...
              {image && (
                <Button type="button" variant="ghost" size="sm" className="rounded-xl gap-2 text-muted-foreground" onClick={() => setImage("")}>
          ...
          <div className="rounded-3xl border border-border/60 bg-card p-6 md:p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input id="user-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Jane Smith" className="h-11 rounded-xl bg-secondary/50" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="user-email">Email Address *</Label>
                <Input id="user-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="jane@example.com" className="h-11 rounded-xl bg-secondary/50" />
              </div>
                ...
                <Select value={role} onValueChange={setRole}>
                  ...
                  <SelectContent>
                    <SelectItem value="user">
                      <span className="flex items-center gap-2"><User className="h-4 w-4 text-muted-foreground" /> Normal User — Standard workspace access</span>
                    </SelectItem>
                    <SelectItem value="contributor">
                      <span className="flex items-center gap-2"><Pencil className="h-4 w-4 text-muted-foreground" /> Contributor — Can create and edit content</span>
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
        ...
        <section className="space-y-6">
          ...
          <div className="rounded-3xl border border-border/60 bg-card p-6 md:p-8 space-y-4">
            <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${isAllWorkspaces ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => setIsAllWorkspaces(true)}>
              <div className="flex items-center justify-between">
                <div><p className="font-semibold">All Workspaces</p><p className="text-sm text-muted-foreground">Access to every workspace you own</p></div>
                {isAllWorkspaces && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </div>
            </div>
            <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${!isAllWorkspaces ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => setIsAllWorkspaces(false)}>
              <div className="flex items-center justify-between">
                <div><p className="font-semibold">Specific Workspaces</p><p className="text-sm text-muted-foreground">Limit access to selected workspaces only</p></div>
                {!isAllWorkspaces && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </div>
            </div>
            {!isAllWorkspaces && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 pl-2">
                {workspaces.length === 0 ? <p className="text-sm text-muted-foreground col-span-2">No workspaces available.</p> : workspaces.map((ws: any) => (
                  <div key={ws.id} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedWorkspaces.includes(ws.id) ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => toggleWorkspace(ws.id)}>
                    <div className="h-9 w-9 rounded-xl bg-secondary flex items-center justify-center shrink-0 overflow-hidden">
                      {ws.logo?.startsWith("http") || ws.logo?.startsWith("data:") ? <img src={ws.logo} alt={ws.name} className="w-full h-full object-cover" /> : <Building2 className="h-4 w-4 text-muted-foreground" />}
                    </div>
                    <span className="font-medium text-sm">{ws.name}</span>
                    {selectedWorkspaces.includes(ws.id) && <Check className="h-4 w-4 text-primary ml-auto" />}
                  </div>
                ))}
              </div>
            )}
        ...
        <section className="space-y-6">
          ...
          <div className="rounded-3xl border border-border/60 bg-card p-6 md:p-8 space-y-4">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs text-muted-foreground bg-secondary px-2 py-1 rounded-full">{selectedModules.length} selected</span>
              <Button variant="ghost" size="sm" onClick={() => setSelectedModules(availableModules.map((m) => m.id))} className="text-xs h-7 rounded-md text-primary">Select All</Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {availableModules.map((mod) => {
                ...
                const Icon = mod.icon;
                return (
                  <button key={mod.id} type="button" onClick={() => toggleModule(mod.id)} className={`flex items-center gap-3 p-4 rounded-2xl border-2 text-left transition-all ${isSelected ? "border-primary bg-primary/5" : "border-border/60 hover:border-border bg-card"}`}>
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${isSelected ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>
                    ...
                    <div className={`h-5 w-5 rounded-md border-2 flex items-center justify-center shrink-0 ${isSelected ? "bg-primary border-primary" : "border-border"}`}>
                      {isSelected && <Check className="h-3 w-3 text-primary-foreground" />}
                    </div>
                  </button>
                );
              })}
              {availableModules.length === 0 && <p className="col-span-2 text-center text-sm text-muted-foreground py-10">No modules available in the current workspace.</p>}
            </div>
          </div>
        </section>
        ...
        <section className="space-y-6">
          ...
          <div className="rounded-3xl border border-border/60 bg-card p-6 md:p-8 space-y-6">
            <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${isAllRoutes ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => setIsAllRoutes(true)}>
              <div className="flex items-center justify-between">
                <div><p className="font-semibold">All Routes</p><p className="text-sm text-muted-foreground">Access to all features inside assigned modules</p></div>
                {isAllRoutes && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </div>
            </div>
            <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${!isAllRoutes ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => setIsAllRoutes(false)}>
              <div className="flex items-center justify-between">
                <div><p className="font-semibold">Specific Routes</p><p className="text-sm text-muted-foreground">Limit access to selected features only</p></div>
                {!isAllRoutes && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </div>
            </div>
            {!isAllRoutes && (
              <div className="space-y-6 pt-2 pl-2">
                {selectedModules.length === 0 ? (
                  <p className="text-sm text-muted-foreground">Select at least one module in the section above first.</p>
                ) : (
                    ...
                    return (
                      <div key={modId} className="space-y-3">
                        <h4 className="font-semibold text-sm flex items-center gap-2 text-foreground">
                          {mod.icon && <mod.icon className="h-4 w-4 text-primary" />} {mod.label} Routes
                        </h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {routes.map((route) => (
                            <div key={route.id} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${selectedRoutes.includes(route.path) ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => toggleRoute(route.path)}>
                              ...
                              {selectedRoutes.includes(route.path) && <Check className="h-4 w-4 text-primary ml-auto shrink-0" />}
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        <section className="space-y-6">
          ...
          <div className="rounded-3xl border border-border/60 bg-card p-6 md:p-8 space-y-4">
            <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${!isTemporary ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => setIsTemporary(false)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-secondary flex items-center justify-center text-muted-foreground"><Shield className="h-5 w-5" /></div>
                  <div><p className="font-semibold">Permanent Access</p><p className="text-sm text-muted-foreground">No expiration — access until manually revoked</p></div>
                </div>
                {!isTemporary && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </div>
            </div>
            <div className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${isTemporary ? "border-primary bg-primary/5" : "border-border/60 hover:border-border"}`} onClick={() => setIsTemporary(true)}>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 rounded-full bg-secondary flex items-center justify-center text-muted-foreground"><CalendarDays className="h-5 w-5" /></div>
                  <div><p className="font-semibold">Temporary Access</p><p className="text-sm text-muted-foreground">Set a date when access automatically expires</p></div>
                </div>
                {isTemporary && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </div>
            </div>
            {isTemporary && (
              <div className="space-y-2 pl-[3.25rem] mt-4 animate-in slide-in-from-top-2">
                ...
                <Input id="expires-at" type="date" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} className="h-11 rounded-xl bg-secondary/50 max-w-xs" min={new Date().toISOString().split("T")[0]} />
              </div>
            )}
          </div>
        </section>
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-64 z-30 bg-background/80 backdrop-blur-md border-t border-border/60 p-6 flex justify-end">
        <div className="max-w-4xl mx-auto w-full flex items-center justify-between">
          <Button variant="ghost" className="gap-2 rounded-xl" onClick={() => navigate({ to: `/dashboard/${workspaceSlug}/users` })}>
            Cancel
          </Button>
          <Button className="gap-2 rounded-xl px-8 shadow-[var(--shadow-glow)]" style={{ background: "var(--gradient-primary)" }} onClick={handleSubmit} disabled={editMutation.isPending}>
            {editMutation.isPending ? <><Loader2 className="h-4 w-4 animate-spin" /> Saving Changes...</> : <><Check className="h-4 w-4" /> Save Changes</>}
          </Button>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-card w-full max-w-lg rounded-3xl shadow-xl border border-border overflow-hidden animate-in zoom-in-95">
            <div className="flex items-center justify-between p-5 border-b border-border">
              ...
              <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => setIsAvatarPickerOpen(false)}><X className="h-4 w-4" /></Button>
            </div>
            <div className="p-4 border-b border-border/60 bg-secondary/10">
              <label className="cursor-pointer flex items-center gap-3 p-3 rounded-xl hover:bg-secondary/50 transition-colors">
                <input type="file" accept="image/*" className="hidden" onChange={handleAvatarFileUpload} />
                <div className="h-9 w-9 rounded-xl bg-primary/10 flex items-center justify-center"><Upload className="h-4 w-4 text-primary" /></div>
                <div><p className="text-sm font-medium">Upload custom photo</p><p className="text-xs text-muted-foreground">PNG, JPG or GIF up to 5MB</p></div>
              </label>
            </div>
            <div className="p-4 border-b border-border/60 bg-secondary/10 overflow-x-auto whitespace-nowrap scrollbar-hide">
              <div className="flex gap-2">
                {AVATAR_CATEGORIES.map((cat) => (
                  <button key={cat.id} onClick={() => setActiveCategory(cat.id)} className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors whitespace-nowrap ${activeCategory === cat.id ? "bg-primary text-primary-foreground" : "bg-secondary hover:bg-secondary/80 text-muted-foreground"}`}>{cat.label}</button>
                ))}
              </div>
            </div>
            <div className="p-5 grid grid-cols-4 gap-3 max-h-64 overflow-y-auto">
              {avatarOptions.map((av) => (
                <button key={av} onClick={() => { setImage(av); setIsAvatarPickerOpen(false); }} className={`aspect-square w-full rounded-2xl border-2 flex items-center justify-center transition-all overflow-hidden ${image === av ? "border-primary ring-2 ring-primary ring-offset-2 ring-offset-background scale-105" : "border-transparent bg-secondary/50 hover:bg-secondary hover:scale-105"}`}>
            ...
            <div className="p-4 border-t border-border bg-secondary/20 flex gap-2">
              <Button onClick={() => setAvatarOptions(generateAvatars(activeCategory))} variant="outline" className="rounded-xl gap-2 flex-1"><Dices className="h-4 w-4" /> Randomize</Button>
              <Button onClick={() => setIsAvatarPickerOpen(false)} variant="ghost" className="rounded-xl">Done</Button>
            </div>
          </div>
        </div>
      )}
...
print("Generated new edit.tsx successfully!")
