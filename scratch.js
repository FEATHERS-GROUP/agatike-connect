...
content = content.replace(/import { createFileRoute, Link, useNavigate, redirect } from "@tanstack\/react-router";\n/, 'import { Link } from "@tanstack/react-router";\n');
...
content = content.replace(/function FacilityCheckoutPage\(\) \{[\s\S]*?const queryClient = useQueryClient\(\);/s, 
...
content = content.replace(/<main className="flex-1 max-w-7xl mx-auto w-full px-4 md:px-8 py-8">/, `<main className="flex-1 w-full px-4 md:px-6 py-6">`);
