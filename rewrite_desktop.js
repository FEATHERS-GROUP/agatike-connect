...
content = content.replace(/import \{ createFileRoute, useNavigate, Link \} from "@tanstack\/react-router";\n/, 'import { useNavigate, Link } from "@tanstack/react-router";\n');

content = content.replace(/function SubscriberPortal\(\) \{/, `export function SubscriberPortalDesktop({
  subscriptionId,
  user,
  subscription,
  space,
...
content = content.replace(/const \{ subscriptionId \} = Route.useParams\(\);\n  const \{ user \} = useUserAuth\(\);\n/, '');
...
content = content.replace(/const calendarEvents = React.useMemo.*?\n  \}, \[sessions, resourceBookings, user\?\.id\]\);\n/s, '');

// Remove the loading and not found states as they will be handled by the parent
content = content.replace(/if \(isSubLoading \|\| isResourcesLoading \|\| isClassesLoading \|\| isResourceBookingsLoading \|\| !sessions\) \{.*?return \(.*?\}\);\n  \}\n/s, '');
...
content = content.replace(/<div className="min-h-screen flex flex-col bg-background\/50">/, '<div className="hidden md:flex min-h-screen flex-col bg-background/50">');
