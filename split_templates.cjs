const fs = require('fs');
const content = fs.readFileSync('src/components/desktop/dashboard/ticket-designer/TicketPreview.tsx', 'utf-8');

// I will use regex to extract the blocks!
// Desktop blocks:
const desktopConcert = content.match(/if \(template === "concert"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)Perf}\n\s*{Stub}\n\s*<\/div>\n\s*\);\n\s*}/)[0];

const desktopMovie = content.match(/if \(template === "movie"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={96} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const desktopExperience = content.match(/if \(template === "experience"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={100} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const desktopConference = content.match(/if \(template === "conference"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={52} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const desktopEntrance = content.match(/if \(template === "entrance"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={84} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

// Mobile blocks:
const mobileRegex = /if \(previewMode === "Mobile"\) {([\s\S]*?)\/\/ Fallback \(shouldn't hit\)\n    return null;\n  }/;
const mobileBlock = content.match(mobileRegex)[1];

const mobileConcert = mobileBlock.match(/if \(template === "concert"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={140} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const mobileMovie = mobileBlock.match(/if \(template === "movie"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={140} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const mobileExperience = mobileBlock.match(/if \(template === "experience"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={140} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const mobileConference = mobileBlock.match(/if \(template === "conference"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={140} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];

const mobileEntrance = mobileBlock.match(/if \(template === "entrance"\) {([\s\S]*?)return \(\n\s*<div\n\s*id="ticket-preview-container"([\s\S]*?)qrValue \|\| orderId} size={140} \/>\n\s*<\/div>([\s\S]*?)<\/div>\n\s*\);\n\s*}/)[0];
