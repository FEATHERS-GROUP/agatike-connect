...
async function run() {
  ...
  const dropRes = await fetch(process.env.HASURA_ADMIN_API.replace('/graphql', '/metadata'), {
    ...
    body: JSON.stringify({
      type: "bulk",
      args: [
        { type: "pg_drop_relationship", args: { table: "events", relationship: "agatike_book_records" } },
        { type: "pg_untrack_table", args: { table: { schema: "public", name: "agatike_book_records" }, cascade: true } }
      ]
    }),
  });
